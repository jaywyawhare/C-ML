...
**Static kernels** — 8 `MTLComputePipelineState` objects compiled once at `cml_metal_backend_init` from a single embedded MSL source:

| PSO | Description |
|-----|-------------|
| `k_fill` | Scalar broadcast fill |
| `k_relu` | ReLU |
| `k_sigmoid` | Sigmoid |
| `k_tanh_k` | Tanh |
| `k_sum_reduce` | Sum reduction (two-pass tree, 256-thread threadgroup) |
| `k_max_reduce` | Max reduction (two-pass tree) |
| `k_matmul_opt` | 64×64 tiled GEMM with 8×8 register block accumulation |
| `k_matmul_fused_bias_relu` | Tiled GEMM + row-bias add + ReLU in one pass |

`k_matmul_opt` dispatches when `M%64==0 && N%64==0 && K%16==0`; otherwise falls back to 16×16 tiling. `k_matmul_fused_bias_relu` is selected when the graph contains a MATMUL→ADD(bias)→RELU chain.

**Dynamic kernel cache** — 48-entry table (`dyn_cache`) keyed by `UOpType`. MSL for an unsupported op is generated by `cml_metal_generate_msl`, compiled with `newLibraryWithSource:`, and its `MTLComputePipelineState` is retained in the cache. All subsequent graph executions reuse the cached PSO. Cache entries are CFReleased in `cml_metal_backend_free`.

Supported op types via codegen: binary (add, sub, mul, div, pow, cmplt, mod, idiv, max), unary (neg, exp, log, sqrt, recip, abs, sin, cos, tan, floor, ceil), activations (relu, sigmoid, tanh, relu6, elu, selu, silu, mish, hardswish), reductions (sum, max, min, mean, prod), fill, where, gather, matmul, conv2d.

Ops with no codegen path fall back to CPU: the tensor is downloaded (D2H), the CPU kernel executes, and the result is re-uploaded (H2D).

### Buffer cache

256-entry table (`buffers[]`) mapping CPU `data_ptr → MTLBuffer`. On Apple Silicon, `MTLResourceStorageModeShared` gives unified CPU/GPU memory — H2D and D2H are `memcpy`, not PCIe transfers.

- **Input buffers** (`is_input=true`): marked by `cml_metal_get_or_upload_buffer`. Survive graph executions; re-uploaded only when the CPU pointer's content changes.
- **Output buffers**: allocated by `cml_metal_alloc_output_buffer`, pooled by tensor pointer.
- `cml_metal_release_intermediate_buffers` invalidates non-input entries after graph execution.
- Selective D2H: the executor only downloads tensors whose output is not consumed by a later GPU node in the same graph.
...
int cml_metal_encode_kernel(CMLMetalBackend* backend,
                             void* encoder,           /* id<MTLComputeCommandEncoder> */
...
**Header:** `include/ops/ir/gpu/opencl_ir_backend.h`
...
A cross-vendor IR graph backend. It compiles a set of static OpenCL C kernels at `init` time, executes IR graphs on the GPU, and only transfers the final output back to CPU. Intermediate tensors stay on the GPU across nodes within a single graph execution.

### GEMM Kernels

Three GEMM kernels are compiled at init time:

| Kernel | When used | Configuration |
|--------|-----------|---------------|
| `matmul` | M,N multiples of 128 and K multiple of 16 | TSM=TSN=128, TSK=16, 8×8 register block, float4 global loads, A transposed in SLM |
| `matmul_fused_bias_relu` | Fused matmul + row-bias + ReLU, same alignment | Same tile config as `matmul`; saves two extra kernel launches |
| `matmul_naive` | Small or non-aligned dimensions | One thread per output element; always correct |

The BEAM autotuner (see below) takes priority over `matmul` when enabled and the shape meets alignment requirements.

### Dispatch Logic

```
ocl_exec_matmul(M, N, K):
  if BEAM enabled && M,N >= 64:
      autotune / cache-lookup → best parameterized variant
  elif M%128==0 && N%128==0 && K%16==0:
      matmul  (float4 loads, A-transposed SLM)
  else:
      matmul_naive
```

For fused patterns (`matmul → add_bias → relu` in the IR graph), the executor detects the pattern and dispatches `matmul_fused_bias_relu` in a single kernel launch.

### GPU Threshold

Graphs with fewer than 50M FLOPs are executed on CPU to avoid kernel launch and H2D/D2H overhead dominating.

### BEAM GEMM Autotuner

The OpenCL backend includes a built-in per-shape GEMM autotuner. On the first call for a given (M, N, K) shape, it benchmarks parameterized tile variants and caches the winner.

**Parameters searched:**

| Dimension | Values |
|-----------|--------|
| Tile size M×N | 128×128, 128×64, 64×128, 64×64 |
| K tile (TSK) | 16, 8 |
| Register block | 8×8, 8×4, 4×8, 4×4 |
| SLM row padding | 0, 1 |

Each valid combination is compiled into its own `beam_gemm_N` OpenCL kernel at backend init. Variants requiring more than 80 registers or exceeding 64 KB SLM are rejected before compilation.

**Timing:** Uses a dedicated OpenCL profiling command queue (`CL_QUEUE_PROFILING_ENABLE`). Each candidate runs 1 warmup + 3 timed executions; the median is used.

**Cache:** Results are stored in a 64-entry per-session hash table keyed by `(M<<40)|(N<<20)|K`. Shapes are re-tuned across process restarts (no disk persistence).
...
// Graph execution (called automatically by the CML runtime)
int                 cml_opencl_execute_graph(CMLOpenCLIRBackend* backend, CMLGraph_t ir);
```

### Buffer Management

The backend tracks up to 512 GPU buffers (`CML_OCL_MAX_TRACKED_BUFFERS`). Leaf input tensors are cached across graph executions by CPU data pointer, avoiding redundant H2D uploads when the same tensor is reused. Intermediate tensors are pooled and reused within a session.
