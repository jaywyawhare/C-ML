...
bool dtype_vec_valid(VecDType vt) {
    ...
    if (vt.scalar >= DTYPE_FLOAT8_E4M3 && vt.n > 2)
        return false;

    if (vt.scalar == DTYPE_BOOL && vt.n > 1)
        return false;
    return true;
}
...
static const char* dtype_c_base(DType d) {
    ...
    case DTYPE_FLOAT8_E4M3_FNUZ:
        return "__fp8_e4m3_fnuz";
    case DTYPE_FLOAT8_E5M2_FNUZ:
        return "__fp8_e5m2_fnuz";
    default:
        return "unknown";
    }
}
