...
Tensor* tensor_concat(Tensor** tensors, int num_tensors, int dim) {
    ...
    for (int t = 0; t < num_tensors; t++) {
        ...
        for (int d = normalized_dim + 1; d < tensor->ndim; d++) {
            slice_size *= tensor->shape[d];
        }

        size_t num_slices = 1;
        for (int d = 0; d < normalized_dim; d++) {
            num_slices *= tensor->shape[d];
        }

        size_t slice_elements = slice_size * tensor->shape[normalized_dim];

        // Copy each slice
        for (size_t s = 0; s < num_slices; s++) {
            size_t src_offset = s * slice_elements;
            size_t dst_offset = s * (slice_size * total_size) + offset * slice_size;
...
Tensor* tensor_stack(Tensor** tensors, int num_tensors, int dim) {
    ...
    int* out_shape = CM_MALLOC(out_ndim * sizeof(int));
    if (!out_shape)
        return NULL;
...
Tensor** tensor_split(Tensor* tensor, int num_splits, int dim, int* split_sizes) {
    ...
    Tensor** results = CM_MALLOC(num_splits * sizeof(Tensor*));
    if (!results)
        return NULL;
    ...
    if (split_sizes) {
        ...
        sizes         = CM_MALLOC(num_splits * sizeof(int));
        if (!sizes) {
            CM_FREE(results);
            return NULL;
        }
    ...
    for (int i = 0; i < num_splits; i++) {
        ...
        size_t slice_elements = slice_size * sizes[i];
        size_t src_offset     = offset * slice_size;

        for (size_t s = 0; s < num_slices; s++) {
            memcpy(split_data + s * slice_elements,
                   tensor_data + s * (slice_size * dim_size) + src_offset,
...
Tensor* tensor_gather(Tensor* input, Tensor* indices, int dim) {
    ...
    if (!input_strides || !output_strides) {
        tensor_free(output);
        if (input_strides)
            CM_FREE(input_strides);
        if (output_strides)
            CM_FREE(output_strides);
        return NULL;
    }
    ...
    for (size_t i = 0; i < output->numel; i++) {
        ...
        int* out_indices = CM_MALLOC(output->ndim * sizeof(int));
        size_t idx       = i;
        for (int d = output->ndim - 1; d >= 0; d--) {
            out_indices[d] = idx % output->shape[d];
            idx /= output->shape[d];
        }
        ...
        for (int d = 0; d < output->ndim; d++) {
            idx_offset += out_indices[d] * output_strides[d];
        }
        ...
        int* in_indices = CM_MALLOC(input->ndim * sizeof(int));
        for (int d = 0; d < input->ndim; d++) {
        ...
        size_t in_offset = 0;
        for (int d = 0; d < input->ndim; d++) {
            in_offset += in_indices[d] * input_strides[d];
        }
...
Tensor* tensor_scatter(Tensor* input, int dim, Tensor* index, Tensor* src) {
    ...
    for (size_t i = 0; i < src->numel; i++) {
        ...
        int* src_indices = CM_MALLOC(src->ndim * sizeof(int));
        size_t idx       = i;
        for (int d = src->ndim - 1; d >= 0; d--) {
            src_indices[d] = idx % src->shape[d];
            idx /= src->shape[d];
        }
        ...
        for (int d = 0; d < src->ndim; d++) {
            idx_offset += src_indices[d] * src_strides[d];
        }
        ...
        size_t out_offset = 0;
        for (int d = 0; d < output->ndim; d++) {
            out_offset += out_indices[d] * output_strides[d];
        }
