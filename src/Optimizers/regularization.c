...
Node *compute_regularization_node(Node *param, RegularizerConfig config)
{
    ...
    {
        Node *abs_param = pow_tensor(pow_tensor(param, tensor(2.0f, 0)), tensor(0.5f, 0));
        result = mul(tensor(config.lambda, 0), abs_param);
        break;
    }
    case L2_REGULARIZATION:
    {
        Node *squared = pow_tensor(param, tensor(2.0f, 0));
    ...
    case ELASTIC_NET:
    {
        Node *l1_term = mul(tensor(config.lambda * config.l1_ratio, 0),
                           pow_tensor(pow_tensor(param, tensor(2.0f, 0)), tensor(0.5f, 0)));
        Node *l2_term = mul(tensor(0.5f * config.lambda * (1.0f - config.l1_ratio), 0),
                           pow_tensor(param, tensor(2.0f, 0)));
                                ...
                                float gamma, int step_size)
{
    ...
    case STEP_LR:
    {
        ...
        result = mul(lr, pow_tensor(gamma_node, power));
        break;
    }
    case EXPONENTIAL_LR:
    {
        ...
        Node *gamma_node = tensor(gamma, 0);
        result = mul(lr, pow_tensor(gamma_node, epoch_node));
        break;
    }
    case COSINE_LR:
    {
        ...
        Node *cos_term = mul(pi_node, div_tensor(epoch_node, step_node));
        Node *cos_val = tensor(cosf(cos_term->tensor->storage->data[0]), 0);
        Node *scale = mul(tensor(0.5f, 0), add(tensor(1.0f, 0), cos_val));
        result = mul(lr, scale);
        break;
    }
    default:
