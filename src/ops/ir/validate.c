...
static void push_diag(CMLValidateDiag* diags, int max_diags, int* count, CMLValidateCode code,
    ...
    if (!diags)
        return;
    if (max_diags > 0 && (*count) > max_diags)
        return;
...
static bool dfs_has_cycle(struct IRNode** arr, int n, uint8_t* color, int idx,
    ...
    if (color[idx] == 2)
        return false;
    if (color[idx] == 1) {
        push_diag(diags, max_diags, ndiags, CML_VALID_CYCLE, idx, "Cycle detected at node %d (%s)",
                  idx, uop_type_to_string(arr[idx]->type));
        return true;
    }
    color[idx]          = 1;
...
#define RECORD(code, idx, ...)                                                                     \
    ...
    for (int i = 0; i < num_nodes; ++i) {
        ...
        if (exp >= 0 && n->num_inputs < exp) {
            RECORD(CML_VALID_MISSING_INPUT, i, "Node %d (%s): has %d inputs, expected %d", i,
                   uop_type_to_string(n->type), n->num_inputs, exp);
        }
        if (n->inputs) {
            for (int s = 0; s < n->num_inputs; ++s) {
                if (!n->inputs[s]) {
                    RECORD(CML_VALID_MISSING_INPUT, i, "Node %d (%s): input slot %d is NULL", i,
                           uop_type_to_string(n->type), s);
                }
            }
        }
    }
    ...
    if (opts->check_cycles) {
        ...
        if (color) {
            for (int i = 0; i < num_nodes; ++i)
                if (color[i] == 0)
                    dfs_has_cycle(nodes, num_nodes, color, i, diags, max_diag_count, &ndiags);
            cml_free(color);
        }
    }
    ...
    if (opts->check_dtypes) {
        for (int i = 0; i < num_nodes; ++i) {
            ...
            if (!is_binary(n->type))
                continue;
            if (n->num_inputs < 2 || !n->inputs)
                continue;
            Tensor* a = n->inputs[0];
            Tensor* b = n->inputs[1];
            if (a && b && a->dtype != b->dtype) {
                RECORD(CML_VALID_DTYPE_MISMATCH, i,
                       "Node %d (%s): input dtypes mismatch (%d vs %d)", i,
                       uop_type_to_string(n->type), a->dtype, b->dtype);
            }
        }
    }
        ...
        for (int i = 0; i < num_nodes; ++i) {
            ...
            if (n->is_executed && !n->output) {
                RECORD(CML_VALID_SHAPE_MISMATCH, i,
                       "Node %d (%s): executed but output tensor is NULL", i,
...
void cml_validate_print_diags(const CMLValidateDiag* diags, int num_diags) {
    for (int i = 0; i < num_diags; ++i) {
        ...
        fprintf(stderr, "[validate] node=%d %s: %s\n", d->node_index,
                cml_validate_code_str(d->code), d->message);
    }
}
...
const char* cml_validate_code_str(CMLValidateCode code) {
    ...
    case CML_VALID_OK:
        return "OK";
    case CML_VALID_NULL_GRAPH:
        return "NULL_GRAPH";
    case CML_VALID_NULL_NODE:
        return "NULL_NODE";
    case CML_VALID_CYCLE:
        return "CYCLE";
    case CML_VALID_MISSING_INPUT:
        return "MISSING_INPUT";
    case CML_VALID_TOO_MANY_INPUTS:
        return "TOO_MANY_INPUTS";
    case CML_VALID_SHAPE_MISMATCH:
        return "SHAPE_MISMATCH";
    case CML_VALID_DTYPE_MISMATCH:
        return "DTYPE_MISMATCH";
    case CML_VALID_INVALID_AXIS:
        return "INVALID_AXIS";
    case CML_VALID_UNKNOWN_OP:
        return "UNKNOWN_OP";
    case CML_VALID_EMPTY_GRAPH:
        return "EMPTY_GRAPH";
    case CML_VALID_DEAD_OUTPUT:
        return "DEAD_OUTPUT";
    case CML_VALID_INTERNAL:
        return "INTERNAL";
    default:
        return "UNKNOWN";
    }
