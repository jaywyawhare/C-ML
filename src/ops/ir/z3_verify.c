...
#elif defined(_WIN32)
static void* z3_load_sym(const char* name) { return (void*)GetProcAddress((HMODULE)z3.lib, name); }
#else
    ...
    while (node && node_count < 256) {
        ...
        if (!node->inputs || node->num_inputs == 0) {
            ...
            if (found == node->num_inputs) {
                node_exprs[node_count] = z3_build_node_expr(ctx, node, in_exprs, node->num_inputs);
            }
        }
    ...
    while (node) {
        ...
        if (node->output_shape && node->output_ndim > 0) {
            for (int d = 0; d < node->output_ndim; d++) {
                ...
                Z3_ast bounds[2]    = {z3.mk_ge(ctx, idx, zero), z3.mk_lt(ctx, idx, size)};
