...
static FusionMatch* match_softmax_ce_bwd(struct IRNode* start, struct CMLGraph* ir) {
    ...
    if (!sub_node || sub_node->type != UOP_SUB)
        return NULL;
...
FusionPatternRegistry* cml_fusion_registry_create(void) {
    ...
    for (int t = 0; t < FUSION_TARGET_COUNT; t++) {
        cml_fusion_register_pattern(reg, "matmul_bias_relu", FUSION_PATTERN_MATMUL_BIAS_RELU,
                                    (FusionTarget)t, 100, match_matmul_bias_relu,
                                    emit_matmul_bias_relu);

        cml_fusion_register_pattern(reg, "elementwise_chain", FUSION_PATTERN_ELEMENTWISE_CHAIN,
                                    (FusionTarget)t, 50, match_elementwise_chain,
                                    emit_elementwise_chain);

        cml_fusion_register_pattern(reg, "softmax_ce_bwd", FUSION_PATTERN_SOFTMAX_CE_BWD,
                                    (FusionTarget)t, 90, match_softmax_ce_bwd, emit_softmax_ce_bwd);

        cml_fusion_register_pattern(reg, "layernorm_bwd", FUSION_PATTERN_LAYERNORM_BWD,
                                    (FusionTarget)t, 85, match_layernorm_bwd, emit_layernorm_bwd);

        cml_fusion_register_pattern(reg, "gelu_bwd", FUSION_PATTERN_GELU_BWD, (FusionTarget)t, 80,
                                    match_gelu_bwd, emit_gelu_bwd);
    }
