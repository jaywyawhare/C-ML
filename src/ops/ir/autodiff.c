...
static Tensor* ad_reshape(Tensor* x, const int* shape, int ndim) {
    ReshapeParams p;
    int buf[16];
    memcpy(buf, shape, (size_t)ndim * sizeof(int));
    p.new_shape = buf;
    p.new_ndim  = ndim;
...
static Tensor* ad_permute(Tensor* x, const int* perm, int ndim) {
    PermuteParams p;
    int buf[16];
    memcpy(buf, perm, (size_t)ndim * sizeof(int));
    p.perm     = buf;
    p.num_dims = ndim;
...
static Tensor* ad_sum(Tensor* x, const int* dims, int ndims, bool keepdim) {
    ...
    int buf[16];
    if (dims && ndims > 0) {
        memcpy(buf, dims, (size_t)ndims * sizeof(int));
        p.dims = buf;
    } else
        p.dims = NULL;
    p.num_dims = ndims;
...
int cml_ir_grad(CMLGraph_t ir, struct IRNode* loss_node, bool differentiable_grads) {
    if (!ir || !loss_node || !loss_node->output)
        return -1;
    ...
    for (int i = n - 1; i >= 0; i--) {
        ...
        if (!g)
            continue; /* nothing downstream needs this */
        ...
        case UOP_SPMM: {
            ...
            if (spp && nd->num_inputs >= 5) {
                ...
                if (vals->numel == 0) {
                    ...
                    gm_accum(&map, vals,
                             uop_fill_ex(vals->shape, vals->ndim, 0.0f, vals->dtype, vals->device));
                    gm_accum(&map, B, uop_fill_ex(B->shape, B->ndim, 0.0f, B->dtype, B->device));
                    break;
                }
        ...
        case UOP_IM2COL: { /* dX = col2im(g) — scatter windows back */
            ...
            if (ip && a->ndim == 4) {
                Col2imParams cp = {ip->kh, ip->kw, ip->sh,      ip->sw,      ip->ph,     ip->pw,
                                   ip->dh, ip->dw, a->shape[1], a->shape[2], a->shape[3]};
        ...
        case UOP_SQRT: /* 0.5/sqrt(x) = 0.5/out */
            gm_accum(&map, a,
                     uop_mul(g, uop_mul(uop_fill(out->shape, out->ndim, 0.5f), uop_recip(out))));
            break;
        ...
        case UOP_ASIN: /* 1/sqrt(1-x²) */
            gm_accum(&map, a,
                     uop_mul(g, uop_rsqrt(uop_max(uop_sub(ad_k(a, 1.0f), uop_square(a)),
                                                  ad_k(a, 1e-12f)))));
            break;
        case UOP_ACOS: /* -1/sqrt(1-x²) */
            gm_accum(&map, a,
                     uop_neg(uop_mul(g, uop_rsqrt(uop_max(uop_sub(ad_k(a, 1.0f), uop_square(a)),
                                                          ad_k(a, 1e-12f))))));
            break;
        case UOP_ATAN: /* 1/(1+x²) */
        ...
        case UOP_ACOSH: /* 1/sqrt(x²-1) */
            gm_accum(&map, a,
                     uop_mul(g, uop_rsqrt(uop_max(uop_sub(uop_square(a), ad_k(a, 1.0f)),
                                                  ad_k(a, 1e-12f)))));
            break;
        case UOP_ATANH: /* 1/(1-x²) */
            gm_accum(&map, a,
                     uop_mul(g, uop_recip(uop_max(uop_sub(ad_k(a, 1.0f), uop_square(a)),
                                                  ad_k(a, 1e-12f)))));
            break;
        case UOP_ERF: /* 2/sqrt(pi) · e^(-x²) */
            gm_accum(
                &map, a,
                uop_mul(g, uop_mul(ad_k(a, 1.1283791670955126f), uop_exp(uop_neg(uop_square(a))))));
            break;
        ...
        case UOP_HARD_SIGMOID: /* 1/6 on (-3, 3), else 0 */
            gm_accum(&map, a,
                     uop_mul(uop_mul(g, ad_k(a, 1.0f / 6.0f)),
                             uop_mul(uop_cmpgt(a, ad_k(a, -3.0f)), uop_cmplt(a, ad_k(a, 3.0f)))));
            break;
        case UOP_HARD_TANH: /* 1 on (-1, 1), else 0 */
            gm_accum(
                &map, a,
                uop_mul(g, uop_mul(uop_cmpgt(a, ad_k(a, -1.0f)), uop_cmplt(a, ad_k(a, 1.0f)))));
            break;
        case UOP_RELU6: /* 1 on (0, 6), else 0 */
            gm_accum(&map, a,
                     uop_mul(g, uop_mul(uop_cmpgt(a, ad_k(a, 0.0f)), uop_cmplt(a, ad_k(a, 6.0f)))));
            break;
        case UOP_QUICK_GELU: { /* x·s(1.702x): s + 1.702·x·s·(1-s) */
            Tensor* sg = uop_sigmoid(uop_mul(a, ad_k(a, 1.702f)));
            Tensor* d  = uop_add(
                sg, uop_mul(uop_mul(ad_k(a, 1.702f), a), uop_mul(sg, uop_sub(ad_k(a, 1.0f), sg))));
        ...
        case UOP_SOFTSIGN: /* 1/(1+|x|)² */
            gm_accum(&map, a,
                     uop_mul(g, uop_recip(uop_square(uop_add(ad_k(a, 1.0f), uop_abs(a))))));
            break;
        ...
        case UOP_DIV:
            ...
            gm_accum(&map, b, unbroadcast(uop_neg(uop_div(uop_mul(g, out), b)), b->shape, b->ndim));
            break;
        case UOP_SQUARE:
        ...
        case UOP_RSQRT: /* -½·out³ */
            gm_accum(&map, a,
                     uop_neg(uop_mul(g, uop_mul(ad_k(a, 0.5f), uop_mul(out, uop_mul(out, out))))));
            break;
        case UOP_MINIMUM: {
        ...
        case UOP_TRIU:
        case UOP_TRIL: { /* masked entries contributed nothing */
        ...
        case UOP_PROD: { /* d/dx_i prod = prod / x_i */
            ...
            if (ob && gb)
                gm_accum(&map, a, uop_mul(gb, uop_div(ob, a)));
            break;
        }
        case UOP_LOGSUMEXP: { /* softmax(x) = e^(x - out) */
            ...
            if (ob && gb)
                gm_accum(&map, a, uop_mul(gb, uop_exp(uop_sub(a, ob))));
            break;
        }
        ...
        case UOP_LOGCUMSUMEXP: { /* e^x_i · revcum(g·e^-out) */
            ...
            if (r)
                gm_accum(&map, a, uop_mul(uop_exp(a), r));
            break;
        }
        case UOP_CUMMAX:
        case UOP_CUMMIN: {
        ...
        case UOP_TILE: {
            ...
            if (tp && tp->repeats && a->ndim <= 8 && a->ndim == out->ndim) {
                ...
                if (r)
                    gm_accum(&map, a, ad_reshape(r, a->shape, a->ndim));
            }
            break;
        }
        ...
        case UOP_DIAG: {
            ...
            if (a->ndim == 1)
                gm_accum(&map, a, uop_diagonal(g, off, 0, 1)); /* 1-D -> matrix */
            else
                gm_accum(&map, a, uop_diag(g, off)); /* matrix -> 1-D */
            break;
        }
        case UOP_DIAGONAL: {
            DiagParams* dp = (DiagParams*)nd->params;
        ...
        case UOP_LERP: { /* a + t·(b-a) */
            ...
            if (t) {
                gm_accum(&map, a,
                         unbroadcast(uop_mul(g, uop_sub(ad_k(g, 1.0f), t)), a->shape, a->ndim));
                if (b)
                    gm_accum(&map, b, unbroadcast(uop_mul(g, t), b->shape, b->ndim));
                if (b)
                    gm_accum(&map, t, unbroadcast(uop_mul(g, uop_sub(b, a)), t->shape, t->ndim));
            }
            break;
        }
        ...
        case UOP_SORT: {
            ...
            if (dim >= 0 && dim < a->ndim) {
                ...
                if (perm)
                    gm_accum(&map, a, uop_scatter_add(perm, g, dim, a->shape[dim]));
            }
            break;
        }
        case UOP_TOPK: {
        ...
        case UOP_MASKED_SELECT: {
            ...
            if (nd->num_inputs >= 2 && b) {
                ...
                if (gg)
                    gm_accum(&map, a, ad_reshape(uop_mul(gg, bf), a->shape, a->ndim));
            }
            break;
        }
        case UOP_STRIDE:
        ...
        case UOP_POW: { /* out=aᵇ: da=g·b·out/a, db=g·out·ln(a) */
            gm_accum(
                &map, a,
                unbroadcast(uop_mul(uop_mul(uop_mul(g, b), out), uop_recip(a)), a->shape, a->ndim));
...
#define FE_OPERAND(ref)                                                                            \
    ((ref) >= 0 ? ((ref) < nd->num_inputs && nd->inputs[ref] ? nd->inputs[ref] : NULL)             \
                ...
                for (int st = 0; st < ns && fe_ok; st++) {
                    ...
                    switch (fp->op[st]) {
                    case UOP_ADD:
                        r = vb ? uop_add(va, vb) : NULL;
                        break;
                    case UOP_SUB:
                        r = vb ? uop_sub(va, vb) : NULL;
                        break;
                    case UOP_MUL:
                        r = vb ? uop_mul(va, vb) : NULL;
                        break;
                    case UOP_DIV:
                        r = vb ? uop_div(va, vb) : NULL;
                        break;
                    case UOP_MAX:
                        r = vb ? uop_max(va, vb) : NULL;
                        break;
                    case UOP_MINIMUM:
                        r = vb ? uop_minimum(va, vb) : NULL;
                        break;
                    case UOP_POW:
                        r = vb ? uop_pow(va, vb) : NULL;
                        break;
                    case UOP_NEG:
                        r = uop_neg(va);
                        break;
                ...
                for (int st = ns - 1; st >= 0 && gstep[st]; st--) {
                    ...
                    case UOP_POW:
                        ...
                        cb =
                            unbroadcast(uop_mul(uop_mul(gg, vs), uop_log(va)), vb->shape, vb->ndim);
                        break;
                    case UOP_NEG:
                        ca = uop_neg(gg);
                        break;
                    case UOP_RECIP:
                        ca = uop_neg(uop_mul(gg, uop_mul(vs, vs)));
                        break;
                    case UOP_EXP:
                        ca = uop_mul(gg, vs);
                        break;
                    case UOP_LOG:
                        ...
                        break;
                    case UOP_SQRT:
                        ca = uop_mul(gg, uop_mul(vs, ad_k(va, 0.5f)));
                        break;
                    case UOP_SIN:
                        ca = uop_mul(gg, uop_cos(va));
                        break;
                    case UOP_COS:
                        ca = uop_neg(uop_mul(gg, uop_sin(va)));
                        break;
                    case UOP_ABS:
                        ca = uop_mul(gg, uop_sign(va));
                        break;
                    case UOP_RELU: {
                    ...
                    case UOP_WHERE:
                        ...
                        if (vc)
                            cc = uop_mul(gg, uop_sub(ad_k(vc, 1.0f), vc));
                        if (vb)
                            cb = uop_mul(gg, va);
                        break;
                    case UOP_FILL:
        ...
        default: {
            ...
            if (autodiff_strict_grad()) {
                ...
                error_stack_push(CM_NOT_IMPLEMENTED,
                                 "autograd: no VJP for this op under CML_STRICT_GRAD=1", __FILE__,
                                 __LINE__, __func__);
                return -1;
            }
            break;
    ...
    for (int i = 0; i < map.count; i++) {
        ...
        if (v && v->requires_grad) {
            ...
            if (ir->grad_publish_count == ir->grad_publish_cap) {
                ...
                ir->grad_publish_log = cml_realloc(ir->grad_publish_log,
