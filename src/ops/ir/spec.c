...
static void validate_tensor_level(CMLGraph_t graph, CMLSpecResult* result) {
    ...
    while (node) {
        ...
        if (is_binary_op(node->type) && node->num_inputs != 2) {
            spec_add_error(result, idx, "binary op requires exactly 2 inputs", CML_SPEC_TENSOR);
        }
        if (is_unary_op(node->type) && node->num_inputs != 1) {
            spec_add_error(result, idx, "unary op requires exactly 1 input", CML_SPEC_TENSOR);
        }
        ...
        if (node->num_inputs >= 2 && node->inputs && node->inputs[0] && node->inputs[1]) {
            ...
            if ((is_float_dtype(d0) && is_int_dtype(d1)) ||
                ...
                if (is_binary_op(node->type)) {
                    spec_add_error(result, idx, "mixed int/float without cast", CML_SPEC_TENSOR);
                }
            }
        }

        if (is_binary_op(node->type) && node->num_inputs == 2 && node->inputs && node->inputs[0] &&
            node->inputs[1]) {
            ...
            if (a->ndim > 0 && b->ndim > 0 && a->shape && b->shape) {
                ...
                for (int d = 0; d < max_nd; d++) {
                    ...
                    if (da != db && da != 1 && db != 1) {
                        spec_add_error(result, idx, "shapes not broadcast-compatible",
                                       CML_SPEC_TENSOR);
                        break;
                    }
                }
            ...
            if (rp->dims && node->inputs && node->inputs[0]) {
                ...
                for (int d = 0; d < rp->num_dims; d++) {
                    if (rp->dims[d] < 0 || rp->dims[d] >= ndim) {
                        spec_add_error(result, idx, "reduce axis out of bounds", CML_SPEC_TENSOR);
                        break;
                    }
                }
            }
        ...
        for (int v = 0; v < num_visited; v++) {
            if (visited[v] == node) {
                spec_add_error(result, idx, "cycle detected in graph", CML_SPEC_TENSOR);
                goto done_cycle;
            }
        }
...
static void validate_kernel_level(CMLGraph_t graph, CMLSpecResult* result) {
    ...
    while (node) {
        ...
        if (!found) {
            spec_add_error(result, idx, "node not assigned to any fusion group", CML_SPEC_KERNEL);
        }
        node = node->next;
        idx++;
    }
    ...
    for (int g = 0; g < sched->num_groups; g++) {
        ...
        for (int n = 0; n < grp->num_nodes; n++) {
            if (!grp->nodes[n]) {
                spec_add_error(result, g, "group references freed node", CML_SPEC_KERNEL);
            }
        }
    }
        ...
        for (int n = 0; n < grp->num_nodes; n++) {
            ...
            if (out->ndim > 0 && out->shape) {
                ...
                for (int d = 0; d < out->ndim; d++) {
                    if (out->shape[d] < 0) {
                        spec_add_error(result, g, "negative buffer dimension", CML_SPEC_KERNEL);
                        break;
                    }
    ...
    for (int g = 0; g < sched->num_groups; g++) {
        ...
        for (int i = 0; i < prog->num_ops; i++) {
            ...
            if (op->kind == LINOP_LOAD && !op->is_eliminated) {
                ...
                if (!matched) {
                    spec_add_error(result, g, "LOAD without any matching STORE", CML_SPEC_LINEAR);
                }
                break;
            }
        }
        ...
        for (int i = 0; i < prog->num_ops; i++) {
            ...
            if (op->kind == LINOP_LOOP) {
                ...
                loop_depth--;
                if (loop_depth < 0) {
                    spec_add_error(result, g, "ENDLOOP without matching LOOP", CML_SPEC_LINEAR);
                    break;
                }
            }
        }
        if (loop_depth != 0) {
            spec_add_error(result, g, "mismatched LOOP/ENDLOOP nesting", CML_SPEC_LINEAR);
        }
    ...
    for (int g = 0; g < sched->num_groups; g++) {
        ...
        for (int n = 0; n < grp->num_nodes; n++) {
            ...
            if (nd->output && nd->output->ndim > 0 && nd->output->shape) {
                ...
                if (!valid || sz == 0) {
                    spec_add_error(result, g, "buffer has zero or negative size", CML_SPEC_PROGRAM);
                }
            }
        }
        ...
        for (int d = 0; d < 3; d++) {
            if (prog->group_dims[d] <= 0) {
                spec_add_error(result, g, "kernel launch dimension is non-positive",
                               CML_SPEC_PROGRAM);
                break;
            }
        }
        ...
        if (prog->next_vreg > MAX_VIRTUAL_REGS) {
            spec_add_error(result, g, "register count exceeds hardware limit", CML_SPEC_PROGRAM);
        }
...
void cml_spec_result_print(const CMLSpecResult* result) {
    ...
    static const char* level_names[] = {"TENSOR", "KERNEL", "LINEAR", "PROGRAM"};

    printf("Spec Validation: %s (%d error%s)\n", result->valid ? "PASS" : "FAIL",
           result->num_errors, result->num_errors == 1 ? "" : "s");
