...
static CMLTCHardware tc_detect_hardware(void) {
    ...
    if (cml_wmma_available())
        return TC_HW_WMMA;
    if (cml_amx_available())
        return TC_HW_AMX;
    if (cml_xmx_available())
        return TC_HW_XMX;
    return TC_HW_NONE;
}
...
static void extract_matmul_dims(struct IRNode* node, int* m, int* n, int* k) {
    ...
    *k = 0;
    if (!node || node->num_inputs < 2)
        return;
    if (!node->input_shapes || !node->input_ndims)
        return;
...
static int rewrite_matmul_to_wmma(CMLGraph_t ir, struct IRNode* node) {
    ...
    struct IRNode* wmma = create_wmma_node(final_a, final_b, pm, pn, pk, out_shape, out_ndim);
    cml_free(out_shape);
    if (!wmma)
        return 0;
...
static bool is_fused_matmul_pattern(CMLGraph_t ir, struct IRNode* reduce_node,
    ...
    *out_b = cml_ir_find_by_output(ir, expand_b->input_names[0]);
    ...
    if (!(*out_a)->output_shape || !(*out_b)->output_shape)
        return false;
    if ((*out_a)->output_ndim < 2 || (*out_b)->output_ndim < 2)
        return false;
...
static int rewrite_fused_matmul(CMLGraph_t ir, struct IRNode* reduce_node, struct IRNode* src_a,
    ...
    struct IRNode* wmma =
        create_wmma_node(src_a->output_name, src_b->output_name, pm, pn, pk, out_shape, out_ndim);
    cml_free(out_shape);
    if (!wmma)
        return 0;
