...
static int vk_submit_and_wait(CMLVulkanBackend* vk, VkCommandBuffer cmd) {
    ...
    /* Submit */
    HCQ_VkSubmitInfo submit = {.sType                = VK_STYPE_SUBMIT_INFO,
                               .pNext                = NULL,
...
static VkCommandBuffer vk_begin_one_time_cmd(CMLVulkanBackend* vk) {
    HCQ_VkCmdBufAllocInfo alloc_info = {.sType              = VK_STYPE_CMD_BUF_ALLOC_INFO,
                                        .pNext              = NULL,
    ...
    HCQ_VkCmdBufBeginInfo begin = {.sType            = VK_STYPE_CMD_BUF_BEGIN_INFO,
                                   .pNext            = NULL,
...
static int vk_create_staging_buffer(CMLVulkanBackend* vk, size_t bytes, uint32_t usage_bits,
                                    VkBuffer* out_buf, VkDeviceMemory* out_mem) {
    HCQ_VkBufferCreateInfo buf_ci = {.sType                 = VK_STYPE_BUFFER_CREATE_INFO,
                                     .pNext                 = NULL,
                                     .flags                 = 0,
                                     .size                  = (uint64_t)bytes,
                                     .usage                 = usage_bits,
    ...
    HCQ_VkMemoryAllocInfo alloc = {.sType           = VK_STYPE_MEMORY_ALLOC_INFO,
                                   .pNext           = NULL,
...
int cml_hcq_vulkan_submit_kernel(CMLHCQQueue* queue, const CMLHCQKernelDesc* desc) {
    ...
    vk->vkCmdBindDescriptorSets(cmd, VK_BIND_POINT_COMPUTE, kernel->pipeline_layout, 0, 1,
                                &kernel->desc_set, 0, NULL);

    /* Dispatch */
    vk->vkCmdDispatch(cmd, (uint32_t)desc->grid[0], (uint32_t)desc->grid[1],
...
int cml_hcq_vulkan_memcpy_h2d(CMLHCQQueue* queue, void* dst, const void* src, size_t bytes) {
    ...
    if (vk_create_staging_buffer(vk, bytes, VK_BUF_USAGE_TRANSFER_SRC, &staging, &staging_mem) != 0)
        return -1;
...
int cml_hcq_vulkan_memcpy_d2h(CMLHCQQueue* queue, void* dst, const void* src, size_t bytes) {
    ...
    VkBuffer staging           = NULL;
    VkDeviceMemory staging_mem = NULL;
    if (vk_create_staging_buffer(vk, bytes, VK_BUF_USAGE_TRANSFER_DST, &staging, &staging_mem) != 0)
        return -1;
