...
static CMLScheduleItemType dominant_type(CMLScheduleItemType a, CMLScheduleItemType b) {
    if (a == SCHED_MATMUL || b == SCHED_MATMUL)
        return SCHED_MATMUL;
    if (a == SCHED_CONV || b == SCHED_CONV)
        return SCHED_CONV;
    if (a == SCHED_REDUCE || b == SCHED_REDUCE)
        return SCHED_REDUCE;
...
static int fusion_group_add_node(CMLFusionGroup* g, struct IRNode* node) {
    ...
    if (g->num_nodes >= g->node_capacity) {
        ...
        struct IRNode** tmp = cml_realloc(g->nodes, (size_t)nc * sizeof(struct IRNode*));
        if (!tmp)
            return -1;
        g->nodes         = tmp;
        g->node_capacity = nc;
    ...
    CMLScheduleItemType kind = classify_fusion_op(node->type);
...
CMLFusionSchedule* cml_fusion_schedule_create(CMLGraph_t graph, const CMLScheduleOptions* opts) {
    ...
    while (node) {
        ...
        if (cur && cur->num_nodes > 0 && opts->enable_fusion) {
            ...
            if (!can_extend && opts->allow_reduce_elem_fusion &&
                cml_schedule_is_reduction(last->type) && cml_schedule_is_elementwise(node->type)) {
    ...
    sched->fusion_ratio =
        (sched->num_groups > 0) ? (float)total_ops / (float)sched->num_groups : 0.0f;
...
void cml_fusion_schedule_print(const CMLFusionSchedule* sched) {
    ...
    for (int i = 0; i < sched->num_groups; i++) {
        ...
        printf("  [%d] %-12s  nodes=%d  color=%d  flops=%zu  mem=%zu  elim=%d\n", i,
               fusion_sched_type_name(g->type), g->num_nodes, g->color, g->total_flops,
               g->total_memory, g->num_eliminated);
