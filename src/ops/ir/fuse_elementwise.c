...
static size_t node_numel(struct IRNode* n) {
    ...
    if (n->output_shape && n->output_ndim > 0) {
        size_t p = 1;
        for (int i = 0; i < n->output_ndim; i++)
            p *= (size_t)n->output_shape[i];
        return p;
    }
    return 0;
}
...
static int is_trailing_bcast(struct IRNode* rootnode, Tensor* in) {
    ...
    if (!osh || ond <= 0 || in->ndim > ond)
        return 0;
    for (int i = 0; i < in->ndim; i++)
        if (in->shape[i] != osh[ond - in->ndim + i])
            return 0;
    return 1;
}
...
int cml_ir_fuse_elementwise(CMLGraph_t ir) {
    ...
    for (int r = n - 1; r >= 0; r--) {
        ...
        if (consumed[r] || !fe_supported(root->type))
            continue;
        if (root->is_executed || (root->output && root->output->is_executed))
            continue;
        ...
        if (root->requires_grad && !g_fe_allow_grad)
            continue;
        if (!node_is_f32(root))
            continue;
        ...
        for (int qi = 0; qi < m; qi++) {
            ...
            for (int k = 0; k < cur->num_inputs && cur->inputs; k++) {
                ...
                if (consumed[pid])
                    continue;
                if (!fe_supported(prod->type))
                    continue;
                if (!node_is_f32(prod))
                    continue; /* fused kernel is f32-only */
                if (prod->requires_grad && !g_fe_allow_grad)
                    continue; /* see root guard */
                ...
                if (prod->is_executed || (prod->output && prod->output->is_executed))
                    continue;
                if (use[pid] != 1)
                    continue; /* used elsewhere -> keep materialized */
                if (node_numel(prod) != onumel)
                    continue; /* different shape -> external broadcast input */
                if (m >= 256)
                    break;
                consumed[pid] = 1;
                members[m++]  = prod;
            ...
            if (nd->type == UOP_FILL) {
                ...
                for (int k = 0; k < nd->num_inputs && k < 3; k++) {
                    ...
                    if (internal >= 0) {
                        ...
                        if (en != onumel && en != 1 &&
                            !(en > 0 && onumel % en == 0 && is_trailing_bcast(root, in))) {
                            ok = 0;
                            break;
                        }
                        if (!in || in->dtype != DTYPE_FLOAT32) {
                            ok = 0;
                            break;
                        }
                        int idx = -1;
        ...
        for (struct IRNode* p = ir->head; p && p != root; p = p->next)
            prev = p;
        ...
        while (cnode) {
            ...
            if (cnode->input_names) {
                for (int i = 0; i < cnode->num_inputs; i++)
                    cml_free(cnode->input_names[i]);
                cml_free(cnode->input_names);
            }
            cml_free(cnode->inputs);
            cml_free(cnode->output_name);
            cml_free(cnode->output_shape);
        ...
        root->output = NULL;
        if (root->input_names) {
            for (int i = 0; i < root->num_inputs; i++)
                cml_free(root->input_names[i]);
            cml_free(root->input_names);
        }
        cml_free(root->inputs);
        cml_free(root->output_name);
        cml_free(root->output_shape);
        cml_intern_remove(ir->intern_table, root);
        cml_ir_free_node_params(root);
        cml_free(root);
...
int cml_ir_fuse_matmul_epilogue(CMLGraph_t ir) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (M->type != UOP_MATMUL || M->params)
            continue; /* plain gemm only */
        if (M->is_executed || (M->output && M->output->is_executed))
            continue;
        ...
        if (M->requires_grad)
            continue;
        if (use[i] != 1 || !M->output)
            continue; /* single consumer */
        /* find the unique consumer F */
        struct IRNode* F = NULL;
        int mi           = -1;
        for (int j = 0; j < n && !F; j++) {
            struct IRNode* c = nodes[j];
            for (int k = 0; k < c->num_inputs && c->inputs; k++)
                if (c->inputs[k] == M->output) {
                    F  = c;
                    mi = k;
                    break;
                }
        }
        if (!F || F->type != UOP_FUSED_ELEMENTWISE || !F->params)
            continue;
        if (F->requires_grad)
            continue; /* inference only (see above) */
        if (!F->output || !node_is_f32(F) || !node_is_f32(M))
            continue;
        if (node_numel(F) != node_numel(M))
            continue; /* epilogue is elementwise over the gemm output */
        FusedElementwiseParams* sf = (FusedElementwiseParams*)F->params;
        if (F->num_inputs > 33 || sf->num_steps <= 0)
            continue;
...
#undef REMAP
        ...
        if (M->output)
            M->output->ir_node = NULL;
        M->output = F->output;
        if (M->output)
            M->output->ir_node = M;
        if (M->output_shape) {
            cml_free(M->output_shape);
            M->output_shape = NULL;
        }
        if (F->output_shape && F->output_ndim > 0) {
            M->output_shape = cml_malloc((size_t)F->output_ndim * sizeof(int));
            if (M->output_shape) {
                memcpy(M->output_shape, F->output_shape, (size_t)F->output_ndim * sizeof(int));
                M->output_ndim = F->output_ndim;
            }
        }

        /* Unlink + free F's shell (its output was transferred to M). */
        struct IRNode* prev = NULL;
        for (struct IRNode* p = ir->head; p && p != F; p = p->next)
            prev = p;
        if (prev)
            prev->next = F->next;
        else
        ...
        if (F->input_names) {
            for (int k = 0; k < F->num_inputs; k++)
                cml_free(F->input_names[k]);
            cml_free(F->input_names);
        }
        cml_free(F->inputs);
        cml_free(F->output_name);
        cml_free(F->output_shape);
        cml_ir_free_node_params(F);
        cml_free(F);
