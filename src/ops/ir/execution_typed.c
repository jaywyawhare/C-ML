...
bool cml_exec_needs_typed(struct IRNode* node, Tensor* out) {
    if (!node)
        return false;
    if (handles_own_dtype(node->type))
        return false;
    ...
    for (int i = 0; i < node->num_inputs && node->inputs; i++) {
        ...
        if (t && t->data && t->dtype != DTYPE_FLOAT32)
            return true;
    }
...
static int unary_f64(UOpType type, double x, double* r, struct IRNode* node) {
    ...
    case UOP_RELU:
        *r = x < 0.0 ? 0.0 : x;
        return 0;
    case UOP_RELU6:
        *r = isnan(x) ? x : fmin(fmax(x, 0.0), 6.0);
        return 0;
    case UOP_HARD_TANH:
        *r = isnan(x) ? x : fmin(fmax(x, -1.0), 1.0);
        return 0;
    case UOP_SOFTSIGN:
        *r = x / (1.0 + fabs(x));
        return 0;
    ...
    case UOP_HARD_SIGMOID:
        *r = fmin(fmax((x + 3.0) / 6.0, 0.0), 1.0);
        return 0;
    case UOP_QUICK_GELU:
        *r = x * (1.0 / (1.0 + exp(-1.702 * x)));
        return 0;
    ...
    case UOP_SOFTPLUS:
        *r = fmax(x, 0.0) + log1p(exp(-fabs(x)));
        return 0;
    case UOP_LOGSIGMOID:
        *r = fmin(x, 0.0) - log1p(exp(-fabs(x)));
        return 0;
    case UOP_MISH:
        *r = x * tanh(fmax(x, 0.0) + log1p(exp(-fabs(x))));
        return 0;
    case UOP_GELU: {
...
static int binary_i64(UOpType type, int64_t x, int64_t y, int64_t* r) {
    ...
    case UOP_BITWISE_XOR:
        *r = x ^ y;
        return 0;
    case UOP_LSHIFT:
        *r = (y >= 0 && y < 64) ? (int64_t)((uint64_t)x << y) : 0;
        return 0;
    case UOP_RSHIFT:
        *r = (y >= 0 && y < 64) ? (x >> y) : (x < 0 ? -1 : 0);
        return 0;
...
static int exec_unary(struct IRNode* node, Tensor* out) {
    ...
    double probe;
    if (unary_f64(node->type, 0.5, &probe, node) != 0)
        return -1;

    for (size_t i = 0; i < out->numel; i++) {
        double x = cml_load_f64(a->data, bcast_idx(a, out, i), a->dtype), r;
        if (unary_f64(node->type, x, &r, node) != 0)
            return -1;
...
static size_t bcast_idx(const Tensor* inp, const Tensor* out, size_t flat_i) {
    ...
    if (ond > 16 || ind > 16 || ond <= 0 || ind <= 0)
        return 0;
    out_str[ond - 1] = 1;
    for (int d = ond - 2; d >= 0; d--)
        out_str[d] = out_str[d + 1] * (size_t)out->shape[d + 1];
    inp_str[ind - 1] = 1;
    for (int d = ind - 2; d >= 0; d--)
        inp_str[d] = inp_str[d + 1] * (size_t)inp->shape[d + 1];
...
static void pad_nd_typed(const void* ad, const Tensor* a, const Tensor* out, void* od,
    ...
    for (size_t i = 0; i < out->numel; i++) {
        ...
        if (ok)
            ...
            cml_store_f64(od, i, out->dtype, (double)p->value);
    }
}
...
static void scatter_nd_typed(struct IRNode* node, const void* ad, const Tensor* idx_t,
    ...
    for (size_t j = 0; j < idx_t->numel; j++) {
        ...
        for (int d = idx_t->ndim - 1; d >= 0; d--) {
            ...
            if (d == dim)
                c = (size_t)(int)num_at(node, 1, j);
            if (c >= (size_t)out->shape[d]) {
                ok = 0;
                break;
            }
            off += c * out_str[d];
...
static int layout_kernel(struct IRNode* node, Tensor* out, void* od) {
    ...
    case UOP_RAND_UNIFORM:
        return cml_cpu_lazy_rand_uniform(out);
    case UOP_RAND_NORMAL:
        return cml_cpu_lazy_rand_normal(out);
    case UOP_RAND_INT: {
    ...
    case UOP_UNFLATTEN:
        if (!ad)
            return -1;
        if (od == ad)
            return 0; /* view shares the buffer */
        if (an == out->numel) {
            memcpy(od, ad, out->numel * esz);
            return 0;
        }
        for (size_t i = 0; i < out->numel; i++)
            ELEM_COPY(od, i, ad, bidx(i, an), esz);
        return 0;
    ...
    case UOP_CAT: {
        ...
        if (dim < 0 || node->num_inputs < 1)
            return -1;

        size_t outer = 1, inner = 1;
        for (int d = 0; d < dim; d++)
            outer *= (size_t)out->shape[d];
        for (int d = dim + 1; d < out->ndim; d++)
            inner *= (size_t)out->shape[d];
        size_t out_count = (size_t)out->shape[dim];

        size_t placed = 0;
        for (int t = 0; t < node->num_inputs; t++) {
            Tensor* s = in_at(node, t);
            if (!s || !s->data || s->ndim != out->ndim)
                return -1;
            size_t cnt = (size_t)s->shape[dim];
            if (placed + cnt > out_count)
                return -1;
    ...
    case UOP_SCATTER: {
        ...
        if (!ad || !idx || !idx->data || !src || !src->data)
            return -1;
        {
    ...
    case UOP_GATHER: {
        ...
        Tensor* idx = in_at(node, 1);
        if (!ad || !od || !idx || !idx->data || idx->numel == 0)
            return -1;
        ...
        size_t outer = 1, inner = 1;
        for (int d = 0; d < dim; d++)
            outer *= (size_t)a->shape[d];
        for (int d = dim + 1; d < a->ndim; d++)
            inner *= (size_t)a->shape[d];
        ...
        for (size_t o = 0; o < outer; o++) {
            for (size_t j = 0; j < idx->numel; j++) {
                ...
                if (iv < 0 || (size_t)iv >= dim_size) {
                    LOG_ERROR("typed GATHER: index %lld out of bounds [0, %zu)", (long long)iv,
                              dim_size);
                    return -1;
                }
                for (size_t k = 0; k < inner && oi < out->numel; k++, oi++)
                    ELEM_COPY(od, oi, ad, o * dim_size * inner + (size_t)iv * inner + k, esz);
            }
        }
        return 0;
    }
        ...
        for (size_t o = 0; o < outer; o++)
            for (size_t j = 0; j < cnt; j++)
                for (size_t m = 0; m < inner; m++)
                    ELEM_COPY(od, (o * cnt + (j + (size_t)s) % cnt) * inner + m, ad,
                              (o * cnt + j) * inner + m, esz);
        return 0;
    }
    ...
    case UOP_FLIP: {
        ...
        for (size_t o = 0; o < outer; o++)
            for (size_t j = 0; j < cnt; j++)
                for (size_t m = 0; m < inner; m++)
                    ELEM_COPY(od, (o * cnt + (cnt - 1 - j)) * inner + m, ad,
                              (o * cnt + j) * inner + m, esz);
        return 0;
    }
        ...
        for (size_t o = 0; o < outer; o++)
            for (size_t j = 0; j < cnt; j++)
                for (size_t r = 0; r < reps; r++)
                    for (size_t m = 0; m < inner; m++)
                        ELEM_COPY(od, ((o * cnt + j) * reps + r) * inner + m, ad,
                                  (o * cnt + j) * inner + m, esz);
        return 0;
    }
    ...
    case UOP_WHERE: {
        ...
        for (size_t i = 0; i < out->numel; i++) {
            ...
            if (s->dtype == out->dtype)
                ELEM_COPY(od, i, s->data, si, esz);
            else
                cml_store_f64(od, i, out->dtype, cml_load_f64(s->data, si, s->dtype));
        }
        return 0;
    }
    ...
    case UOP_MASKED_SELECT: {
        ...
        for (size_t i = 0; i < an && i < m->numel; i++)
            if (num_at(node, 1, i) != 0.0)
                ELEM_COPY(od, cnt++, ad, i, esz);
...
static int exec_reduce_like(struct IRNode* node, Tensor* out) {
    ...
    if (t == UOP_TRACE) {
        ...
        double s = 0;
        for (int i = 0; i < n; i++)
            s += cml_load_f64(a->data, (size_t)i * cols + i, a->dtype);
    ...
    for (size_t o = 0; o < outer; o++) {
        for (size_t m = 0; m < inner; m++) {
            ...
            case UOP_ALL:
                acc = 1.0;
                for (size_t j = 0; j < cnt; j++)
                    if (cml_load_f64(a->data, base + j * inner, a->dtype) == 0.0) {
                        acc = 0.0;
                        break;
                    }
                break;
            ...
            size_t oi = o * inner + m;
            if (oi < out->numel)
                cml_store_f64(out->data, oi, out->dtype, acc);
        }
    }
    return 0;
}
...
static int exec_compute(struct IRNode* node, Tensor* out) {
    ...
    case UOP_LINEAR: {
        ...
        for (size_t m = 0; m < rows; m++)
            for (size_t n = 0; n < N; n++) {
                ...
                if (b && b->data)
                    s += cml_load_f64(b->data, n, b->dtype);
                        ...
                        for (int ow = 0; ow < OW; ow++) {
                            ...
                            for (int ic = 0; ic < IC; ic++)
                                for (int kd = 0; kd < KD; kd++) {
                                    int iz =
                                        odp * p->stride[0] - p->padding[0] + kd * p->dilation[0];
                                    if (iz < 0 || iz >= ID)
                                        continue;
                                    for (int kh = 0; kh < KH; kh++) {
                                        ...
                                        for (int kw = 0; kw < KW; kw++) {
                                            ...
                                            if (ix < 0 || ix >= IW)
                                                continue;
                                            size_t ii =
                                                ((((size_t)b * IC + ic) * ID + iz) * IH + iy) * IW +
                                                ix;
                                            size_t wi =
                                                ((((size_t)oc * IC + ic) * KD + kd) * KH + kh) *
                                                    KW +
                                                kw;
            ...
            for (int ic = 0; ic < IC; ic++)
                for (int ih = 0; ih < IH; ih++)
                    for (int iw = 0; iw < IW; iw++) {
                        double v = cml_load_f64(
                            a->data, (((size_t)b * IC + ic) * IH + ih) * IW + iw, a->dtype);
                        for (int oc = 0; oc < OC; oc++)
                            for (int kh = 0; kh < KH; kh++) {
            ...
            for (int ic = 0; ic < IC; ic++)
                for (int id = 0; id < ID; id++)
                    for (int ih = 0; ih < IH; ih++)
                        for (int iw = 0; iw < IW; iw++) {
                            double v = cml_load_f64(
                                a->data, ((((size_t)b * IC + ic) * ID + id) * IH + ih) * IW + iw,
                                a->dtype);
                            for (int oc = 0; oc < OC; oc++)
                                for (int kd = 0; kd < KD; kd++) {
                                    int odp =
                                    ...
                                    for (int kh = 0; kh < KH; kh++) {
                                        ...
                                        for (int kw = 0; kw < KW; kw++) {
                                            ...
                                            if (ow < 0 || ow >= OW)
                                                continue;
                                            size_t wi =
                                                ((((size_t)ic * OC + oc) * KD + kd) * KH + kh) *
                                                    KW +
                                                kw;
                                            size_t oi =
                                                ((((size_t)b * OC + oc) * OD + odp) * OH + oh) *
                                                    OW +
                                                ow;
                                            cml_store_f64(
                                                od, oi, out->dtype,
    ...
    case UOP_FUSED_ELEMENTWISE: {
        ...
        if (!p || p->num_steps <= 0 || p->num_steps > 256)
            return -1;
        int ns       = p->num_steps;
        ...
        for (size_t i = 0; i < out->numel; i++) {
            for (int s = 0; s < ns; s++) {
                ...
                if (t == UOP_FILL)
                    ...
                    r = (va != 0.0) ? vb : vc;
                else if (binary_f64(t, va, vb, &r) != 0 && unary_f64(t, va, &r, node) != 0) {
                    cml_free(step);
                    return -1;
                }
                step[s] = r;
...
static int dispatch(struct IRNode* node, Tensor* out) {
    if (exec_layout(node, out) == 0)
        return 0;
    if (exec_cumulative(node, out) == 0)
        return 0;
    if (exec_reduce_like(node, out) == 0)
        return 0;
    if (exec_order(node, out) == 0)
        return 0;
    if (exec_compute(node, out) == 0)
        return 0;
    if (node->num_inputs == 1 && exec_unary(node, out) == 0)
        return 0;
    if (node->num_inputs == 2 && exec_binary(node, out) == 0)
        return 0;
    return -1;
}
...
int cml_exec_typed(struct IRNode* node, Tensor* out) {
    ...
    bool aliases = false;
    for (int i = 0; i < node->num_inputs && node->inputs; i++)
        if (node->inputs[i] && node->inputs[i]->data == out->data)
            aliases = true;
    ...
    out->data      = saved;
    if (rc == 0 && produced)
        memcpy(saved, produced, out->numel * esz);
    if (produced == tmp)
        cml_free(tmp);
    return rc;
}
