...
static char* wgsl_binary_kernel(UOpType type) {
    ...
    case UOP_MUL:
        ...
        break;
    case UOP_DIV:
        expr = "a[idx] / (b[idx] + 1e-8)";
        break;
    case UOP_MAX:
        expr = "max(a[idx], b[idx])";
        break;
    case UOP_POW:
        expr = "pow(a[idx], b[idx])";
        break;
    case UOP_CMPLT:
        expr = "select(0.0, 1.0, a[idx] < b[idx])";
        break;
    case UOP_MOD:
        expr = "a[idx] - floor(a[idx] / (b[idx] + 1e-8)) * b[idx]";
        break;
    case UOP_IDIV:
        expr = "floor(a[idx] / (b[idx] + 1e-8))";
        break;
    default:
        return NULL;
    }
...
static char* wgsl_conv2d_kernel(void) {
    ...
    wgsl_appendf(&buf, &cap, &len,
                 ...
                 "                let ih_s = i32(oh * params.stride + kh) - i32(params.pad);\n"
                 "                let iw_s = i32(ow * params.stride + kw) - i32(params.pad);\n"
                 "                if (ih_s >= 0 && ih_s < i32(params.H) && iw_s >= 0 && iw_s < "
                 "i32(params.W)) {\n"
                 "                    let ih = u32(ih_s);\n"
                 "                    let iw = u32(iw_s);\n"
                 "                    let in_idx  = ((n * params.Cin + ci) * params.H + ih) * "
                 "params.W + iw;\n"
                 "                    let w_idx   = ((co * params.Cin + ci) * params.KH + kh) * "
                 "params.KW + kw;\n"
...
char* cml_wgsl_generate(struct IRNode* node) {
    ...
    if (type == UOP_SIGMOID)
        return wgsl_sigmoid_kernel();
    if (type == UOP_TANH)
        return wgsl_tanh_kernel();
    if (type == UOP_ELU)
        return wgsl_elu_kernel();
    if (type == UOP_SELU)
        return wgsl_selu_kernel();
    if (type == UOP_SILU)
        return wgsl_silu_kernel();
    if (type == UOP_MISH)
        return wgsl_mish_kernel();
    if (type == UOP_HARDSWISH)
        return wgsl_hardswish_kernel();

    if (type == UOP_SUM)
        return wgsl_sum_kernel();
    if (type == UOP_MAX_REDUCE)
        return wgsl_max_reduce_kernel();
    if (type == UOP_MIN_REDUCE)
        return wgsl_min_reduce_kernel();
    if (type == UOP_MEAN)
        return wgsl_mean_kernel();
    if (type == UOP_PROD)
        return wgsl_prod_kernel();

    if (type == UOP_MATMUL)
        return wgsl_matmul_kernel();
    if (type == UOP_CONV2D)
        return wgsl_conv2d_kernel();
    if (type == UOP_FILL)
        return wgsl_fill_kernel();
    if (type == UOP_WHERE)
        return wgsl_where_kernel();
    if (type == UOP_GATHER)
        return wgsl_gather_kernel();
...
int cml_webgpu_execute_graph(CMLWebGPUBackend* backend, CMLGraph_t graph) {
    ...
    while (node) {
        ...
        if (!wgsl_src) {
            LOG_ERROR("webgpu_execute_graph: codegen failed for UOp %d", (int)node->type);
            return -1;
        }

        CMLWebGPUKernel* kernel = cml_webgpu_compile_wgsl(backend, wgsl_src, "main");
        cml_free(wgsl_src);
        if (!kernel) {
            LOG_ERROR("webgpu_execute_graph: compile failed for UOp %d", (int)node->type);
            return -1;
        }
        ...
        if (ok == 0) {
            ...
            ok = cml_webgpu_launch_kernel(backend, kernel, workgroups, gpu_buffers, buffer_sizes,
                                          total_buffers);
        }

        if (ok == 0 && out_size > 0 && node->output && node->output->data) {
            ok =
                cml_webgpu_download(backend, node->output->data, gpu_buffers[num_inputs], out_size);
        }
        ...
        if (ok != 0) {
            LOG_ERROR("webgpu_execute_graph: execution failed for UOp %d", (int)node->type);
            return -1;
        }
