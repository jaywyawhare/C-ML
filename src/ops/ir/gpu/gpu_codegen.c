...
CMLGPUCodegen* cml_gpu_codegen_create(GPUTarget target, void* backend) {
    ...
    if (target == GPU_TARGET_CUDA) {
        ...
        snprintf(cg->target_cpu, sizeof(cg->target_cpu), "sm_%d%d",
                 cg->cuda->compute_capability_major, cg->cuda->compute_capability_minor);
...
static LLVMValueRef emit_global_thread_id(LLVMBuilderRef bld, LLVMModuleRef mod, LLVMContextRef ctx,
    ...
    if (target == GPU_TARGET_CUDA) {
        ...
        if (!tid_fn)
            tid_fn = LLVMAddFunction(mod, "llvm.nvvm.read.ptx.sreg.tid.x", intr_type);

        LLVMValueRef ctaid_fn = LLVMGetNamedFunction(mod, "llvm.nvvm.read.ptx.sreg.ctaid.x");
        if (!ctaid_fn)
            ctaid_fn = LLVMAddFunction(mod, "llvm.nvvm.read.ptx.sreg.ctaid.x", intr_type);

        LLVMValueRef ntid_fn = LLVMGetNamedFunction(mod, "llvm.nvvm.read.ptx.sreg.ntid.x");
        if (!ntid_fn)
            ntid_fn = LLVMAddFunction(mod, "llvm.nvvm.read.ptx.sreg.ntid.x", intr_type);
        ...
        if (!wid_fn)
            wid_fn = LLVMAddFunction(mod, "llvm.amdgcn.workitem.id.x", intr_type);

        LLVMValueRef gid_fn = LLVMGetNamedFunction(mod, "llvm.amdgcn.workgroup.id.x");
        if (!gid_fn)
            gid_fn = LLVMAddFunction(mod, "llvm.amdgcn.workgroup.id.x", intr_type);
...
static LLVMModuleRef gpu_build_binary_op(LLVMContextRef ctx, UOpType type, const char* fn_name,
                                         CMLGPUCodegen* cg) {
    ...
    switch (type) {
    case UOP_ADD:
        result = LLVMBuildFAdd(bld, v0, v1, "add");
        break;
    case UOP_SUB:
        result = LLVMBuildFSub(bld, v0, v1, "sub");
        break;
    case UOP_MUL:
        result = LLVMBuildFMul(bld, v0, v1, "mul");
        break;
    case UOP_DIV: {
    ...
    case UOP_POW: {
        ...
        if (!pow_fn)
            pow_fn = LLVMAddFunction(mod, "llvm.pow.f32", pow_ft);
    ...
    case UOP_IDIV: {
        ...
        if (!floor_fn)
            floor_fn = LLVMAddFunction(mod, "llvm.floor.f32", floor_ft);
...
static LLVMModuleRef gpu_build_unary_op(LLVMContextRef ctx, UOpType type, const char* fn_name,
    ...
    LLVMTypeRef f32 = t.f32, i32 = t.i32, i64 = t.i64, ptr1 = t.ptr1, void_t = t.void_t;
    (void)f32;
    (void)i64;
...
#define GPU_INTRINSIC1(name_str, arg, res_name)                                                    \
    do {                                                                                           \
        ...
        if (!_fn)                                                                                  \
            _fn = LLVMAddFunction(mod, name_str, _ft);                                             \
    ...
    case UOP_TAN: {
        ...
        if (!sin_fn)
            sin_fn = LLVMAddFunction(mod, "llvm.sin.f32", ft1);
        LLVMValueRef cos_fn = LLVMGetNamedFunction(mod, "llvm.cos.f32");
        if (!cos_fn)
            cos_fn = LLVMAddFunction(mod, "llvm.cos.f32", ft1);
    ...
    case UOP_HARD_SIGMOID: {
        ...
        if (!maxn)
            maxn = LLVMAddFunction(mod, "llvm.maxnum.f32", bin_ft);
        LLVMValueRef minn = LLVMGetNamedFunction(mod, "llvm.minnum.f32");
        if (!minn)
            minn = LLVMAddFunction(mod, "llvm.minnum.f32", bin_ft);
...
static LLVMModuleRef gpu_build_fill_op(LLVMContextRef ctx, const char* fn_name, float fill_value,
                                       CMLGPUCodegen* cg) {
    LLVMModuleRef mod = LLVMModuleCreateWithNameInContext(fn_name, ctx);
...
static LLVMModuleRef gpu_build_reduction(LLVMContextRef ctx, UOpType type, const char* fn_name,
                                         CMLGPUCodegen* cg) {
    LLVMModuleRef mod = LLVMModuleCreateWithNameInContext(fn_name, ctx);
    ...
    if (type == UOP_SUM || type == UOP_MEAN) {
        LLVMBuildAtomicRMW(bld, LLVMAtomicRMWBinOpFAdd, out_gep, val, LLVMAtomicOrderingMonotonic,
                           0);
        ...
        LLVMBuildAtomicRMW(bld, LLVMAtomicRMWBinOpXchg, out_gep, val, LLVMAtomicOrderingMonotonic,
                           0);
        LLVMBuildBr(bld, bb_end);

        LLVMPositionBuilderAtEnd(bld, bb_ord);
        LLVMBuildAtomicRMW(bld,
                           type == UOP_MAX_REDUCE ? LLVMAtomicRMWBinOpFMax : LLVMAtomicRMWBinOpFMin,
        ...
        LLVMValueRef xchg =
            LLVMBuildAtomicCmpXchg(bld, out_gep, cur, desired, LLVMAtomicOrderingMonotonic,
...
static LLVMModuleRef gpu_build_matmul(LLVMContextRef ctx, const char* fn_name, CMLGPUCodegen* cg) {
    LLVMModuleRef mod = LLVMModuleCreateWithNameInContext(fn_name, ctx);
...
static LLVMModuleRef gpu_build_conv2d(LLVMContextRef ctx, const char* fn_name, CMLGPUCodegen* cg) {
    LLVMModuleRef mod = LLVMModuleCreateWithNameInContext(fn_name, ctx);
...
static char* emit_gpu_code(CMLGPUCodegen* cg, LLVMModuleRef mod, size_t* out_size) {
    ...
    if (LLVMGetTargetFromTriple(cg->target_triple, &target, &err) != 0) {
        LOG_ERROR("GPU codegen: Failed to get target for '%s': %s", cg->target_triple,
                  err ? err : "unknown");
        LLVMDisposeMessage(err);
        return NULL;
    }
    ...
    LLVMTargetMachineRef tm =
        LLVMCreateTargetMachine(target, cg->target_triple, cg->target_cpu, "",
    ...
    LOG_DEBUG("GPU codegen: Emitted %zu bytes of %s code", size,
              cg->target == GPU_TARGET_CUDA ? "PTX" : "HSACO");
...
static bool is_binary_op(UOpType type) {
    return type == UOP_ADD || type == UOP_SUB || type == UOP_MUL || type == UOP_DIV ||
           ...
           type == UOP_MOD || type == UOP_MINIMUM || type == UOP_CMPGT || type == UOP_CMPGE ||
...
static bool is_unary_op(UOpType type) {
    return type == UOP_NEG || type == UOP_EXP || type == UOP_LOG || type == UOP_SQRT ||
           ...
           type == UOP_RECIP || type == UOP_SIGMOID || type == UOP_TANH || type == UOP_ELU ||
           type == UOP_SELU || type == UOP_MISH || type == UOP_SILU || type == UOP_HARDSWISH ||
           type == UOP_GELU || type == UOP_QUICK_GELU || type == UOP_LEAKY_RELU ||
           type == UOP_HARD_SIGMOID || type == UOP_HARD_TANH || type == UOP_RELU6 ||
           type == UOP_SQUARE || type == UOP_RSQRT || type == UOP_EXP2 || type == UOP_LOG2 ||
...
static int gpu_execute_node(CMLGPUCodegen* cg, struct IRNode* node) {
    ...
    matmul_cleanup:
        ...
        void* d_out = gpu_alloc_zero(cg, out->numel);
        if (!d_input || !d_weight || !d_bias || !d_out)
            goto conv2d_cleanup;
        ...
        void* args[] = {&d_input, &d_weight, &d_bias,    &d_out,     &batch,     &in_c,
                        &in_h,    &in_w,     &out_c_val, &out_h_val, &out_w_val, &kh,
                        &kw,      &sh,       &sw,        &ph,        &pw};
