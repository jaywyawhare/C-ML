...
static LLVMTargetMachineRef create_target_machine(CMLAMDLLVMCompiler* comp) {
    ...
    return LLVMCreateTargetMachine(target, comp->target_triple, comp->target_cpu, comp->features,
...
int cml_amd_llvm_compile_ir(CMLAMDLLVMCompiler* comp, const char* ir_source, void** code_object,
    ...
    *code_size   = 0;
    ...
    LLVMMemoryBufferRef ir_buf =
        LLVMCreateMemoryBufferWithMemoryRangeCopy(ir_source, strlen(ir_source), "amd_ir");
    ...
    LOG_INFO("AMD LLVM: compiled %zu bytes of AMDGPU code object for %s", *code_size,
             comp->target_cpu);
    return 0;
}
