...
char* cml_ptx_gen_unary(CMLPTXCodegen* cg, UOpType op, const char* kernel_name) {
    ...
    switch (op) {
    case UOP_NEG:
        pos += snprintf(ptx + pos, (size_t)(PTX_BUF_SIZE - pos), "    neg.f32 %%f1, %%f0;\n\n");
        break;
    case UOP_EXP:
        // exp(x) = exp2(x * log2(e))
    ...
    case UOP_SQRT:
        ...
        break;
    case UOP_ABS:
        pos += snprintf(ptx + pos, (size_t)(PTX_BUF_SIZE - pos), "    abs.f32 %%f1, %%f0;\n\n");
        break;
    case UOP_SIN:
        pos +=
            snprintf(ptx + pos, (size_t)(PTX_BUF_SIZE - pos), "    sin.approx.f32 %%f1, %%f0;\n\n");
        break;
    case UOP_COS:
        pos +=
            snprintf(ptx + pos, (size_t)(PTX_BUF_SIZE - pos), "    cos.approx.f32 %%f1, %%f0;\n\n");
        break;
    case UOP_TAN:
        // tan(x) = sin(x) / cos(x)
        pos += snprintf(ptx + pos, (size_t)(PTX_BUF_SIZE - pos),
    ...
    case UOP_RECIP:
        pos +=
            snprintf(ptx + pos, (size_t)(PTX_BUF_SIZE - pos), "    rcp.approx.f32 %%f1, %%f0;\n\n");
        break;
    case UOP_FLOOR:
        pos += snprintf(ptx + pos, (size_t)(PTX_BUF_SIZE - pos),
    ...
    case UOP_SQUARE:
        pos +=
            snprintf(ptx + pos, (size_t)(PTX_BUF_SIZE - pos), "    mul.f32 %%f1, %%f0, %%f0;\n\n");
        break;
    case UOP_RSQRT:
    ...
    case UOP_EXP2:
        pos +=
            snprintf(ptx + pos, (size_t)(PTX_BUF_SIZE - pos), "    ex2.approx.f32 %%f1, %%f0;\n\n");
        break;
    case UOP_LOG2:
        pos +=
            snprintf(ptx + pos, (size_t)(PTX_BUF_SIZE - pos), "    lg2.approx.f32 %%f1, %%f0;\n\n");
        break;
    case UOP_SIGN:
...
char* cml_ptx_gen_binary(CMLPTXCodegen* cg, UOpType op, const char* kernel_name) {
    ...
    case UOP_ADD:
        pos +=
            snprintf(ptx + pos, (size_t)(PTX_BUF_SIZE - pos), "    add.f32 %%f2, %%f0, %%f1;\n\n");
        break;
    case UOP_SUB:
        pos +=
            snprintf(ptx + pos, (size_t)(PTX_BUF_SIZE - pos), "    sub.f32 %%f2, %%f0, %%f1;\n\n");
        break;
    case UOP_MUL:
        pos +=
            snprintf(ptx + pos, (size_t)(PTX_BUF_SIZE - pos), "    mul.f32 %%f2, %%f0, %%f1;\n\n");
        break;
    case UOP_DIV:
        pos += snprintf(ptx + pos, (size_t)(PTX_BUF_SIZE - pos),
    ...
    case UOP_MAX:
        pos +=
            snprintf(ptx + pos, (size_t)(PTX_BUF_SIZE - pos), "    max.f32 %%f2, %%f0, %%f1;\n\n");
        break;
    case UOP_MINIMUM:
        // PTX min.f32 returns the non-NaN operand, as the existing MAX case
        pos +=
            snprintf(ptx + pos, (size_t)(PTX_BUF_SIZE - pos), "    min.f32 %%f2, %%f0, %%f1;\n\n");
        break;
    case UOP_CMPGT:
        pos += snprintf(ptx + pos, (size_t)(PTX_BUF_SIZE - pos),
...
char* cml_ptx_gen_reduction(CMLPTXCodegen* cg, UOpType op, const char* kernel_name) {
    if (!cg || !kernel_name)
        return NULL;
    if (op != UOP_SUM && op != UOP_MEAN && op != UOP_MAX_REDUCE && op != UOP_MIN_REDUCE &&
        op != UOP_PROD) {
        return NULL;
    }
...
char* cml_ptx_gen_conv2d(CMLPTXCodegen* cg, const char* kernel_name) {
    ...
    pos += snprintf(src + pos, (size_t)(CUDA_SRC_BUF_SIZE - pos),
                    ...
                    "                    int in_idx = ((n * in_channels + (ic_start + ic)) * in_h "
                    "+ ih) * in_w + iw;\n"
                    "                    int w_idx = ((oc * channels_per_group + ic) * kernel_h + "
                    "kh) * kernel_w + kw;\n"
...
static int ptx_execute_node(CMLPTXCodegen* cg, struct IRNode* node) {
    ...
    unary_cleanup:
        if (d_in)
            cml_cuda_free(cuda, d_in);
        if (d_out)
            cml_cuda_free(cuda, d_out);
    ...
    binary_cleanup:
        if (d_a)
            cml_cuda_free(cuda, d_a);
        if (d_b)
            cml_cuda_free(cuda, d_b);
        if (d_out)
            cml_cuda_free(cuda, d_out);
        cml_cuda_kernel_free(cuda, kernel);
        if (result == 0) {
    ...
    if (type == UOP_WHERE) {
        if (node->num_inputs < 3 || !node->inputs[0] || !node->inputs[1] || !node->inputs[2] ||
            !node->inputs[0]->data || !node->inputs[1]->data || !node->inputs[2]->data)
            return -1;
