...
void nv_qmd_init(NVQmd* qmd, uint32_t arch) {
    ...
    qmd_set_field(qmd->data, QMD3_OUTER_PUT_MODE_WORD, QMD3_OUTER_PUT_MODE_LO,
                  QMD3_OUTER_PUT_MODE_HI, 0x1);

    qmd_set_field(qmd->data, QMD3_VERSION_WORD, QMD3_VERSION_LO, QMD3_VERSION_HI, version);

    qmd_set_field(qmd->data, QMD3_MAJOR_VERSION_WORD, QMD3_MAJOR_VERSION_LO, QMD3_MAJOR_VERSION_HI,
                  0);

    qmd_set_field(qmd->data, QMD3_API_CALL_LIMIT_WORD, QMD3_API_CALL_LIMIT_LO,
                  QMD3_API_CALL_LIMIT_HI, 0x1);

    qmd_set_field(qmd->data, QMD3_SM_GLOBAL_CACHING_WORD, QMD3_SM_GLOBAL_CACHING_LO,
                  QMD3_SM_GLOBAL_CACHING_HI, 1);
...
void nv_qmd_set_register_count(NVQmd* qmd, uint32_t count) {
    if (count > 255)
        count = 255;
    qmd_set_field(qmd->data, QMD3_REGISTER_COUNT_WORD, QMD3_REGISTER_COUNT_LO,
...
void nv_qmd_set_barrier_count(NVQmd* qmd, uint32_t count) {
    if (count > 31)
        count = 31;
    qmd_set_field(qmd->data, QMD3_BARRIER_COUNT_WORD, QMD3_BARRIER_COUNT_LO, QMD3_BARRIER_COUNT_HI,
                  count);
}
...
static const char* elf_get_shstrtab(const uint8_t* data, size_t size, const Elf64_Ehdr_t* ehdr) {
    ...
    if (off + sizeof(Elf64_Shdr_t) > size)
        return NULL;

    const Elf64_Shdr_t* shstr = (const Elf64_Shdr_t*)(data + off);
    if (shstr->sh_offset + shstr->sh_size > size)
        return NULL;
