...
int cml_hcq_nv_submit_kernel(CMLHCQQueue* queue, const CMLHCQKernelDesc* desc) {
    ...
    uint32_t grid[3]  = {(uint32_t)desc->grid[0], (uint32_t)desc->grid[1], (uint32_t)desc->grid[2]};
    uint32_t block[3] = {(uint32_t)desc->block[0], (uint32_t)desc->block[1],
                         (uint32_t)desc->block[2]};

    int ret = cml_nv_kernel_launch(nv, (CMLNVKernel*)desc->compiled_kernel, grid, block, desc->args,
                                   desc->num_args);
    if (ret == 0) {
        qd->submit_count++;
...
int cml_hcq_nv_signal_wait_cpu(CMLHCQSignal* signal, uint64_t timeout_ms) {
    ...
    while (*nv->semaphore < target) {
        ...
        if (now >= deadline) {
            LOG_ERROR("NV HCQ: signal wait timed out (current=%u, target=%u)", *nv->semaphore,
                      target);
            return -1;
        }
