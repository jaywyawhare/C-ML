...
static const char* flame_kind(const struct IRNode* n) {
    ...
    if (n->type == UOP_FUSED_ELEMENTWISE || n->is_fused || n->fused_kernel || n->fusion_type != 0)
        return "fused";
...
static void flame_atexit_flush(void) {
    if (g_count > 0)
        cml_flame_export("flamegraph.json");
}
...
static FlameEntry* flame_find_or_add(const char* op, const char* kind, const char* phase,
    ...
    for (int i = 0; i < g_nentries; i++) {
        ...
        if (e->op == op && e->kind == kind && e->phase == phase && e->numel == numel &&
            ((e->scope == scope) || (e->scope && scope && strcmp(e->scope, scope) == 0)) &&
            ((e->stack == stack) || (e->stack && stack && strcmp(e->stack, stack) == 0)))
            return e;
    }
...
void cml_flame_record(const struct IRNode* node, double ms) {
    ...
    if (!registered) {
        atexit(flame_atexit_flush);
        registered = 1;
    }

    FlameEntry* e =
        flame_find_or_add(uop_type_to_string(node->type), flame_kind(node), flame_phase(node),
                          flame_numel(node), node->scope, node->build_stack);
    if (!e)
    ...
    if (g_nspans < FLAME_MAX_SPANS) {
        if (g_nspans >= g_scap) {
            ...
            if (ncap > FLAME_MAX_SPANS)
                ncap = FLAME_MAX_SPANS;
...
int cml_flame_export(const char* path) {
    ...
    fprintf(f, "{\"total_ms\":%.4f,\"num_spans\":%lld,\"aggregated\":true,\"spans\":[", g_total,
            g_count);
    for (int i = 0; i < g_nentries; i++) {
        FlameEntry* e = &g_entries[i];
        fprintf(f,
                "%s{\"i\":%d,\"op\":\"%s\",\"kind\":\"%s\",\"phase\":\"%s\","
                "\"scope\":\"%s\",\"stack\":\"%s\","
                "\"numel\":%lld,\"ms\":%.4f,\"count\":%lld,\"max_ms\":%.4f,"
                "\"first_ms\":%.4f}",
                i ? "," : "", i, e->op ? e->op : "?", e->kind ? e->kind : "other",
                e->phase ? e->phase : "forward", e->scope ? e->scope : "", e->stack ? e->stack : "",
                e->numel, e->total_ms, e->count, e->max_ms, e->first_ms < 0.0 ? 0.0 : e->first_ms);
    }
    fprintf(f, "]");
...
int cml_flame_export_chrome_trace(const char* path) {
    ...
    for (int i = 0; i < g_nspans; i++) {
        ...
        fprintf(f,
                ...
                i ? "," : "", e->op ? e->op : "?", e->kind ? e->kind : "other",
                (double)sp->t0 * 1000.0, (double)sp->dur * 1000.0, (int)getpid(),
                e->phase ? e->phase : "forward", e->scope ? e->scope : "", e->numel);
    }
