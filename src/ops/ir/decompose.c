...
static struct IRNode* create_primitive_node(CMLGraph_t ir, UOpType type, Tensor** inputs,
                                            int num_inputs, void* params, int* out_shape,
                                            int out_ndim);
...
static struct IRNode* create_primitive_node(CMLGraph_t ir, UOpType type, Tensor** inputs,
    ...
    if (num_inputs > 0 && inputs) {
        ...
        for (int i = 0; i < num_inputs; i++) {
            ...
            if (inputs[i] && inputs[i]->ir_node &&
                ...
                node->input_names[i] =
                    cml_strdup(((struct IRNode*)inputs[i]->ir_node)->output_name);
            } else {
    ...
    for (int i = 0; i < num_inputs && inputs; i++) {
        ...
        if (!in)
            continue;
        if (!node->build_stack && in->build_stack)
            node->build_stack = cml_strdup(in->build_stack);
        if (!node->scope && in->scope)
            node->scope = cml_strdup(in->scope);
    }
...
static void replace_node_with_chain(CMLGraph_t ir, struct IRNode* original,
                                    struct IRNode* chain_head, struct IRNode* chain_tail) {
    if (!ir || !original || !chain_head || !chain_tail)
        return;
    ...
    if (orig_output && chain_output) {
        ...
        if (chain_output->shape)
            cml_free(chain_output->shape);
        if (chain_output->strides)
            cml_free(chain_output->strides);
        cml_free(chain_output);

        // Point chain_tail at the original output tensor
        chain_tail->output = orig_output;

        if (chain_tail->output_name)
            cml_free(chain_tail->output_name);
        chain_tail->output_name =
            cml_strdup(original->output_name ? original->output_name : "_decomp");
    }
...
static int decompose_gelu(CMLGraph_t ir, struct IRNode* node) {
    ...
    struct IRNode* sig =
        chain_emit(ir, &head, &tail, UOP_SIGMOID, &sx->output, 1, NULL, shape, ndim);
    if (!sig)
        return -1;
...
static int decompose_hardswish(CMLGraph_t ir, struct IRNode* node) {
    ...
    struct IRNode* three = insert_fill_node(ir, s, nd, 3.0f);
    if (!three)
        return -1;
    chain_append(&h, &t, three);
    Tensor* a1[]       = {x, three->output};
    struct IRNode* xp3 = create_primitive_node(ir, UOP_ADD, a1, 2, NULL, s, nd);
    if (!xp3)
        return -1;
    chain_append(&h, &t, xp3);
    struct IRNode* zero = insert_fill_node(ir, s, nd, 0.0f);
    if (!zero)
        return -1;
    chain_append(&h, &t, zero);
    Tensor* m1[]     = {xp3->output, zero->output};
    struct IRNode* r = create_primitive_node(ir, UOP_MAX, m1, 2, NULL, s, nd);
    if (!r)
        return -1;
    chain_append(&h, &t, r);
    struct IRNode* six = insert_fill_node(ir, s, nd, 6.0f);
    if (!six)
        return -1;
    chain_append(&h, &t, six);
    Tensor* c1[]        = {r->output, six->output};
    struct IRNode* cond = create_primitive_node(ir, UOP_CMPLT, c1, 2, NULL, s, nd);
    if (!cond)
        return -1;
    chain_append(&h, &t, cond);
    Tensor* w1[]         = {cond->output, r->output, six->output};
    struct IRNode* clamp = create_primitive_node(ir, UOP_WHERE, w1, 3, NULL, s, nd);
    if (!clamp)
        return -1;
    chain_append(&h, &t, clamp);
    struct IRNode* sixth = insert_fill_node(ir, s, nd, 1.0f / 6.0f);
    if (!sixth)
        return -1;
    chain_append(&h, &t, sixth);
    Tensor* mm[]      = {x, clamp->output};
    struct IRNode* xc = create_primitive_node(ir, UOP_MUL, mm, 2, NULL, s, nd);
    if (!xc)
        return -1;
    chain_append(&h, &t, xc);
    Tensor* mm2[]      = {xc->output, sixth->output};
    struct IRNode* res = create_primitive_node(ir, UOP_MUL, mm2, 2, NULL, s, nd);
    if (!res)
        return -1;
    chain_append(&h, &t, res);
...
static int decompose_mish(CMLGraph_t ir, struct IRNode* node) {
    ...
    struct IRNode* ex = create_primitive_node(ir, UOP_EXP, &x, 1, NULL, s, nd);
    if (!ex)
        return -1;
    chain_append(&h, &t, ex);
    struct IRNode* one = insert_fill_node(ir, s, nd, 1.0f);
    if (!one)
        return -1;
    chain_append(&h, &t, one);
    Tensor* a1[]       = {one->output, ex->output};
    struct IRNode* sp1 = create_primitive_node(ir, UOP_ADD, a1, 2, NULL, s, nd);
    if (!sp1)
        return -1;
    chain_append(&h, &t, sp1);
    struct IRNode* sp = create_primitive_node(ir, UOP_LOG, &sp1->output, 1, NULL, s, nd);
    if (!sp)
        return -1;
    chain_append(&h, &t, sp);
    struct IRNode* two = insert_fill_node(ir, s, nd, 2.0f);
    if (!two)
        return -1;
    chain_append(&h, &t, two);
    Tensor* m1[]         = {two->output, sp->output};
    struct IRNode* twosp = create_primitive_node(ir, UOP_MUL, m1, 2, NULL, s, nd);
    if (!twosp)
        return -1;
    chain_append(&h, &t, twosp);
    struct IRNode* n = create_primitive_node(ir, UOP_NEG, &twosp->output, 1, NULL, s, nd);
    if (!n)
        return -1;
    chain_append(&h, &t, n);
    struct IRNode* en = create_primitive_node(ir, UOP_EXP, &n->output, 1, NULL, s, nd);
    if (!en)
        return -1;
    chain_append(&h, &t, en);
    Tensor* a2[]     = {one->output, en->output};
    struct IRNode* d = create_primitive_node(ir, UOP_ADD, a2, 2, NULL, s, nd);
    if (!d)
        return -1;
    chain_append(&h, &t, d);
    struct IRNode* rc = create_primitive_node(ir, UOP_RECIP, &d->output, 1, NULL, s, nd);
    if (!rc)
        return -1;
    chain_append(&h, &t, rc);
    Tensor* m2[]          = {two->output, rc->output};
    struct IRNode* twosig = create_primitive_node(ir, UOP_MUL, m2, 2, NULL, s, nd);
    if (!twosig)
        return -1;
    chain_append(&h, &t, twosig);
    Tensor* sub1[]    = {twosig->output, one->output};
    struct IRNode* th = create_primitive_node(ir, UOP_SUB, sub1, 2, NULL, s, nd);
    if (!th)
        return -1;
    chain_append(&h, &t, th);
    Tensor* mm[]       = {x, th->output};
    struct IRNode* res = create_primitive_node(ir, UOP_MUL, mm, 2, NULL, s, nd);
    if (!res)
        return -1;
    chain_append(&h, &t, res);
...
static int decompose_mean(CMLGraph_t ir, struct IRNode* node) {
    ...
    if (orig_params) {
        ...
        if (orig_params->dims && orig_params->num_dims > 0) {
            ...
            memcpy(sum_params->dims, orig_params->dims,
                   (size_t)orig_params->num_dims * sizeof(int));
        } else {
    ...
    // sum(x)
    struct IRNode* sum_node =
        create_primitive_node(ir, UOP_SUM, &x, 1, sum_params, out_shape, out_ndim);
    if (!sum_node) {
        cml_free(sum_params->dims);
        cml_free(sum_params);
        return -1;
    }
...
static int decompose_min_reduce(CMLGraph_t ir, struct IRNode* node) {
    ...
    if (orig_params) {
        ...
        if (orig_params->dims && orig_params->num_dims > 0) {
            ...
            memcpy(max_params->dims, orig_params->dims,
                   (size_t)orig_params->num_dims * sizeof(int));
        } else {
...
static struct IRNode* build_variance(CMLGraph_t ir, struct IRNode* node, struct IRNode** head,
    ...
    for (int i = 0; i < xnd; i++) {
        keep_shape[i] = x->shape[i];
        full_shape[i] = x->shape[i];
    }
    float n = 1.0f;
    ...
    struct IRNode* meanx =
        create_primitive_node(ir, UOP_EXPAND, &mean1->output, 1, ep, full_shape, xnd);
    if (!meanx)
        return NULL;
    ...
    if (!sp2)
        return NULL;
    struct IRNode* sum2 =
        create_primitive_node(ir, UOP_SUM, &sq->output, 1, sp2, out_shape, out_ndim);
    if (!sum2)
        return NULL;
...
static int decompose_elu_impl(CMLGraph_t ir, struct IRNode* node, float alpha, float scale) {
    ...
    struct IRNode* cond = create_primitive_node(ir, UOP_CMPLT, ci, 2, NULL, s, nd);
    if (!cond)
        return -1;
    chain_append(&h, &t, cond);
    struct IRNode* ex = create_primitive_node(ir, UOP_EXP, &x, 1, NULL, s, nd);
    ...
    struct IRNode* em1 = create_primitive_node(ir, UOP_SUB, si, 2, NULL, s, nd);
    if (!em1)
        return -1;
    chain_append(&h, &t, em1);
    struct IRNode* af = insert_fill_node(ir, s, nd, alpha);
    if (!af)
        return -1;
    chain_append(&h, &t, af);
    Tensor* mi[]       = {em1->output, af->output};
    struct IRNode* neg = create_primitive_node(ir, UOP_MUL, mi, 2, NULL, s, nd);
    if (!neg)
        return -1;
    chain_append(&h, &t, neg);
    Tensor* wi[]       = {cond->output, x, neg->output};
    struct IRNode* res = create_primitive_node(ir, UOP_WHERE, wi, 3, NULL, s, nd);
    if (!res)
        return -1;
    chain_append(&h, &t, res);

    if (scale != 1.0f) {
        struct IRNode* sc = insert_fill_node(ir, s, nd, scale);
        if (!sc)
            return -1;
        chain_append(&h, &t, sc);
        Tensor* smi[]       = {res->output, sc->output};
        struct IRNode* sres = create_primitive_node(ir, UOP_MUL, smi, 2, NULL, s, nd);
        if (!sres)
            return -1;
        chain_append(&h, &t, sres);
        res = sres;
    }
    (void)res;
    replace_node_with_chain(ir, node, h, t);
...
static int decompose_celu(CMLGraph_t ir, struct IRNode* node) {
    ...
    float alpha     = cp ? cp->min_val : 1.0f;
    if (alpha == 0.0f)
        return 0; // undefined; leave for executor
    struct IRNode *h = NULL, *t = NULL;
    ...
    if (!cond)
        return -1;
    chain_append(&h, &t, cond);
    struct IRNode* inva = insert_fill_node(ir, s, nd, 1.0f / alpha);
    if (!inva)
        return -1;
    chain_append(&h, &t, inva);
    Tensor* xi[]      = {x, inva->output};
    struct IRNode* xa = create_primitive_node(ir, UOP_MUL, xi, 2, NULL, s, nd);
    if (!xa)
        return -1;
    chain_append(&h, &t, xa);
    struct IRNode* ex = create_primitive_node(ir, UOP_EXP, &xa->output, 1, NULL, s, nd);
    if (!ex)
        return -1;
    chain_append(&h, &t, ex);
    struct IRNode* one = insert_fill_node(ir, s, nd, 1.0f);
...
static int decompose_softsign(CMLGraph_t ir, struct IRNode* node) {
    ...
    struct IRNode* negx = create_primitive_node(ir, UOP_NEG, &x, 1, NULL, s, nd);
    if (!negx)
        return -1;
    chain_append(&h, &t, negx);
    Tensor* mi[]        = {x, negx->output};
    struct IRNode* absx = create_primitive_node(ir, UOP_MAX, mi, 2, NULL, s, nd);
    if (!absx)
        return -1;
    chain_append(&h, &t, absx);
    struct IRNode* one = insert_fill_node(ir, s, nd, 1.0f);
    if (!one)
        return -1;
    chain_append(&h, &t, one);
    Tensor* ai[]         = {one->output, absx->output};
    struct IRNode* denom = create_primitive_node(ir, UOP_ADD, ai, 2, NULL, s, nd);
    if (!denom)
        return -1;
    chain_append(&h, &t, denom);
    Tensor* di[]       = {x, denom->output};
    struct IRNode* res = create_primitive_node(ir, UOP_DIV, di, 2, NULL, s, nd);
    if (!res)
        return -1;
    chain_append(&h, &t, res);
    (void)res;
...
static int decompose_neg(CMLGraph_t ir, struct IRNode* node) {
    ...
    int nd           = x->ndim;
    struct IRNode *h = NULL, *t = NULL;
    struct IRNode* m1 = insert_fill_node(ir, s, nd, -1.0f);
    if (!m1)
        return -1;
    chain_append(&h, &t, m1);
    Tensor* mi[]     = {x, m1->output};
    struct IRNode* r = create_primitive_node(ir, UOP_MUL, mi, 2, NULL, s, nd);
    if (!r)
        return -1;
    chain_append(&h, &t, r);
    replace_node_with_chain(ir, node, h, t);
...
static int decompose_sub(CMLGraph_t ir, struct IRNode* node) {
    ...
    if (!os)
        return -1;
    struct IRNode *h = NULL, *t = NULL;
    struct IRNode* m1 = insert_fill_node(ir, b->shape, b->ndim, -1.0f);
    if (!m1)
        return -1;
    chain_append(&h, &t, m1);
    Tensor* ni[]      = {b, m1->output};
    struct IRNode* nb = create_primitive_node(ir, UOP_MUL, ni, 2, NULL, b->shape, b->ndim);
    if (!nb)
        return -1;
    chain_append(&h, &t, nb);
    Tensor* ai[]     = {a, nb->output};
    struct IRNode* r = create_primitive_node(ir, UOP_ADD, ai, 2, NULL, os, on);
    if (!r)
        return -1;
    chain_append(&h, &t, r);
    replace_node_with_chain(ir, node, h, t);
...
static int decompose_div(CMLGraph_t ir, struct IRNode* node) {
    ...
    if (!os)
        return -1;
    struct IRNode *h = NULL, *t = NULL;
    struct IRNode* rb = create_primitive_node(ir, UOP_RECIP, &b, 1, NULL, b->shape, b->ndim);
    if (!rb)
        return -1;
    chain_append(&h, &t, rb);
    Tensor* mi[]     = {a, rb->output};
    struct IRNode* r = create_primitive_node(ir, UOP_MUL, mi, 2, NULL, os, on);
    if (!r)
        return -1;
    chain_append(&h, &t, r);
    replace_node_with_chain(ir, node, h, t);
...
static int decompose_pool2d(CMLGraph_t ir, struct IRNode* node, bool is_max) {
    ...
    if (dh != 1 || dw != 1 || p->ceil_mode)
        return 0;
    if (!is_max && (ph > 0 || pw > 0) && !p->count_include_pad)
        return 0;
...
static int decompose_conv3d(CMLGraph_t ir, struct IRNode* node) {
    ...
    if (p->dilation[0] != 1 || p->dilation[1] != 1 || p->dilation[2] != 1)
        return 0;
    if (x->shape[1] != Cin)
        return 0;
    ...
    if (!pmA)
        return -1;
    struct IRNode* uH =
        insert_unfold_last(ir, pmA->output, 6, kh, sh, &head, &tail); // [N,Cin,Dp,OW,kw,OH,kh]
    if (!uH)
        return -1;
    int pB[7] = {0, 1, 3, 4, 5, 6, 2};
    struct IRNode* pmB =
        insert_permute(ir, uH->output, 7, pB, &head, &tail); // [N,Cin,OW,kw,OH,kh,Dp]
    if (!pmB)
        return -1;
    struct IRNode* uD =
        insert_unfold_last(ir, pmB->output, 7, kd, sd, &head, &tail); // [N,Cin,OW,kw,OH,kh,OD,kd]
    if (!uD)
        return -1;
    int pC[8] = {0, 6, 4, 2, 1, 7, 5, 3};
    struct IRNode* pmC =
        insert_permute(ir, uD->output, 8, pC, &head, &tail); // [N,OD,OH,OW,Cin,kd,kh,kw]
    if (!pmC)
        return -1;
    int im_shape[2]   = {N * M, K};
...
static int decompose_conv_transpose2d(CMLGraph_t ir, struct IRNode* node) {
    ...
    if (p->dilation[0] != 1 || p->dilation[1] != 1)
        return 0;
    if (p->output_padding[0] != 0 || p->output_padding[1] != 0)
        return 0;
...
static int decompose_conv_transpose3d(CMLGraph_t ir, struct IRNode* node) {
    ...
    if (p->dilation[0] != 1 || p->dilation[1] != 1 || p->dilation[2] != 1)
        return 0;
    if (p->output_padding[0] || p->output_padding[1] || p->output_padding[2])
        return 0;
    if (x->shape[1] != w->shape[0])
        return 0;
    ...
    if (!fD)
        return -1; // [N,Cout,OWf,OHf,ODf]
    int pO[5] = {0, 1, 4, 3, 2};
    struct IRNode* pmO =
        insert_permute(ir, fD->output, 5, pO, &head, &tail); // [N,Cout,ODf,OHf,OWf]
    if (!pmO)
        return -1;
    struct IRNode* result = pmO;
...
static int decompose_scan(CMLGraph_t ir, struct IRNode* start) {
    ...
    for (int pass = 0; pass < MAX_PASSES; pass++) {
        ...
        while (node) {
            ...
            switch (node->type) {
            case UOP_NEG:
                result     = decompose_neg(ir, node);
                decomposed = true;
                break;
            case UOP_SUB:
                result     = decompose_sub(ir, node);
                decomposed = true;
                break;
            case UOP_DIV:
                result     = decompose_div(ir, node);
                decomposed = true;
                break;
            case UOP_FLATTEN:
                result     = decompose_flatten(ir, node);
                decomposed = true;
                break;
            case UOP_RELU:
                result     = decompose_relu(ir, node);
                decomposed = true;
                break;
            case UOP_RELU6:
                result     = decompose_relu6(ir, node);
                decomposed = true;
                break;
            case UOP_SILU:
                result     = decompose_silu(ir, node);
                decomposed = true;
                break;
            case UOP_GELU:
                result     = decompose_gelu(ir, node);
                decomposed = true;
                break;
            case UOP_HARDSWISH:
                result     = decompose_hardswish(ir, node);
                decomposed = true;
                break;
            case UOP_MISH:
                result     = decompose_mish(ir, node);
                decomposed = true;
                break;
            case UOP_SIGMOID:
            ...
            case UOP_MAXPOOL2D:
                result     = decompose_maxpool2d(ir, node);
                decomposed = true;
                break;
            case UOP_AVGPOOL2D:
                result     = decompose_avgpool2d(ir, node);
                decomposed = true;
                break;
            case UOP_CONV2D:
                result     = decompose_conv2d(ir, node);
                decomposed = true;
                break;
            case UOP_CONV3D:
                result     = decompose_conv3d(ir, node);
                decomposed = true;
                break;
            case UOP_CONV_TRANSPOSE2D:
                result     = decompose_conv_transpose2d(ir, node);
                decomposed = true;
                break;
            case UOP_CONV_TRANSPOSE3D:
                result     = decompose_conv_transpose3d(ir, node);
                decomposed = true;
                break;
            ...
            case UOP_MEAN:
                ...
                break;
            case UOP_VAR:
                result     = decompose_var(ir, node);
                decomposed = true;
                break;
            case UOP_STD:
                result     = decompose_std(ir, node);
                decomposed = true;
                break;
            case UOP_MIN_REDUCE:
            ...
            case UOP_CLAMP:
                ...
                break;
            case UOP_ELU:
                result     = decompose_elu(ir, node);
                decomposed = true;
                break;
            case UOP_SELU:
                result     = decompose_selu(ir, node);
                decomposed = true;
                break;
            case UOP_CELU:
                result     = decompose_celu(ir, node);
                decomposed = true;
                break;
            case UOP_SOFTSIGN:
                result     = decompose_softsign(ir, node);
                decomposed = true;
                break;
            case UOP_SOFTPLUS:
