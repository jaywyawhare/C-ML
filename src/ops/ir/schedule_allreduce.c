...
ScheduleAllReduce* schedule_allreduce_build(Tensor* t, AllReduceOp op, AllReduceAlgo algo,
    ...
    switch (algo) {
    case AR_ALGO_RING:
        rc = build_ring_steps(ar);
        break;
    case AR_ALGO_FLAT:
        rc = build_flat_steps(ar);
        break;
    case AR_ALGO_RECURSIVE_HALVING:
        rc = build_recursive_halving_steps(ar);
        break;
    case AR_ALGO_TREE:
        rc = build_tree_steps(ar);
        break;
    default:
        rc = build_ring_steps(ar);
        break;
...
int schedule_allreduce_run(ScheduleAllReduce* ar) {
    ...
    if (ar->input->dtype != DTYPE_FLOAT32)
        return 0; /* only f32 modeled */
...
int schedule_allreduce_inject(CMLSchedule* sched, ScheduleAllReduce* ar) {
    ...
    CMLScheduleItem** it =
        (CMLScheduleItem**)cml_realloc(sched->items, (size_t)new_n * sizeof(*it));
    if (!it)
        return -1;
    sched->items = it;
    ...
    for (int s = 0; s < add; ++s) {
        ...
        if (!item)
            return -1; /* prior appends stay consistent; caller frees sched */
        ...
        if (sched->dependencies)
            sched->dependencies[idx] = NULL;
        if (sched->dep_counts)
            sched->dep_counts[idx] = 0;
        sched->num_items++;
    }
    if (sched->item_capacity < new_n)
        sched->item_capacity = new_n;
...
void schedule_allreduce_print(const ScheduleAllReduce* ar) {
    ...
    fprintf(stderr,
            "ScheduleAllReduce: op=%s algo=%s devices=%d steps=%d "
            "chunks=%d buf=%zu bytes\n",
            op_names[ar->op], algo_names[ar->algo], ar->num_devices, ar->num_steps, ar->chunk_count,
            ar->buffer_bytes);
}
