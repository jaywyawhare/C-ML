...
static CMLOptList* opt_list_clone(const CMLOptList* src) {
    ...
    for (int i = 0; i < src->num_opts; i++)
        cml_opt_list_add(dst, src->opts[i].type, src->opts[i].axis, src->opts[i].amount);
    return dst;
}
...
static int apply_local(int axis, int amount, LinearProgram* prog) {
    ...
    if (prog->local_mem_used + local_bytes > OPT_MAX_LOCAL_MEM) {
        LOG_WARNING("OPT_LOCAL: exceeds local memory limit (%zu + %zu > %d)", prog->local_mem_used,
                    local_bytes, OPT_MAX_LOCAL_MEM);
        return -1;
    }
    ...
    LOG_DEBUG("OPT_LOCAL: axis %d split %d -> %d * %d, local_mem=%zu", axis, extent, outer_extent,
              amount, prog->local_mem_used);
    return 0;
}
...
static int apply_group(int axis, int amount, LinearProgram* prog) {
    ...
    if (local_extent > OPT_MAX_WORKGROUP) {
        LOG_WARNING("OPT_GROUP: local extent %d exceeds max workgroup size %d", local_extent,
                    OPT_MAX_WORKGROUP);
        return -1;
    }
    ...
    LOG_DEBUG("OPT_GROUP: axis %d split %d -> group(%d) * local(%d), dim=%d", axis, extent,
              group_extent, local_extent, dim);
    return 0;
}
...
static int apply_unroll(int axis, int amount, LinearProgram* prog) {
    ...
    LOG_DEBUG("OPT_UNROLL: axis %d factor %d, extent %d -> %d", axis, amount, extent, new_extent);
    return 0;
}
...
static int apply_upcast(int axis, int amount, LinearProgram* prog) {
    ...
    for (int i = 0; i < prog->num_ops; i++) {
        LinearOp* op = &prog->ops[i];
        if (op->kind == LINOP_LOAD || op->kind == LINOP_STORE || op->kind == LINOP_COMPUTE) {
            if (op->vec_width <= 1)
                op->vec_width = amount;
        }
    }

    LOG_DEBUG("OPT_UPCAST: axis %d vec_width=%d, extent %d -> %d", axis, amount, extent,
              extent / amount);
    return 0;
}
...
static int apply_padto(int axis, int amount, LinearProgram* prog) {
    ...
    LOG_DEBUG("OPT_PADTO: axis %d padded %d -> %d (multiple of %d)", axis, extent, padded, amount);
    return 0;
}
...
static int apply_nolocals(LinearProgram* prog) {
    ...
    for (int i = 0; i < prog->num_ops; i++) {
        ...
        if (k == LINOP_LOCAL_ALLOC || k == LINOP_LOCAL_LOAD || k == LINOP_LOCAL_STORE ||
            k == LINOP_BARRIER) {
            continue;
        }
        if (dst != i)
            prog->ops[dst] = prog->ops[i];
        dst++;
    }
    prog->num_ops          = dst;
...
int cml_opt_apply(CMLOptList* opts, struct LinearProgram* prog) {
    ...
    for (int i = 0; i < opts->num_opts; i++) {
        ...
        switch (o->type) {
        case OPT_LOCAL:
            rc = apply_local(o->axis, o->amount, prog);
            break;
        case OPT_GROUP:
            rc = apply_group(o->axis, o->amount, prog);
            break;
        case OPT_UNROLL:
            rc = apply_unroll(o->axis, o->amount, prog);
            break;
        case OPT_UPCAST:
            rc = apply_upcast(o->axis, o->amount, prog);
            break;
        case OPT_PADTO:
            rc = apply_padto(o->axis, o->amount, prog);
            break;
        case OPT_NOLOCALS:
            rc = apply_nolocals(prog);
            break;
        }
        if (rc != 0) {
            LOG_WARNING("Opt %s(axis=%d, amount=%d) failed at step %d", cml_opt_type_name(o->type),
                        o->axis, o->amount, i);
            return rc;
        }
    }
...
int cml_opt_enumerate(struct LinearProgram* prog, CMLOptList*** out_lists, int* out_count,
    ...
    for (int ax = 0; ax < prog->num_axes && count < cap; ax++) {
        ...
        for (int ti = 0; ti < NUM_TILE_FACTORS && count < cap; ti++) {
            if (extent % TILE_FACTORS[ti] != 0)
                continue;
            if (TILE_FACTORS[ti] > OPT_MAX_WORKGROUP)
                continue;
            CMLOptList* l = cml_opt_list_create();
        ...
        for (int ti = 0; ti < NUM_TILE_FACTORS && count < cap; ti++) {
            ...
            if (extent % TILE_FACTORS[ti] != 0)
                continue;
            if (local_bytes > OPT_MAX_LOCAL_MEM)
                continue;
            CMLOptList* l = cml_opt_list_create();
    ...
    for (int a = 1; a < single_end && count < cap; a++) {
        for (int b = a + 1; b < single_end && count < cap; b++) {
            ...
            if (oa->axis == ob->axis)
                continue; /* same axis conflicts */

            bool good_pair = (oa->type == OPT_UPCAST && ob->type == OPT_UNROLL) ||
    ...
    *out_count = count;
    ...
    LOG_INFO("OPT enumerate: generated %d configurations for %d axes", count, prog->num_axes);
    return 0;
}
