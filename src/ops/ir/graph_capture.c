...
int cml_graph_capture_begin(CMLCapturedGraph* graph) {
    if (!graph)
        return -1;
    if (graph->state == CML_CAPTURE_RECORDING)
        return -1;
...
int cml_graph_capture_record(CMLCapturedGraph* graph, UOpType op, void* kernel_handle,
    ...
    if (graph->num_nodes >= graph->node_capacity) {
        ...
        CMLCapturedNode* new_nodes =
            (CMLCapturedNode*)cml_realloc(graph->nodes, (size_t)new_cap * sizeof(CMLCapturedNode));
        if (!new_nodes) {
            graph->state = CML_CAPTURE_ERROR;
            return -1;
        }
...
int cml_graph_capture_bind_input(CMLCapturedGraph* graph, int index, Tensor* tensor) {
    ...
    if (index >= graph->num_input_bindings) {
        ...
        Tensor** new_bindings =
            (Tensor**)cml_realloc(graph->input_bindings, (size_t)new_count * sizeof(Tensor*));
        if (!new_bindings)
            return -1;
...
int cml_graph_capture_bind_output(CMLCapturedGraph* graph, int index, Tensor* tensor) {
    ...
    if (index >= graph->num_output_bindings) {
        int new_count = index + 1;
        Tensor** new_bindings =
            (Tensor**)cml_realloc(graph->output_bindings, (size_t)new_count * sizeof(Tensor*));
        if (!new_bindings)
            return -1;
...
void cml_graph_capture_stats(const CMLCapturedGraph* graph, int* replay_count,
    ...
    if (avg_replay_ms) {
        *avg_replay_ms =
            graph->replay_count > 0 ? graph->total_replay_time_ms / graph->replay_count : 0.0;
    }
}
...
void cml_graph_capture_print(const CMLCapturedGraph* graph) {
    ...
    switch (graph->state) {
    case CML_CAPTURE_IDLE:
        state_str = "IDLE";
        break;
    case CML_CAPTURE_RECORDING:
        state_str = "RECORDING";
        break;
    case CML_CAPTURE_READY:
        state_str = "READY";
        break;
    case CML_CAPTURE_ERROR:
        state_str = "ERROR";
        break;
    default:
        state_str = "UNKNOWN";
