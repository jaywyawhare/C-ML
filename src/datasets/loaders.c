...
static int is_image_file(const char* name) {
    ...
    return (strcasecmp(ext, "ppm") == 0 || strcasecmp(ext, "pgm") == 0 ||
            strcasecmp(ext, "raw") == 0 || strcasecmp(ext, "bmp") == 0 ||
...
CMLImageNetLoader* cml_imagenet_open(const char* dir_path, int image_size) {
    ...
    LOG_INFO("[loaders] ImageNet: %d samples, %d classes from %s", loader->num_samples, num_classes,
             dir_path);
...
static void collect_audio_recursive(const char* dir, char*** paths, char*** transcripts, int* count,
    ...
    while ((ent = readdir(d)) != NULL) {
        ...
        if (*count >= *cap) {
            *cap *= 2;
            ...
            if (tmp_t)
                *transcripts = tmp_t;
            if (!tmp_p || !tmp_t)
                return; /* *paths and *transcripts freed by caller */
        }
        ...
        if (ext) {
            ...
            if (off + 5 <= sizeof(txt_path))
                memcpy(ext, ".txt", 5); /* incl NUL, bounded */
        }
...
static int is_text_file(const char* name) {
    ...
    if (!ext)
        return 0;
    ext++;
    return (strcasecmp(ext, "txt") == 0 || strcasecmp(ext, "text") == 0 ||
...
CMLWikipediaLoader* cml_wikipedia_open(const char* dump_dir) {
    ...
    LOG_INFO("[loaders] Wikipedia: %d articles, %.1f MB from %s", count, total / (1024.0 * 1024.0),
             dump_dir);
    return loader;
}
