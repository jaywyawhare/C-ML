...
static void write_meta(CMLRun* run, const char* status) {
    ...
    fputs("{\n", f);
    fputs("  \"id\": ", f);
    json_escape(f, run->id);
    fputs(",\n", f);
    fputs("  \"project\": ", f);
    json_escape(f, run->project);
    fputs(",\n", f);
    fputs("  \"name\": ", f);
    json_escape(f, run->name);
    fputs(",\n", f);
    fputs("  \"sweep\": ", f);
    json_escape(f, run->sweep[0] ? run->sweep : NULL);
    fputs(",\n", f);
    fputs("  \"status\": ", f);
    json_escape(f, status);
    fputs(",\n", f);
    fputs("  \"host\": ", f);
    json_escape(f, run->host);
    fputs(",\n", f);
    fputs("  \"os\": ", f);
    json_escape(f, run->os);
    fputs(",\n", f);
    fputs("  \"git\": ", f);
    json_escape(f, run->git[0] ? run->git : NULL);
    fputs(",\n", f);
    fputs("  \"cmd\": ", f);
    json_escape(f, run->cmd[0] ? run->cmd : NULL);
    fputs(",\n", f);
    fputs("  \"notes\": ", f);
    json_escape(f, run->notes);
    fputs(",\n", f);
    fprintf(f, "  \"tags\": [%s],\n", run->tags ? run->tags : "");
    fprintf(f, "  \"start\": %.3f,\n", run->start_time);
    fprintf(f, "  \"end\": %.3f,\n", done ? end : 0.0);
...
CMLRun* cml_exp_run_init(const char* project, const char* name, const char* config_json) {
...
#ifdef _WIN32
    {
        ...
        if (!GetComputerNameA(run->host, &_sz))
            run->host[0] = '\0';
    }
#else
    if (gethostname(run->host, sizeof(run->host)) != 0)
        run->host[0] = '\0';
#endif
#ifdef _WIN32
    ...
    struct utsname un;
    if (uname(&un) == 0)
        snprintf(run->os, sizeof(run->os), "%s %s", un.sysname, un.release);
#endif
...
void cml_exp_log_histogram(CMLRun* run, const char* name, long step, const float* values, size_t n,
    ...
    for (int b = 0; b < bins; b++)
        fprintf(run->events, "%s%ld", b ? "," : "", counts[b]);
...
void cml_exp_log_system(CMLRun* run, long step) {
    ...
    if (stat) {
        ...
        if (dw > 1e-6)
            cpu_pct = (cpu - run->last_cpu) / dw * 100.0;
        run->last_wall = wall;
        run->last_cpu  = cpu;
    }
    pthread_mutex_lock(&run->lock);
...
void cml_exp_log_curve(CMLRun* run, const char* name, const char* xlabel, const char* ylabel,
    ...
    for (size_t i = 0; i < n; i++)
        fprintf(run->events, "%s%.5g", i ? "," : "", xs[i]);
    fprintf(run->events, "],\"ys\":[");
    for (size_t i = 0; i < n; i++)
        fprintf(run->events, "%s%.5g", i ? "," : "", ys[i]);
    fprintf(run->events, "],\"wall\":%.3f}\n", now_sec());
    fflush(run->events);
    pthread_mutex_unlock(&run->lock);
}
