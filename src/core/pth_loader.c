            ...
            if (pos + len <= size && len > 0) {
                ...
                if (str) {
                    ...
                    if (strstr(str, ".weight") || strstr(str, ".bias") ||
                        ...
                        strstr(str, ".num_batches_tracked") || strstr(str, "embedding")) {
                        if (result->num_keys >= result->key_capacity) {
                            result->key_capacity *= 2;
                            char** new_keys = (char**)cml_realloc(
                                result->keys, (size_t)result->key_capacity * sizeof(char*));
                            if (new_keys)
    ...
    while (pos + sizeof(ZipLocalHeader) < (size_t)file_size) {
        ...
        if (data_part && compression == 0 && uncomp_size > 0) {
            ...
            if (idx >= 0 && idx < 256) {
                ...
                if (idx >= data_file_count)
                    data_file_count = idx + 1;
            }
        }
...
int cml_pth_load_into_module(const CMLPthStateDict* sd, struct Module* module) {
    if (!sd || !module)
        return -1;
    if (!module->parameters || module->num_parameters == 0)
        return -1;
    ...
    for (int i = 0; i < module->num_parameters; i++) {
        ...
        if (!param->tensor || param->tensor->numel != t->numel)
            continue;
