...
static void tensor_map_set(TensorMap* map, const char* name, Tensor* t) {
    ...
    for (int probe = 0; probe < TENSOR_MAP_SIZE; probe++) {
        uint32_t slot = (idx + (uint32_t)probe) % TENSOR_MAP_SIZE;
        if (!map->entries[slot].occupied || strcmp(map->entries[slot].name, name) == 0) {
...
static Tensor* inp(const CMLONNXNode* node, TensorMap* map, int idx) {
    if (idx < 0 || idx >= node->num_inputs)
        return NULL;
    if (!node->inputs[idx] || node->inputs[idx][0] == '\0')
        return NULL;
...
static Tensor* op_cast(const CMLONNXNode* n, TensorMap* m) {
    ...
    case 7:
        ...
        break;
    case 10:
        target = DTYPE_FLOAT16;
        break;
    case 11:
        target = DTYPE_FLOAT64;
        break;
    default:
        target = DTYPE_FLOAT32;
        break;
...
static Tensor* op_constant(const CMLONNXNode* n, TensorMap* m) {
    ...
    if (vis && vis->type == CML_ONNX_ATTR_INTS && vis->value.ints.count > 0) {
        ...
        for (int i = 0; i < count; i++)
            fdata[i] = (float)vis->value.ints.data[i];
        int shape[1] = {count};
...
static Tensor* op_scatter_nd(const CMLONNXNode* n, TensorMap* m) {
    ...
    for (int i = 0; i < k - 1; i++)
        outer *= (size_t)idx->shape[i];

    size_t slice_elems = 1; /* elements per indexed slice */
    for (int i = q; i < data->ndim; i++)
        slice_elems *= (size_t)data->shape[i];
    ...
    for (size_t e = 0; e < outer; e++) {
        ...
        memcpy(out + base, ud + e * slice_elems, sizeof(float) * slice_elems);
    }
...
static void resize_sample_coord(int in_size, int out_size, bool linear, ResizeCoordMode coord,
    ...
    case RESIZE_COORD_ALIGN_CORNERS:
        ...
        u = (out_size > 1) ? (float)o * (float)(in_size - 1) / (float)(out_size - 1) : 0.0f;
        break;
    ...
    if (!linear) {
        int v;
        if (strcmp(nearest_mode, "floor") == 0)
            v = (int)floorf(u);
...
static Tensor* op_resize(const CMLONNXNode* n, TensorMap* m) {
    ...
    if (strcmp(coord_mode, "half_pixel") == 0)
        coord = RESIZE_COORD_HALF_PIXEL;
    else if (strcmp(coord_mode, "asymmetric") == 0)
        coord = RESIZE_COORD_ASYMMETRIC;
    else if (strcmp(coord_mode, "align_corners") == 0)
        coord = RESIZE_COORD_ALIGN_CORNERS;
    else if (strcmp(coord_mode, "pytorch_half_pixel") == 0)
        coord = RESIZE_COORD_PYTORCH_HALF_PIXEL;
    else {
        LOG_ERROR("onnx_ops: Resize coordinate mode '%s' not supported by importer", coord_mode);
        return NULL;
    }
    ...
    for (size_t o = 0; o < out_numel; o++) {
        ...
        for (int d = 0; d < x->ndim; d++) {
            resize_sample_coord(x->shape[d], out_shape[d], linear, coord, nearest_mode, cur[d],
                                &i0[d], &i1[d], &w0[d], &w1[d]);
            if (i1[d] != i0[d] && w1[d] > 0.0f)
                two[ntwo++] = d;
        }
...
static Tensor* run_reduction(const CMLONNXNode* n, TensorMap* m, reduce_fn fn) {
    ...
    if (count == 0) {
        if (attr_int(n, "noop_with_empty_axes", 0) != 0)
            return x;
        for (int i = 0; i < x->ndim; i++)
            axes[i] = i;
        count = x->ndim;
...
int cml_onnx_run(CMLONNXModel* model, Tensor** inputs, int num_inputs, Tensor** outputs,
    ...
    for (int i = 0; i < g->num_initializers; i++) {
        if (g->initializers[i].name[0] && g->initializers[i].tensor) {
            tensor_map_set(map, g->initializers[i].name, g->initializers[i].tensor);
        }
    }
    ...
    for (int i = 0; i < g->num_inputs && input_idx < num_inputs; i++) {
        ...
        if (tensor_map_get(map, iname) != NULL)
            continue;
    ...
    for (int i = 0; i < g->num_nodes; i++) {
        ...
        if (!handler) {
            LOG_ERROR("onnx_ops: unsupported op '%s' (node '%s')", node->op_type, node->name);
            cml_free(map);
            return -2;
        }
        ...
        if (!result) {
            LOG_ERROR("onnx_ops: op '%s' (node '%s') returned NULL", node->op_type, node->name);
            cml_free(map);
            return -3;
        }
