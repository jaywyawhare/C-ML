...
static void pb_init(PBBuf* b) { b->data = NULL; b->len = 0; b->cap = 0; }
...
static bool pb_field_packed_int64(PBBuf* b, int field, const int64_t* v, int n) {
    ...
    if (ok) ok = pb_field_buf(b, field, &payload);
    pb_free(&payload);
    return ok;
}
...
static int onnx_elem_type(DType dt) {
    switch (dt) {
    case DTYPE_FLOAT32: return ONNX_FLOAT;
    case DTYPE_FLOAT64: return ONNX_DOUBLE;
    case DTYPE_INT8:    return ONNX_INT8;
    case DTYPE_UINT8:   return ONNX_UINT8;
    case DTYPE_INT16:   return ONNX_INT16;
    case DTYPE_UINT16:  return ONNX_UINT16;
    case DTYPE_INT32:   return ONNX_INT32;
    case DTYPE_INT64:   return ONNX_INT64;
    case DTYPE_BOOL:    return ONNX_BOOL;
    case DTYPE_FLOAT16: return ONNX_FLOAT16;
    case DTYPE_BFLOAT16:return ONNX_BFLOAT16;
    default:            return 0;
    }
}
...
static const char* intern_name(ExportCtx* c, Tensor* t, const char* name) {
    for (int i = 0; i < c->num_names; i++)
        if (c->names[i].t == t) return c->names[i].name;
    if (c->num_names == c->cap_names) {
        ...
        if (!nn) { ctx_error(c, "out of memory"); return NULL; }
        c->names     = nn;
        c->cap_names = nc;
    }
...
static bool is_user_input(ExportCtx* c, Tensor* t) {
    for (int i = 0; i < c->num_user_inputs; i++)
        if (c->user_inputs[i] == t) return true;
    return false;
}
...
static const char* leaf_name(ExportCtx* c, Tensor* t) {
    ...
    if (!nm) { ctx_error(c, "out of memory"); return NULL; }
    if (!emit_initializer(c, t, nm)) return NULL;
    return nm;
}
...
static bool emit_value_info(PBBuf* dst, int field, Tensor* t, const char* name) {
    ...
    if (!et) return false;
    PBBuf shape, tt, vi;
    pb_init(&shape); pb_init(&tt); pb_init(&vi);
    bool ok = true;
    ...
    pb_free(&shape); pb_free(&tt); pb_free(&vi);
    return ok;
}
...
static bool attr_push(AttrList* l, PBBuf* msg) {
    if (l->n >= ATTR_MAX) { pb_free(msg); return false; }
    l->msgs[l->n++] = *msg;
    pb_init(msg);
    return true;
}
...
static bool attr_int(AttrList* l, const char* name, int64_t v) {
    PBBuf a; pb_init(&a);
    bool ok = pb_field_str(&a, 1, name) &&
              pb_field_varint(&a, 20, OATTR_INT) &&
...
static bool attr_float_attr(AttrList* l, const char* name, float v) {
    PBBuf a; pb_init(&a);
    bool ok = pb_field_str(&a, 1, name) &&
              pb_field_varint(&a, 20, OATTR_FLOAT) &&
...
static bool attr_ints(AttrList* l, const char* name, const int64_t* v, int count) {
    PBBuf a; pb_init(&a);
    bool ok = pb_field_str(&a, 1, name) &&
              pb_field_varint(&a, 20, OATTR_INTS) &&
              pb_field_packed_int64(&a, 8, v, count);
    return ok && attr_push(l, &a);
}

static bool emit_node_full(ExportCtx* c, const char* op_type,
    ...
    pb_free(&nd);
    if (attrs)
        for (int i = 0; i < attrs->n; i++) pb_free(&attrs->msgs[i]);
    if (ok) c->num_nodes_emitted++;
    return ok;
}
...
static int norm_axis(int axis, int rank) {
    if (axis < 0) axis += rank;
    if (axis < 0) return 0;
    if (rank > 0 && axis >= rank) return rank - 1;
    return axis;
}
...
static const char* map_uop(struct IRNode* n) {
    ...
    case UOP_DIV:     return "Div";
    case UOP_MAX:     return "Max";
    case UOP_MINIMUM: return "Min";
    case UOP_POW:     return "Pow";

    /* comparisons */
    case UOP_CMPLT: return "Less";
    case UOP_CMPGT: return "Greater";
    case UOP_CMPEQ: return "Equal";
    case UOP_CMPLE: return "LessOrEqual";
    case UOP_CMPGE: return "GreaterOrEqual";

    /* elementwise unary */
    case UOP_NEG:     return "Neg";
    case UOP_EXP:     return "Exp";
    case UOP_LOG:     return "Log";
    case UOP_SQRT:    return "Sqrt";
    case UOP_ABS:     return "Abs";
    case UOP_SIN:     return "Sin";
    case UOP_COS:     return "Cos";
    case UOP_TANH:    return "Tanh";
    case UOP_SIGMOID: return "Sigmoid";
    case UOP_SIGN:    return "Sign";
    case UOP_FLOOR:   return "Floor";
    case UOP_CEIL:    return "Ceil";
    case UOP_ROUND:   return "Round";
    case UOP_ERF:     return "Erf";

    /* activations */
    case UOP_RELU:         return "Relu";
    case UOP_SOFTPLUS:     return "Softplus";
    case UOP_HARD_SIGMOID: return "HardSigmoid";
    case UOP_HARD_TANH:    return "HardTanh";

    /* matmul / movement / misc */
    case UOP_MATMUL:  return "MatMul";
    case UOP_RESHAPE: return "Reshape";
    case UOP_PERMUTE: return "Transpose";
    case UOP_EXPAND:  return "Expand";
    case UOP_SLICE:   return "Slice";
    case UOP_FLATTEN: return "Flatten";
    case UOP_CAT:     return "Concat";
    case UOP_GATHER:  return "Gather";
    case UOP_WHERE:   return "Where";
    case UOP_CLAMP:   return "Clip";

    /* conv/pool */
    case UOP_CONV2D:           return "Conv";
    case UOP_MAXPOOL2D:        return "MaxPool";
    case UOP_AVGPOOL2D:        return "AveragePool";
    case UOP_CONV_TRANSPOSE2D: return "ConvTranspose";

    /* reductions */
    case UOP_SUM:        return "ReduceSum";
    case UOP_MEAN:       return "ReduceMean";
    case UOP_MAX_REDUCE: return "ReduceMax";
    case UOP_MIN_REDUCE: return "ReduceMin";
    case UOP_PROD:       return "ReduceProd";
...
static bool build_attrs(ExportCtx* c, struct IRNode* n, AttrList* attrs) {
    ...
    case UOP_PERMUTE: {
        ...
        if (!p || !p->perm) { ctx_error(c, "Permute missing params"); return false; }
        int64_t perm[8];
        for (int i = 0; i < p->num_dims && i < 8; i++) perm[i] = p->perm[i];
    ...
    case UOP_CAT: {
        ...
        return attr_int(attrs, "axis",
                        norm_axis(p ? p->dim : 0, n->output_ndim));
    }
    case UOP_GATHER: {
        GatherParams* p = (GatherParams*)n->params;
        int rank = n->inputs[0] ? n->inputs[0]->ndim : 0;
        return attr_int(attrs, "axis",
                        norm_axis(p ? p->dim : 0, rank));
    }
    ...
    case UOP_CONV2D: {
        ...
        if (!p) { ctx_error(c, "Conv2D missing params"); return false; }
        return attr_ints(attrs, "kernel_shape",
                         (int64_t[]){p->kernel_size[0], p->kernel_size[1]}, 2) &&
               attr_ints(attrs, "strides",
                         (int64_t[]){p->stride[0], p->stride[1]}, 2) &&
               attr_ints(attrs, "dilations",
                         (int64_t[]){p->dilation[0], p->dilation[1]}, 2) &&
               attr_ints(attrs, "pads",
                         (int64_t[]){p->padding[0], p->padding[1],
                                     p->padding[0], p->padding[1]}, 4) &&
               attr_int(attrs, "group", p->groups);
    }
    case UOP_MAXPOOL2D: case UOP_AVGPOOL2D: {
        Pool2DParams* p = (Pool2DParams*)n->params;
        if (!p) { ctx_error(c, "pool op missing params"); return false; }
        bool ok = attr_ints(attrs, "kernel_shape",
                            (int64_t[]){p->kernel_size[0], p->kernel_size[1]}, 2) &&
                  attr_ints(attrs, "strides",
                            (int64_t[]){p->stride[0], p->stride[1]}, 2) &&
                  attr_ints(attrs, "pads",
        ...
        if (n->type == UOP_MAXPOOL2D)
            ...
            ok = attr_int(attrs, "count_include_pad",
                          p->count_include_pad ? 1 : 0) && ok;
        return ok;
    }
    default:
        return true;
...
static const char* ensure_one_scalar(ExportCtx* c) {
    ...
    if (existing) return "cml_one_f32";
    ...
    if (!emit_initializer(c, one, "cml_one_f32")) return NULL;
    return intern_name(c, one, "cml_one_f32");
}

/* Int64 shape-vector initializer (Reshape/Expand second operand). */
static const char* emit_shape_initializer(ExportCtx* c, const int* shape,
                                          int ndim, const char* name) {
    int64_t sv[8];
    for (int i = 0; i < ndim && i < 8; i++) sv[i] = shape[i];
    PBBuf tp;
    pb_init(&tp);
    bool ok = pb_field_packed_int64(&tp, 1, sv, ndim) &&
              pb_field_varint(&tp, 2, ONNX_INT64) &&
              pb_field_str(&tp, 8, name) &&
              pb_field_len(&tp, 9, sv, (size_t)ndim * sizeof(int64_t)) &&
              pb_field_buf(&c->initializers, 5, &tp);
    pb_free(&tp);
    if (!ok) { ctx_error(c, "failed to serialize shape initializer"); return NULL; }
    c->num_initializers++;
    return name;
}
 ...
 */
static int export_special(ExportCtx* c, struct IRNode* n,
                          const char* const* in_names, int nin,
                          const char* out_name) {
    switch (n->type) {
    case UOP_LINEAR: {
        /* Gemm(A, B, C?) with transB=1 */
        AttrList attrs = {0};
        bool ok = attr_int(&attrs, "alpha", 1) &&
                  attr_int(&attrs, "beta", 1) &&
                  attr_int(&attrs, "transA", 0) &&
                  attr_int(&attrs, "transB", 1);
    ...
    case UOP_RESHAPE: case UOP_EXPAND: {
        ...
        if (!p || !p->new_shape) {
            ctx_error(c, "%s missing shape params",
                      n->type == UOP_RESHAPE ? "Reshape" : "Expand");
            return -1;
        }
        char buf[80];
        snprintf(buf, sizeof(buf), "%s_shape", out_name);
        const char* shp = emit_shape_initializer(c, p->new_shape,
                                                 p->new_ndim, buf);
        if (!shp) return -1;
        const char* ins[2] = { in_names[0], shp };
        return emit_node_full(c, n->type == UOP_RESHAPE ? "Reshape" : "Expand",
                              ins, 2, out_name, NULL) ? 0 : -1;
    }
    default:
...
int cml_onnx_export_graph(struct CMLGraph* ir_handle,
    ...
    for (; n; n = n->next, idx++) {
        ...
        if (n->num_inputs == 0) {
            if (out->data || (tensor_data_ptr(out) != NULL)) {
                ...
                if (!emit_initializer(&c, out, nm)) goto done;
                if (!intern_name(&c, out, nm)) goto done;
                continue;
            }
        ...
        if (!out_name) {
            char gen[48];
            if (out->ir_node && out->ir_node->output_name &&
                out->ir_node->output_name[0])
                snprintf(gen, sizeof(gen), "%s", out->ir_node->output_name);
            else
                snprintf(gen, sizeof(gen), "n_%d", idx);
            out_name = intern_name(&c, out, gen);
        }
        if (!out_name) goto done;

        const char* op_type = map_uop(n);
        if (!op_type && n->type != UOP_LINEAR && n->type != UOP_RECIP &&
            n->type != UOP_SQUARE) {
            ctx_error(&c, "UOp %s has no ONNX mapping",
                      uop_type_to_string(n->type));
            goto done;
        }
        if (!op_type) op_type = "Add"; /* placeholder; special-cased below */
        ...
        int nin = n->num_inputs < CML_ONNX_MAX_INPUTS ? n->num_inputs
                                                      : CML_ONNX_MAX_INPUTS;
        for (int i = 0; i < nin; i++) {
            Tensor* t = n->inputs[i];
            if (!t) {
            ...
            if (t->ir_node) {
                ...
                if (!nm) { LOG_ERROR("onnx_export: %s", c.err); goto done; }
                in_names[i] = nm;
            }
        }
        ...
        if (!ok) { ctx_error(&c, "node serialization failed"); goto done; }
    }

    /* Graph input value infos */
    for (int i = 0; i < num_inputs; i++) {
        Tensor* t = graph_inputs[i];
        if (!t) { ctx_error(&c, "NULL graph input %d", i); goto done; }
        char nm[32];
        ...
        if (!name) {
            if (!intern_name(&c, t, nm)) goto done;
            name = nm;
        }
    ...
    for (int i = 0; i < num_outputs; i++) {
        ...
        if (!t) { ctx_error(&c, "NULL graph output %d", i); goto done; }
        ...
        if (!nm) {
            ...
            if (!emit_node_full(&c, "Identity", &ln, 1, on, NULL)) goto done;
            nm = on;
        }
    ...
    /* Assemble ModelProto */
    {
        ...
        bool ok = pb_put(&graph, c.nodes.data, c.nodes.len) &&          /* node */
                  ...
                  pb_put(&graph, c.initializers.data,
                         c.initializers.len) &&                          /* initializer */
                  pb_put(&graph, c.vi_inputs.data, c.vi_inputs.len) &&   /* input */
                  pb_put(&graph, c.vi_outputs.data, c.vi_outputs.len);   /* output */

        /* OperatorSetIdProto{domain=1 "", version=2} */
        ok = ok && pb_field_str(&opset, 1, "") &&
             pb_field_varint(&opset, 2, 11);
            ...
            if (!f) {
                ...
                if (w == model.len) {
                    LOG_INFO("onnx_export: wrote %s (%zu bytes, %d nodes, "
                             "%d initializers)", filepath, model.len,
                             c.num_nodes_emitted, c.num_initializers);
                    rc = 0;
                } else {
    ...
    free(c.names);
    if (rc != 0)
        LOG_ERROR("onnx_export: export failed: %s",
