...
Tensor* cml_quantize_int8(Tensor* tensor, const QuantParams* params, QuantParams* out_params) {
    ...
    TensorConfig config = {
        .dtype = DTYPE_INT8, .device = tensor->device, .has_dtype = true, .has_device = true};
...
Tensor* cml_dequantize_int8(Tensor* tensor, const QuantParams* params) {
    ...
    TensorConfig config = {
        .dtype = DTYPE_FLOAT32, .device = tensor->device, .has_dtype = true, .has_device = true};
...
Tensor* cml_quantize_weight_int8(Tensor* weight, bool symmetric) {
    ...
    TensorConfig config = {
        .dtype = DTYPE_INT8, .device = weight->device, .has_dtype = true, .has_device = true};
...
Tensor* cml_quantize_uint8(Tensor* tensor, const QuantParams* params, QuantParams* out_params) {
    ...
    TensorConfig config = {
        .dtype = DTYPE_UINT8, .device = tensor->device, .has_dtype = true, .has_device = true};
    Tensor* quantized = tensor_empty(shape, tensor->ndim, &config);
    cml_free(shape);
    if (!quantized)
        return NULL;
...
const float CML_NF4_TABLE[16] = {-1.0f,    -0.6962f, -0.5251f, -0.3949f, -0.2844f, -0.1848f,
...
Tensor* cml_dequantize_nf4(Tensor* nf4_tensor, const float* scales, int num_scales, int block_size,
    ...
    TensorConfig config = {.dtype      = DTYPE_FLOAT32,
                           .device     = nf4_tensor->device,
                           .has_dtype  = true,
                           .has_device = true};
    ...
    for (size_t i = 0; i < packed_size; i++) {
        ...
        if (elem0 < original_numel) {
            ...
            if (block_idx >= num_scales)
                block_idx = num_scales - 1;
...
QatObserver* cml_qat_observer_create(CmlQatObserverMode mode, float momentum) {
    ...
    if (mode == CML_QAT_OBS_MOVING_AVG_MINMAX && !(momentum > 0.0f && momentum <= 1.0f)) {
        LOG_ERROR("cml_qat_observer_create: momentum must be in (0, 1], got %f", (double)momentum);
        return NULL;
    }
...
int cml_qat_observer_update(QatObserver* obs, Tensor* tensor) {
    ...
    if (!obs->initialized) {
        ...
        if (batch_min < obs->running_min)
            obs->running_min = batch_min;
        if (batch_max > obs->running_max)
            obs->running_max = batch_max;
...
Tensor* cml_qat_fake_quant(Tensor* tensor, const QuantParams* params) {
    ...
    if (!(params->scale > 0.0f)) {
        LOG_ERROR("cml_qat_fake_quant: scale must be positive, got %f", (double)params->scale);
        return NULL;
    }
    tensor_ensure_executed(tensor);
    if (!tensor->data) {
...
Tensor* cml_qat_fake_quant_observed(Tensor* tensor, QatObserver* obs) {
    ...
    if (cml_qat_observer_update(obs, tensor) != 0)
        return NULL;
