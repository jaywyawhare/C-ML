...
static const EfficientNetBlockConfig b0_blocks[] = {
    {1, 32, 16, 3, 1, 1, 4},   {6, 16, 24, 3, 2, 2, 4},  {6, 24, 40, 5, 2, 2, 4},
    {6, 40, 80, 3, 2, 3, 4},   {6, 80, 112, 5, 1, 3, 4}, {6, 112, 192, 5, 2, 4, 4},
    {6, 192, 320, 3, 1, 1, 4},
};
...
static void add_mbconv(Sequential* model, int in_ch, int out_ch, int expand_ratio, int kernel_size,
    ...
    if (expand_ratio != 1) {
        sequential_add(model,
                       (Module*)nn_conv2d(in_ch, expanded, 1, 1, 0, 1, false, dtype, device));
        sequential_add(model,
                       (Module*)nn_batchnorm2d(expanded, 1e-5f, 0.1f, true, true, dtype, device));
        sequential_add(model, (Module*)nn_silu());
    }

    sequential_add(model, (Module*)nn_conv2d(expanded, expanded, kernel_size, stride, padding, 1,
                                             false, dtype, device));
    sequential_add(model,
                   (Module*)nn_batchnorm2d(expanded, 1e-5f, 0.1f, true, true, dtype, device));
    sequential_add(model, (Module*)nn_silu());
...
Module* cml_zoo_efficientnet_b0(const EfficientNetConfig* config) {
    ...
    sequential_add(model,
                   (Module*)nn_batchnorm2d(32, 1e-5f, 0.1f, true, true, cfg.dtype, cfg.device));
    sequential_add(model, (Module*)nn_silu());
    ...
    for (int s = 0; s < num_stages; s++) {
        ...
        for (int b = 0; b < blk->num_blocks; b++) {
            ...
            add_mbconv(model, in_ch, blk->out_channels, blk->expand_ratio, blk->kernel_size, stride,
                       blk->se_ratio, cfg.dtype, cfg.device);
        }
    }

    sequential_add(model, (Module*)nn_conv2d(320, 1280, 1, 1, 0, 1, false, cfg.dtype, cfg.device));
    sequential_add(model,
                   (Module*)nn_batchnorm2d(1280, 1e-5f, 0.1f, true, true, cfg.dtype, cfg.device));
    sequential_add(model, (Module*)nn_silu());
...
static Module* build_efficientnet(float width_mult, float depth_mult, int num_classes,
    ...
    sequential_add(model,
                   (Module*)nn_batchnorm2d(stem_out, 1e-5f, 0.1f, true, true, dtype, device));
    sequential_add(model, (Module*)nn_silu());
    ...
    for (int s = 0; s < num_stages; s++) {
        ...
        for (int b = 0; b < num_reps; b++) {
            ...
            int stride = (b == 0) ? blk->stride : 1;
            add_mbconv(model, in_ch, stage_out, blk->expand_ratio, blk->kernel_size, stride,
                       blk->se_ratio, dtype, device);
        }
        prev_out = stage_out;
    }
    ...
    sequential_add(model,
                   (Module*)nn_batchnorm2d(head_out, 1e-5f, 0.1f, true, true, dtype, device));
    sequential_add(model, (Module*)nn_silu());
    sequential_add(model, (Module*)nn_adaptive_avgpool2d(1, 1));
    sequential_add(model, (Module*)nn_flatten(1, -1));
...
Module* cml_zoo_efficientnet_b1(const EfficientNetConfig* config) {
    ...
    Module* m              = build_efficientnet(cfg.width_mult, cfg.depth_mult, cfg.num_classes,
                                                cfg.dropout_rate, cfg.dtype, cfg.device);
