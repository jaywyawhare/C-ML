...
static int t5_rel_bucket(int rel, bool bidirectional, int num_buckets, int max_distance) {
    ...
    if (n < max_exact) {
        ...
        int v = max_exact + (int)(logf((float)n / (float)max_exact) /
    ...
    for (int h = 0; h < n_head; h++) {
        for (int i = 0; i < seq_q; i++) {
            for (int j = 0; j < seq_k; j++) {
                ...
                idx_data[((size_t)h * seq_q + i) * seq_k + j] = (float)(h * num_buckets + b);
            }
        }
    }
...
static Tensor* t5_enc_block_forward(Module* module, Tensor* input) {
    ...
    Tensor* bias =
        t5_build_rel_bias(block->rel_bias, block->n_head, block->num_buckets, seq, seq, true);
    if (bias && input->ndim == 3)
        bias = t5_batch_bias(bias, input->shape[0]);

    Tensor* attn_out =
        multihead_attention_forward_bias(block->self_attn, normed, normed, normed, NULL, bias);
    if (!attn_out)
        return NULL;
...
static void t5_enc_block_free(Module* module) {
    ...
    if (block->self_attn)
        module_free((Module*)block->self_attn);
    if (block->norm2)
...
static Module* create_t5_enc_block(int d_model, int n_head, int d_ff, int num_buckets, int n_layer,
    ...
    if (module_init((Module*)block, "T5EncoderBlock", t5_enc_block_forward, t5_enc_block_free) !=
        0) {
        cml_free(block);
        return NULL;
    }
...
static Tensor* t5_dec_self_attn(T5DecoderBlock* block, Tensor* input) {
    ...
    int seq = input->ndim == 3 ? input->shape[1] : input->shape[0];
    Tensor* bias =
        t5_build_rel_bias(block->self_rel_bias, block->n_head, block->num_buckets, seq, seq, false);
    if (bias && input->ndim == 3)
        bias = t5_batch_bias(bias, input->shape[0]);
...
static Tensor* t5_dec_block_forward_with_memory(T5DecoderBlock* block, Tensor* input,
    ...
    Tensor* cross_out =
        multihead_attention_forward(block->cross_attn, normed, memory, memory, NULL);
    if (!cross_out)
        return NULL;
...
static void t5_dec_block_free(Module* module) {
    ...
    if (block->norm2)
        module_free((Module*)block->norm2);
    if (block->cross_attn)
        module_free((Module*)block->cross_attn);
    if (block->norm3)
        module_free((Module*)block->norm3);
    if (block->mlp)
        module_free((Module*)block->mlp);
    cml_free(block);
}
...
static Module* create_t5_dec_block(int d_model, int n_head, int d_ff, int num_buckets, int n_layer,
    ...
    if (module_init((Module*)block, "T5DecoderBlock", t5_dec_block_forward, t5_dec_block_free) !=
        0) {
        cml_free(block);
        return NULL;
    }
...
static void t5_free(Module* module) {
    ...
    if (!t5)
        return;
    if (t5->shared_embed)
        module_free((Module*)t5->shared_embed);
    if (t5->enc_blocks)
        module_free((Module*)t5->enc_blocks);
    if (t5->enc_norm)
        module_free((Module*)t5->enc_norm);
    if (t5->dec_blocks)
        module_free((Module*)t5->dec_blocks);
    if (t5->dec_norm)
        module_free((Module*)t5->dec_norm);
    if (t5->lm_head)
        module_free((Module*)t5->lm_head);
    cml_free(t5);
}
...
Module* cml_zoo_t5_create(T5Config* config, DType dtype, DeviceType device) {
    ...
    for (int i = 0; i < config->n_layer; i++)
        module_list_append(t5->enc_blocks, create_t5_enc_block(config->d_model, config->n_head,
                                                               config->d_ff, config->num_buckets,
                                                               config->n_layer, dtype, device));
    t5->enc_norm = nn_layernorm(config->d_model, 1e-6f, true, dtype, device);

    t5->dec_blocks = nn_module_list();
    for (int i = 0; i < config->n_layer; i++)
        module_list_append(t5->dec_blocks, create_t5_dec_block(config->d_model, config->n_head,
                                                               config->d_ff, config->num_buckets,
                                                               config->n_layer, dtype, device));
    ...
    LOG_INFO("Created T5 (%d layers, %d hidden, %d heads, %d vocab)", config->n_layer,
             config->d_model, config->n_head, config->vocab_size);
    return (Module*)t5;
}
