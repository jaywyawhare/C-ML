...
static void backbone_free(Module* module) {
    ...
    if (!bb)
        return;
    if (bb->backbone_stem)
        module_free((Module*)bb->backbone_stem);
    if (bb->layer2)
...
static void fpn_free(Module* module) {
    ...
    if (fpn->extra_p6)
        module_free(fpn->extra_p6);
    if (fpn->extra_p7)
        module_free(fpn->extra_p7);
    cml_free(fpn);
}
...
static void retinanet_free(Module* module) {
    ...
    if (net->fpn)
        module_free(net->fpn);
    if (net->cls_subnet)
        module_free(net->cls_subnet);
    if (net->box_subnet)
        module_free(net->box_subnet);
    cml_free(net);
}
...
Module* cml_zoo_retinanet_create(const RetinaNetConfig* cfg, DType dtype, DeviceType device) {
    ...
    sequential_add(bb->backbone_stem,
                   (Module*)nn_batchnorm2d(64, 1e-5f, 0.1f, true, true, dtype, device));
    ...
    net->cls_subnet =
        create_subnet(c.fpn_channels, 4, c.num_classes * c.num_anchors, dtype, device);
    net->box_subnet = create_subnet(c.fpn_channels, 4, 4 * c.num_anchors, dtype, device);

    LOG_INFO("Created RetinaNet (%d classes, %d anchors, FPN %d)", c.num_classes, c.num_anchors,
             c.fpn_channels);
    return (Module*)net;
}
