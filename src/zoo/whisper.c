...
Module* cml_zoo_whisper(const WhisperConfig* config) {
    ...
    if (cfg.size == WHISPER_TINY)
        size_str = "tiny";
    else if (cfg.size == WHISPER_SMALL)
        size_str = "small";
    else if (cfg.size == WHISPER_MEDIUM)
        size_str = "medium";
    else if (cfg.size == WHISPER_LARGE)
        size_str = "large";

    LOG_INFO("Created Whisper-%s: %d encoder layers, %d decoder layers, dim=%d", size_str,
             cfg.n_audio_layer, cfg.n_text_layer, cfg.n_audio_state);
    return (Module*)model;
}
