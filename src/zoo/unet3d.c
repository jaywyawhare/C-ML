...
static void unet3d_free(Module* module) {
    ...
    for (int i = 0; i < net->depth; i++) {
        if (net->enc_blocks[i])
            module_free((Module*)net->enc_blocks[i]);
        if (net->pools[i])
            module_free((Module*)net->pools[i]);
        if (net->ups[i])
            module_free((Module*)net->ups[i]);
        if (net->dec_blocks[i])
            module_free((Module*)net->dec_blocks[i]);
    }
    if (net->bottleneck)
        module_free((Module*)net->bottleneck);
    if (net->final_conv)
        module_free((Module*)net->final_conv);
    cml_free(net);
}
...
Module* cml_zoo_unet3d_create(const CMLUNet3DConfig* config, DType dtype, DeviceType device) {
    ...
    net->final_conv =
        nn_conv3d(config->base_filters, config->num_classes, 1, 1, 0, 1, true, dtype, device);

    LOG_INFO("Created UNet3D (depth=%d, base_filters=%d, in=%d, classes=%d)", depth,
