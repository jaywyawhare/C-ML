...
bool sym_divisible_by(const SymExpr* expr, int64_t divisor) {
    ...
    case SYM_MUL: {
        int64_t cv;
        if (is_const_any(expr->binop.right, &cv) && cv % divisor == 0)
            return true;
        if (is_const_any(expr->binop.left, &cv) && cv % divisor == 0)
            return true;
...
SymExpr* sym_simplify_divmod(SymExpr* e) {
    ...
    if (e->type == SYM_ADD || e->type == SYM_MUL || e->type == SYM_DIV || e->type == SYM_MOD ||
        ...
        case SYM_MUL:
            ...
            break;
        case SYM_DIV:
            rebuilt = sym_floordiv(sl, sr);
            break;
        case SYM_MOD:
            rebuilt = sym_mod_pos(sl, sr);
            break;
        case SYM_MIN:
            rebuilt = sym_min_expr(sl, sr);
            break;
        case SYM_MAX:
            rebuilt = sym_max_expr(sl, sr);
            break;
        default:
            break;
        }
