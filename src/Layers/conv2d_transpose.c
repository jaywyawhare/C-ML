...
/**
 * @brief Initializes a Conv2D Transpose Layer.
 *
 * @param layer Pointer to the Conv2DTransposeLayer structure.
                                ...
                                int padding, int stride, int dilation)
{
    ...
    if (!validate_conv2d_transpose_params(input_channels, output_channels, kernel_size,
                                          input_height, input_width, padding, stride, dilation))
    {
        LOG_ERROR("Invalid parameters for Conv2D Transpose layer.");
    ...
    layer->output_height = compute_output_size_conv2d_transpose(input_height, kernel_size, padding, stride, dilation);
    layer->output_width = compute_output_size_conv2d_transpose(input_width, kernel_size, padding, stride, dilation);
    ...
    LOG_DEBUG("Initialized Conv2DTransposeLayer with input channels: %d, output channels: %d, kernel size: %d, input height: %d, input width: %d, padding: %d, stride: %d, dilation: %d",
...
/**
 * @brief Computes the output size for Conv2D Transpose Layer.
 *
...
int compute_output_size_conv2d_transpose(int input_size, int kernel_size,
                                         int padding, int stride, int dilation)
{
    return stride * (input_size - 1) + (kernel_size - 1) * dilation + 1 - 2 * padding;
}

/**
 * @brief Validates the parameters for Conv2D Transpose Layer.
 *
 * @param input_channels Number of input channels.
 * @param output_channels Number of output channels.
 * @param kernel_size Size of the convolution kernel.
...
/**
 * @brief Performs the forward pass for the Conv2D Transpose Layer.
 *
 * @param layer Pointer to the Conv2DTransposeLayer structure.
...
int forward_conv2d_transpose(Conv2DTransposeLayer *layer, float *input, float *output)
{
    ...
    if (!validate_conv2d_transpose_params(layer->input_channels, layer->output_channels,
                                          layer->kernel_size, layer->input_height,
                                          layer->input_width, layer->padding, layer->stride, layer->dilation))
    {
        return CM_INVALID_PARAMETER_ERROR;
    }
    ...
    LOG_DEBUG("Performing forward pass for Conv2DTransposeLayer.");

    return CM_SUCCESS;
}

/**
 * @brief Performs the backward pass for the Conv2D Transpose Layer.
 *
 * @param layer Pointer to the Conv2DTransposeLayer structure.
 * @param input Input data array.
 * @param output Output data array.
...
int backward_conv2d_transpose(Conv2DTransposeLayer *layer, float *input, float *output, float *d_output, float *d_input)
{
    ...
    LOG_DEBUG("Performing backward pass for Conv2DTransposeLayer.");
...
/**
 * @brief Updates the weights and biases of the Conv2D Transpose Layer.
 *
 * @param layer Pointer to the Conv2DTransposeLayer structure.
...
/**
 * @brief Frees the memory allocated for the Conv2D Transpose Layer.
 *
 * @param layer Pointer to the Conv2DTransposeLayer structure.
 * @return int Error code.
 */
int free_conv2d_transpose(Conv2DTransposeLayer *layer)
{
    ...
    LOG_DEBUG("Freed memory for Conv2DTransposeLayer.");
