...
Timer* profiler_timer_create(const char* name) {
    Timer* timer = CM_MALLOC(sizeof(Timer));
    if (!timer)
        return NULL;
    ...
    if (name) {
        ...
        timer->name = CM_MALLOC(len);
        if (timer->name) {
            memcpy((void*)timer->name, name, len);
        }
    } else {
        timer->name = NULL;
    }
...
Profiler* profiler_create(void) {
    Profiler* profiler = CM_MALLOC(sizeof(Profiler));
    if (!profiler)
        return NULL;
...
int profiler_start(Profiler* profiler, const char* name) {
    ...
    if (profiler->num_timers >= profiler->capacity) {
        ...
        Timer** new_timers = CM_REALLOC(profiler->timers, new_capacity * sizeof(Timer*));
        if (!new_timers)
            return -1;
