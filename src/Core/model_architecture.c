...
ModelArchitecture* model_architecture_create(void) {
    ModelArchitecture* arch = CM_MALLOC(sizeof(ModelArchitecture));
    if (!arch)
        return NULL;
...
static int extract_from_sequential(Sequential* seq, ModelArchitecture* arch) {
    ...
    if (arch->num_layers + num_modules > arch->capacity) {
        size_t new_capacity = arch->capacity == 0 ? 16 : arch->capacity * 2;
        while (new_capacity < arch->num_layers + num_modules) {
            new_capacity *= 2;
        }
        LayerInfo* new_layers = CM_REALLOC(arch->layers, new_capacity * sizeof(LayerInfo));
        if (!new_layers) {
    ...
    for (int i = 0; i < num_modules; i++) {
        ...
        if (extract_layer_info(child, arch->num_layers, info) == 0) {
            arch->num_layers++;
        }
    }
