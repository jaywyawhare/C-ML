...
static clock_t g_epoch_start_time        = 0;
...
static int training_metrics_ensure_capacity(TrainingMetrics* metrics, size_t num_epochs) {
    ...
    metrics->epoch_training_losses =
        CM_REALLOC(metrics->epoch_training_losses, new_capacity * sizeof(float));
    metrics->epoch_training_accuracies =
        CM_REALLOC(metrics->epoch_training_accuracies, new_capacity * sizeof(float));
    metrics->epoch_times = CM_REALLOC(metrics->epoch_times, new_capacity * sizeof(float));
    metrics->epoch_learning_rates =
        CM_REALLOC(metrics->epoch_learning_rates, new_capacity * sizeof(float));

    // Grow validation arrays if they exist
    if (metrics->epoch_validation_losses) {
        metrics->epoch_validation_losses =
            CM_REALLOC(metrics->epoch_validation_losses, new_capacity * sizeof(float));
        metrics->epoch_validation_accuracies =
            CM_REALLOC(metrics->epoch_validation_accuracies, new_capacity * sizeof(float));
    }

    // Grow test arrays if they exist
    if (metrics->epoch_testing_losses) {
        metrics->epoch_testing_losses =
            CM_REALLOC(metrics->epoch_testing_losses, new_capacity * sizeof(float));
        metrics->epoch_testing_accuracies =
            CM_REALLOC(metrics->epoch_testing_accuracies, new_capacity * sizeof(float));
    }
...
TrainingMetrics* training_metrics_create(size_t num_epochs) {
    TrainingMetrics* metrics = CM_MALLOC(sizeof(TrainingMetrics));
    if (!metrics)
        return NULL;
    ...
    metrics->epoch_training_losses     = CM_MALLOC(num_epochs * sizeof(float));
    metrics->epoch_training_accuracies = CM_MALLOC(num_epochs * sizeof(float));
    ...
    metrics->epoch_times      = CM_MALLOC(num_epochs * sizeof(float));
    metrics->total_time       = 0.0f;
    metrics->epoch_start_time = 0;

    // Learning rate history tracking
    metrics->epoch_learning_rates = CM_MALLOC(num_epochs * sizeof(float));
    ...
    if (!metrics->epoch_training_losses || !metrics->epoch_training_accuracies ||
        ...
        if (metrics->epoch_training_losses)
            CM_FREE(metrics->epoch_training_losses);
        if (metrics->epoch_training_accuracies)
            CM_FREE(metrics->epoch_training_accuracies);
        if (metrics->epoch_times)
            CM_FREE(metrics->epoch_times);
        if (metrics->epoch_learning_rates)
            CM_FREE(metrics->epoch_learning_rates);
        CM_FREE(metrics);
        return NULL;
    }
...
void training_metrics_record_epoch_full(TrainingMetrics* metrics, size_t epoch, float train_loss,
    ...
    if (test_loss != INFINITY && test_accuracy != INFINITY && !isnan(test_loss) &&
        !isnan(test_accuracy) && test_loss >= 0.0f && test_accuracy >= 0.0f) {
        if (!metrics->epoch_testing_losses) {
            metrics->epoch_testing_losses     = CM_MALLOC(metrics->num_epochs * sizeof(float));
            metrics->epoch_testing_accuracies = CM_MALLOC(metrics->num_epochs * sizeof(float));
    ...
    // Allow 0 values - they are valid loss/accuracy values
    if (val_loss != INFINITY && val_accuracy != INFINITY && !isnan(val_loss) &&
        !isnan(val_accuracy) && val_loss >= 0.0f && val_accuracy >= 0.0f) {
        if (!metrics->epoch_validation_losses) {
            metrics->epoch_validation_losses     = CM_MALLOC(metrics->num_epochs * sizeof(float));
            metrics->epoch_validation_accuracies = CM_MALLOC(metrics->num_epochs * sizeof(float));
...
void training_metrics_set_summary(TrainingMetrics* metrics, const char* summary) {
    ...
    if (metrics->model_summary) {
        CM_FREE(metrics->model_summary);
    }

    if (summary) {
        size_t len             = strlen(summary);
        metrics->model_summary = CM_MALLOC(len + 1);
        if (metrics->model_summary) {
...
void training_metrics_set_learning_rate(TrainingMetrics* metrics, float lr, const char* scheduler) {
    ...
    if (metrics->lr_schedule) {
        CM_FREE(metrics->lr_schedule);
    }

    if (scheduler) {
        size_t len           = strlen(scheduler);
        metrics->lr_schedule = CM_MALLOC(len + 1);
        if (metrics->lr_schedule) {
...
void training_metrics_set_lr_schedule_params(TrainingMetrics* metrics, const char* params) {
    ...
    if (metrics->lr_schedule_params) {
        CM_FREE(metrics->lr_schedule_params);
    }

    if (params) {
        size_t len                  = strlen(params);
        metrics->lr_schedule_params = CM_MALLOC(len + 1);
        if (metrics->lr_schedule_params) {
...
int training_metrics_export_json(const TrainingMetrics* metrics, const char* path,
    ...
    if (completed_epochs_with_time > 0) {
        avg_epoch_time /= completed_epochs_with_time;
    }
    ...
    if (avg_epoch_time > 0.0f && current_epoch < metrics->num_epochs) {
        estimated_remaining = avg_epoch_time * (metrics->num_epochs - current_epoch);
    }

    // Export time metrics (always export, even if 0)
    fprintf(f, "  \"epoch_time\": %.6f,\n", avg_epoch_time > 0.0f ? avg_epoch_time : 0.0f);
    fprintf(f, "  \"time_per_epoch\": %.6f,\n", avg_epoch_time > 0.0f ? avg_epoch_time : 0.0f);
    fprintf(f, "  \"total_time\": %.6f,\n",
            metrics->total_time > 0.0f ? metrics->total_time : 0.0f);
    fprintf(f, "  \"elapsed_time\": %.6f,\n",
            metrics->total_time > 0.0f ? metrics->total_time : 0.0f);
    fprintf(f, "  \"estimated_remaining\": %.6f,\n",
            estimated_remaining > 0.0f ? estimated_remaining : 0.0f);
    fprintf(f, "  \"epochs_per_hour\": %.6f,\n", epochs_per_hour > 0.0f ? epochs_per_hour : 0.0f);

    // Export learning rate (always export, even if 0)
    fprintf(f, "  \"learning_rate\": %.8f,\n",
            metrics->learning_rate > 0.0f ? metrics->learning_rate : 0.0f);
    fprintf(f, "  \"lr\": %.8f,\n", metrics->learning_rate > 0.0f ? metrics->learning_rate : 0.0f);
    ...
    for (size_t i = 0; i < num_epochs_to_export; i++) {
        if (i > 0)
            fputs(", ", f);
        fprintf(f, "%.8f",
                metrics->epoch_learning_rates[i] > 0.0f ? metrics->epoch_learning_rates[i] : 0.0f);
    }
    fputs("],\n", f);
    ...
    fprintf(f, "  \"gradient_norm\": %.8f,\n",
            metrics->gradient_norm > 0.0f ? metrics->gradient_norm : 0.0f);
    ...
    for (size_t i = 0; i < num_epochs_to_export; i++) {
        if (i > 0)
            fputs(", ", f);
        fprintf(f, "%.6f", metrics->epoch_training_losses[i]);
    }
    fputs("],\n", f);
    ...
    for (size_t i = 0; i < num_epochs_to_export; i++) {
        if (i > 0)
            fputs(", ", f);
        fprintf(f, "%.6f", metrics->epoch_training_accuracies[i]);
    }
    fputs("],\n", f);
    ...
    if (metrics->epoch_testing_losses) {
        ...
        if (has_test_data && test_loss != INFINITY) {
            fprintf(f, "  \"testing_loss\": %.6f,\n", test_loss);
            fprintf(f, "  \"test_loss\": %.6f,\n", test_loss);
        }

        if (has_test_data && test_accuracy != INFINITY) {
        ...
        if (has_test_data) {
            ...
            for (size_t i = 0; i < num_epochs_to_export; i++) {
                ...
                float val = (metrics->epoch_testing_losses[i] == INFINITY ||
            ...
            for (size_t i = 0; i < num_epochs_to_export; i++) {
                if (i > 0)
                    fputs(", ", f);
                // Export INFINITY as 0.0f (not evaluated)
                float val = (metrics->epoch_testing_accuracies[i] == INFINITY ||
    ...
    if (metrics->epoch_validation_losses) {
        ...
        for (size_t i = 0; i < num_epochs_to_export; i++) {
            if (i > 0)
                fputs(", ", f);
            // Export INFINITY as 0.0f (not evaluated)
            float val = (metrics->epoch_validation_losses[i] == INFINITY ||
        ...
        for (size_t i = 0; i < num_epochs_to_export; i++) {
            if (i > 0)
                fputs(", ", f);
            // Export INFINITY as 0.0f (not evaluated)
            float val = (metrics->epoch_validation_accuracies[i] == INFINITY ||
...
void training_metrics_log(TrainingMetrics* metrics, size_t epoch) {
    ...
    if (completed_epochs > 0) {
        avg_epoch_time /= completed_epochs;
    }
    ...
    if (avg_epoch_time > 0.0f && epoch < metrics->num_epochs) {
        estimated_remaining = avg_epoch_time * (metrics->num_epochs - epoch - 1);
    }
    ...
    printf("Est. Remaining:   %.2f min\n", estimated_remaining / 60.0f);
    printf("Epochs/Hour:      %.2f %s\n", epochs_display, epochs_unit);
    if (metrics->learning_rate > 0.0f) {
        printf("Learning Rate:     %.8f", metrics->learning_rate);
        if (metrics->lr_schedule) {
...
void training_metrics_free(TrainingMetrics* metrics) {
    ...
    if (metrics->epoch_training_accuracies)
        CM_FREE(metrics->epoch_training_accuracies);
    if (metrics->epoch_testing_losses)
        CM_FREE(metrics->epoch_testing_losses);
    if (metrics->epoch_testing_accuracies)
        CM_FREE(metrics->epoch_testing_accuracies);
    if (metrics->epoch_validation_losses)
        CM_FREE(metrics->epoch_validation_losses);
    if (metrics->epoch_validation_accuracies)
        CM_FREE(metrics->epoch_validation_accuracies);
    if (metrics->epoch_times)
        CM_FREE(metrics->epoch_times);
    if (metrics->epoch_learning_rates)
        CM_FREE(metrics->epoch_learning_rates);
...
int training_step(Module* model, Tensor* X, Tensor* y, Tensor* (*loss_fn)(Tensor*, Tensor*),
    ...
    *loss_out        = loss_data ? loss_data[0] : INFINITY;
    ...
    if (metrics) {
        ...
        if ((viz && viz[0] != '\0') ||
            ...
            const char* metrics_path = "viz-ui/public/training.json";
...
static void training_metrics_auto_detect_epoch(Optimizer* optimizer) {
    ...
    // If zero_grad was called, this is likely the start of a new epoch
    if (g_zero_grad_called && g_epoch_in_progress) {
        // End previous epoch
        if (g_epoch_start_time > 0) {
            clock_t epoch_end     = clock();
            double epoch_time_sec = ((double)(epoch_end - g_epoch_start_time)) / CLOCKS_PER_SEC;
...
void training_metrics_auto_export_architecture(Module* model) {
    if (!model || g_architecture_exported)
        return;

    // Export architecture to JSON
    const char* arch_path = "viz-ui/public/model_architecture.json";
        ...
        if (g_global_metrics) {
            ...
            if (f) {
                ...
                if (size > 0 && size < 1024 * 1024) { // Limit to 1MB
                    char* json_str = CM_MALLOC(size + 1);
                    if (json_str) {
    ...
    if (is_scalar) {
        ...
        if (g_current_epoch < g_global_metrics->num_epochs) {
            ...
            if (g_current_epoch == 0 || loss_value < g_global_metrics->best_loss ||
                g_global_metrics->best_loss == 0.0f) {
...
void training_metrics_auto_capture_validation(float val_loss, float val_accuracy) {
    ...
    if (!g_global_metrics->epoch_validation_losses) {
        g_global_metrics->epoch_validation_losses =
            CM_MALLOC(g_global_metrics->num_epochs * sizeof(float));
    ...
    if (g_current_epoch < g_global_metrics->num_epochs) {
        g_global_metrics->epoch_validation_losses[g_current_epoch]     = val_loss;
        g_global_metrics->epoch_validation_accuracies[g_current_epoch] = val_accuracy;
