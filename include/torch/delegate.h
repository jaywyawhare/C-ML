...
typedef struct TorchDelegate {
    ...
    int (*compile)(struct TorchDelegate* self, CMLGraph_t subgraph, void** blob_out,
                   size_t* blob_size_out);

    /* Runtime: execute a previously compiled blob (required for registration). */
    int (*execute)(struct TorchDelegate* self, const void* blob, size_t blob_size, Tensor** inputs,
...
CML_API TorchDelegatePlan* torch_delegate_partition_graph(CMLGraph_t ir, CMLBackendType preferred);
CML_API void torch_delegate_plan_free(TorchDelegatePlan* plan);

CML_API int torch_delegate_execute_plan(TorchDelegatePlan* plan, Tensor** inputs, int num_inputs,
