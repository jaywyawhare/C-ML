...
int cml_metal_encode_kernel(CMLMetalBackend* backend, void* encoder, CMLMetalKernel* kernel,
                            size_t grid[3], size_t block[3], void** gpu_buffers,
                            int num_gpu_buffers, const void** bytes, const size_t* byte_sizes,
                            int num_bytes);

int cml_metal_launch_kernel(CMLMetalBackend* backend, CMLMetalKernel* kernel, size_t grid[3],
                            size_t block[3], void** buffers, int num_buffers);

void* cml_metal_alloc(CMLMetalBackend* backend, size_t size);
void cml_metal_free(CMLMetalBackend* backend, void* buffer);
int cml_metal_upload(CMLMetalBackend* backend, void* dst_buffer, const void* src_host, size_t size);
int cml_metal_download(CMLMetalBackend* backend, void* dst_host, const void* src_buffer,
                       size_t size);

void* cml_metal_get_or_upload_buffer(CMLMetalBackend* backend, void* data_ptr, size_t size,
                                     bool is_input);

void* cml_metal_alloc_output_buffer(CMLMetalBackend* backend, void* tensor_key, size_t size);

void cml_metal_release_intermediate_buffers(CMLMetalBackend* backend);

int cml_metal_download_buffer(CMLMetalBackend* backend, void* gpu_buf, void* dst, size_t size);
...
int cml_mps_matmul_fused_bias_relu(const float* A, const float* B, const float* bias, float* C,
                                   int M, int N, int K);
