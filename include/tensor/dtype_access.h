...
static inline bool cml_dtype_is_float(DType d) {
    return d == DTYPE_FLOAT32 || d == DTYPE_FLOAT64 || d == DTYPE_FLOAT16 ||
           d == DTYPE_BFLOAT16 || d == DTYPE_FLOAT8_E4M3 || d == DTYPE_FLOAT8_E5M2 ||
...
static inline double cml_load_f64(const void* p, size_t i, DType d) {
    switch (d) {
    case DTYPE_FLOAT32:          return (double)((const float*)p)[i];
    case DTYPE_FLOAT64:          return ((const double*)p)[i];
    case DTYPE_FLOAT16:          return (double)fp16_to_float(((const uint16_t*)p)[i]);
    case DTYPE_BFLOAT16:         return (double)bf16_to_float(((const uint16_t*)p)[i]);
    case DTYPE_FLOAT8_E4M3:      return (double)fp8_e4m3_to_float(((const uint8_t*)p)[i]);
    case DTYPE_FLOAT8_E5M2:      return (double)fp8_e5m2_to_float(((const uint8_t*)p)[i]);
    case DTYPE_FLOAT8_E4M3_FNUZ: return (double)fp8e4m3fnuz_to_float(((const uint8_t*)p)[i]);
    case DTYPE_FLOAT8_E5M2_FNUZ: return (double)fp8e5m2fnuz_to_float(((const uint8_t*)p)[i]);
    default:                     return (double)cml_load_i64(p, i, d);
    }
}

static inline void cml_store_f64(void* p, size_t i, DType d, double v) {
    switch (d) {
    case DTYPE_FLOAT32:          ((float*)p)[i]    = (float)v; break;
    case DTYPE_FLOAT64:          ((double*)p)[i]   = v; break;
    case DTYPE_FLOAT16:          ((uint16_t*)p)[i] = float_to_fp16((float)v); break;
    case DTYPE_BFLOAT16:         ((uint16_t*)p)[i] = float_to_bf16((float)v); break;
    case DTYPE_FLOAT8_E4M3:      ((uint8_t*)p)[i]  = float_to_fp8_e4m3((float)v); break;
    case DTYPE_FLOAT8_E5M2:      ((uint8_t*)p)[i]  = float_to_fp8_e5m2((float)v); break;
    case DTYPE_FLOAT8_E4M3_FNUZ: ((uint8_t*)p)[i]  = float_to_fp8e4m3fnuz((float)v); break;
    case DTYPE_FLOAT8_E5M2_FNUZ: ((uint8_t*)p)[i]  = float_to_fp8e5m2fnuz((float)v); break;
    default:                     cml_store_i64(p, i, d, (int64_t)v); break;
    }
}
