...
void test_pooling()
{
    {
        PollingLayer layer;
        int ret = initialize_polling(&layer, 2, 2);
        assert(ret == CM_SUCCESS);

        ret = compute_polling_output_size(8, 2, 2);
        assert(ret == 4);

        float input[8] = {1, 3, 2, 5, 0, 1, 4, 2};
        float output[4];
        ret = forward_polling(&layer, input, output, 8);
        assert(ret == 4);
...

    {
        int ret;
        PollingLayer layer;
        ret = initialize_polling(&layer, 0, 2);
        assert(ret == CM_INVALID_KERNEL_SIZE_ERROR);

        ret = initialize_polling(&layer, 2, 0);
        assert(ret == CM_INVALID_STRIDE_ERROR);

        ret = initialize_polling(&layer, 2, 2);
        assert(ret == CM_SUCCESS);

        ret = compute_polling_output_size(-1, 2, 2);
        assert(ret == CM_INVALID_INPUT_ERROR);

        ret = forward_polling(&layer, NULL, (float[4]){0}, 8);
        assert(ret == CM_NULL_POINTER_ERROR);
        ret = forward_polling(&layer, (float[8]){1, 2, 3, 4, 5, 6, 7, 8}, NULL, 8);
        assert(ret == CM_NULL_POINTER_ERROR);
