...
static int test_rope_basic(void) {
    ...
    CMLRoPEConfig rope_cfg = {.dim = 8, .max_seq_len = 128, .base = 10000.0f};
...
static int test_moe_create_free(void) {
    CMLMoEConfig cfg = {.num_experts       = 4,
                        .top_k             = 2,
                        .input_dim         = 16,
                        .hidden_dim        = 32,
    ...
    if (!moe)
        return 0;
    if (moe->config.num_experts != 4) {
        cml_moe_free(moe);
        return 0;
    }
    if (moe->config.top_k != 2) {
        cml_moe_free(moe);
        return 0;
    }
    if (!moe->gate_weight) {
...
static int test_moe_routing(void) {
    ...
    if (routing->shape[0] != 2) {
        tensor_free(routing);
        cml_moe_free(moe);
        return 0;
    }
    if (routing->shape[1] != 4) {
        tensor_free(routing);
        cml_moe_free(moe);
        return 0;
    }
...
static int test_moe_single_expert(void) {
    ...
    CMLMoEConfig cfg = {.num_experts       = 2,
                        .top_k             = 2,
                        .input_dim         = 4,
                        .hidden_dim        = 8,
                        .capacity_factor   = 1.0f,
...
static int test_tokenizer_special_tokens(void) {
    ...
    if (tok->eos_token_id != 1) {
        cml_tokenizer_free(tok);
        return 0;
    }
    if (tok->pad_token_id != 2) {
        cml_tokenizer_free(tok);
        return 0;
    }
    if (tok->unk_token_id != 3) {
        cml_tokenizer_free(tok);
        return 0;
    }
...
static int test_tokenizer_vocab_size(void) {
    ...
    if (!tok)
        return 0;
    if (cml_tokenizer_vocab_size(tok) != 3) {
        cml_tokenizer_free(tok);
        return 0;
    }
    ...
    /* Also test with NULL */
    if (cml_tokenizer_vocab_size(NULL) != 0)
        return 0;
