...
static int test_model_churn(void) {
    ...
    for (int cycle = 0; cycle < 8; cycle++) {
        ...
        if (!l1 || !l2 || !seq || sequential_add(seq, (Module*)l1) != 0 ||
...
static int test_attention_soak(void) {
    ...
    if (!lq || !lk || !lv || !lo || !seq)
        return 0;
    sequential_add(seq, (Module*)lq);
    sequential_add(seq, (Module*)lk);
    sequential_add(seq, (Module*)lv);
    sequential_add(seq, (Module*)lo);
