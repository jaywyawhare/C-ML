...
static void test_sliding_window(void) {
    ...
    CMLGQAConfig cfg_full = {.num_heads    = num_heads,
                             .num_kv_heads = num_kv_heads,
                             .head_dim     = head_dim,
                             .scale        = 0.0f,
                             .causal       = true,
    ...
    tensor_free(K);
    tensor_free(V);
    tensor_free(out_full);
    tensor_free(out_win);
    tensor_free(out_flash_win);
    tests_passed++;
    printf("  PASS\n");
}
...
static void test_flash_large_tiles(void) {
    ...
    CMLGQAConfig cfg = {.num_heads    = num_heads,
                        .num_kv_heads = num_kv_heads,
                        .head_dim     = head_dim,
                        .scale        = 0.0f,
                        .causal       = true,
    ...
    CMLFlashAttentionConfig flash_cfg = {.tile_size_q  = 128, /* larger than seq_len */
                                         .tile_size_kv = 128,
                                         .enabled      = true};
