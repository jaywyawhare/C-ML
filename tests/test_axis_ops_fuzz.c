...
static const char* ax_name(int o) {
    static const char* n[NUM_AX_OPS] = {
        "sum",    "mean",   "prod",      "max_reduce", "min_reduce", "var",    "std",
        "any",    "all",    "logsumexp", "argmax",     "argmin",     "cumsum", "cumprod",
        "cummax", "cummin", "sort_asc",  "sort_desc",  "argsort"};
    return n[o];
}
...
static Tensor* build(int op, Tensor* a, int axis) {
    ...
    case R_SUM:
        return uop_sum(a, &rp);
    case R_MEAN:
        return uop_mean(a, &rp);
    case R_PROD:
        return uop_prod(a, &rp);
    case R_MAX:
        return uop_max_reduce(a, &rp);
    case R_MIN:
        return uop_min_reduce(a, &rp);
    case R_VAR:
        return uop_var(a, &rp);
    case R_STD:
        return uop_std(a, &rp);
    ...
    case R_ALL:
        return uop_all(a, &rp);
    case R_LOGSUMEXP:
        return uop_logsumexp(a, &rp);
    case R_ARGMAX:
        return uop_argmax(a, &rp);
    case R_ARGMIN:
        return uop_argmin(a, &rp);
    case C_CUMSUM:
        return uop_cumsum(a, axis);
    case C_CUMPROD:
        return uop_cumprod(a, axis);
    case C_CUMMAX:
        return uop_cummax(a, axis);
    case C_CUMMIN:
        return uop_cummin(a, axis);
    case S_SORT_ASC:
        return uop_sort(a, axis, false);
    case S_SORT_DESC:
        return uop_sort(a, axis, true);
    case S_ARGSORT:
        return uop_argsort(a, axis, false);
    default:
        return NULL;
    }
}
...
static void ref_lane(int op, const float* in, size_t base, size_t inner, size_t count,
    ...
    double run = (op == C_CUMPROD)  ? 1
                 : (op == C_CUMMAX) ? -INFINITY
                 : (op == C_CUMMIN) ? INFINITY
                                    : 0;
    for (size_t j = 0; j < count; j++) {
        ...
        for (size_t m = 0; m < inner; m++) {
            ...
            for (size_t j = 0; j < nout; j++) {
                ...
                if (fabsf(got - lane[j]) > tol) {
                    printf("  %-11s iter %d: [%zu] = %g, expected %g   shape [", ax_name(op), iter,
                           oi, got, lane[j]);
                    for (int i = 0; i < nd; i++)
                        printf("%d%s", shape[i], i + 1 < nd ? "," : "");
                    printf("] axis %d\n", axis);
