...
int main(void) {
    ...
    int sok = sig != NULL &&
              cml_hcq_signal_record(q, sig) == 0 && sig->signaled &&
              cml_hcq_queue_wait(q, sig) == 0 &&
              cml_hcq_signal_wait_cpu(sig, 0) == 0 &&
    ...
    int pok = pipe && s1 && s2 &&
              cml_hcq_pipeline_add_stage(pipe, s1) == 0 &&
              cml_hcq_pipeline_add_stage(pipe, s2) == 0 &&
              cml_hcq_pipeline_execute(pipe) == 0 &&
