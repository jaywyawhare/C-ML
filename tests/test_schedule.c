...
static int test_is_elementwise(void) {
    return cml_schedule_is_elementwise(UOP_ADD) && cml_schedule_is_elementwise(UOP_MUL) &&
           cml_schedule_is_elementwise(UOP_SIGMOID) && cml_schedule_is_elementwise(UOP_NEG) &&
           cml_schedule_is_elementwise(UOP_TANH) && cml_schedule_is_elementwise(UOP_ELU) &&
           cml_schedule_is_elementwise(UOP_SILU) && !cml_schedule_is_elementwise(UOP_SUM) &&
           !cml_schedule_is_elementwise(UOP_MATMUL);
}

static int test_is_reduction(void) {
    return cml_schedule_is_reduction(UOP_SUM) && cml_schedule_is_reduction(UOP_MEAN) &&
           cml_schedule_is_reduction(UOP_MAX_REDUCE) && cml_schedule_is_reduction(UOP_PROD) &&
           cml_schedule_is_reduction(UOP_ARGMAX) && cml_schedule_is_reduction(UOP_LOGSUMEXP) &&
           !cml_schedule_is_reduction(UOP_ADD) && !cml_schedule_is_reduction(UOP_MATMUL);
}

static int test_is_movement(void) {
    return cml_schedule_is_movement(UOP_RESHAPE) && cml_schedule_is_movement(UOP_PERMUTE) &&
           cml_schedule_is_movement(UOP_EXPAND) && cml_schedule_is_movement(UOP_FLATTEN) &&
           cml_schedule_is_movement(UOP_CAT) && !cml_schedule_is_movement(UOP_ADD) &&
           !cml_schedule_is_movement(UOP_SUM);
}

static int test_can_fuse_elementwise(void) {
    return cml_schedule_can_fuse(UOP_ADD, UOP_MUL) && cml_schedule_can_fuse(UOP_MUL, UOP_SIGMOID);
}

static int test_cannot_fuse_after_reduce(void) {
    /* reduction -> elementwise must NOT fuse */
    return !cml_schedule_can_fuse(UOP_SUM, UOP_ADD) && !cml_schedule_can_fuse(UOP_MEAN, UOP_MUL);
}

static int test_can_fuse_before_reduce(void) {
    /* elementwise -> reduction CAN fuse */
    return cml_schedule_can_fuse(UOP_ADD, UOP_SUM) && cml_schedule_can_fuse(UOP_MUL, UOP_MEAN);
}
...
static int test_schedule_single_op(void) {
    ...
    CMLSchedule* s = cml_schedule_create(g, NULL);
    int ok         = (s != NULL && cml_schedule_num_kernels(s) == 1 && s->total_ops == 1);
    if (ok) {
...
static int test_schedule_fusion_ratio(void) {
    ...
    CMLSchedule* s = cml_schedule_create(g, NULL);
    int ok         = (s != NULL && s->total_ops == 6 && s->total_kernels == 1);
    if (ok) {
...
static int test_schedule_get_item_bounds(void) {
    ...
    CMLSchedule* s = cml_schedule_create(g, NULL);
    int ok = (cml_schedule_get_item(s, -1) == NULL) && (cml_schedule_get_item(s, 999) == NULL) &&
             (cml_schedule_get_item(NULL, 0) == NULL) && (cml_schedule_get_item(s, 0) != NULL);
    cml_schedule_free(s);
    cml_ir_free(g);
    tensor_free(a);
    return ok;
...
static int test_schedule_num_kernels_null(void) { return cml_schedule_num_kernels(NULL) == 0; }
