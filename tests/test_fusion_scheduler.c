...
static int test_fusion_create_empty(void) {
    ...
    int ok =
        (s != NULL && s->num_groups == 0 && s->total_ops_before == 0 && s->total_groups_after == 0);
...
static int test_fusion_single_op(void) {
    ...
    CMLFusionSchedule* s = cml_fusion_schedule_create(g, NULL);
    int ok               = (s != NULL && s->num_groups == 1 && s->total_ops_before == 1);
    if (ok) {
        ok = (s->groups[0] != NULL && s->groups[0]->num_nodes == 1 &&
...
static int test_fusion_elementwise_chain_fused(void) {
    ...
    if (ok) {
        ok = (s->groups[0]->num_nodes == 3 && s->groups[0]->type == SCHED_ELEMENTWISE);
    }
    cml_fusion_schedule_free(s);
    cml_ir_free(g);
    tensor_free(a);
...
static int test_fusion_reduce_breaks_fusion(void) {
    ...
    CMLFusionSchedule* s = cml_fusion_schedule_create(g, NULL);
    int ok               = (s != NULL && s->total_ops_before == 3 && s->num_groups == 2);
    if (ok) {
        /* First group: add + sum (reduce), second: mul (elem) */
        ok = (s->groups[0]->type == SCHED_REDUCE && s->groups[0]->num_nodes == 2);
        ok = ok && (s->groups[1]->type == SCHED_ELEMENTWISE && s->groups[1]->num_nodes == 1);
    }
    cml_fusion_schedule_free(s);
    cml_ir_free(g);
    tensor_free(a);
...
static int test_fusion_matmul_bias_fused(void) {
    ...
    if (ok) {
        ok = (s->groups[0]->type == SCHED_MATMUL && s->groups[0]->num_nodes == 2);
    }
    cml_fusion_schedule_free(s);
    cml_ir_free(g);
    tensor_free(a);
...
static int test_fusion_ratio_gt_1(void) {
    ...
    CMLFusionSchedule* s = cml_fusion_schedule_create(g, NULL);
    int ok               = (s != NULL && s->total_ops_before == 6 && s->total_groups_after == 1 &&
              s->fusion_ratio > 1.0f);
    cml_fusion_schedule_free(s);
    cml_ir_free(g);
    tensor_free(a);
...
static int test_fusion_execution_order(void) {
    ...
    CMLFusionSchedule* s = cml_fusion_schedule_create(g, NULL);
    int ok = (s != NULL && s->execution_order != NULL && s->num_ordered == s->num_groups);
    /* Order should be sequential: 0, 1, ... */
    if (ok) {
        for (int i = 0; i < s->num_ordered; i++) {
            if (s->execution_order[i] != i) {
                ok = 0;
                break;
            }
        }
