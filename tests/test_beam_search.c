...
static void test_create_free(void) {
    ...
    REQUIRE(ctx != NULL);
    REQUIRE(ctx->beam_width > 0);
    REQUIRE(ctx->warmup_runs >= 0);
    REQUIRE(ctx->timing_runs >= 0);
    REQUIRE(ctx->cache_count == 0);
    REQUIRE(ctx->num_candidates == 0);
...
static void test_store_and_lookup(void) {
    ...
    REQUIRE(ret == 0);
    REQUIRE(found_config.block_size_x == 128);
    REQUIRE(found_config.unroll_factor == 4);
    REQUIRE(found_config.vec_width == 4);
...
static void test_store_multiple(void) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        REQUIRE(ret == 0);
        REQUIRE(found.block_size_x == 32 * (i + 1));
        REQUIRE(found.vec_width == i + 1);
    }
...
static void test_tune(void) {
    ...
    REQUIRE(ret == 0);
    REQUIRE(cached.block_size_x == best.block_size_x);
