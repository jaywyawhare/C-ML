...
static const char* mv_name(int o) {
    static const char* n[NUM_MV_OPS] = {
        "roll",        "repeat_interleave", "cat",      "stack",   "tile",          "pad_const",
        "pad_reflect", "pad_replicate",     "diagonal", "scatter", "masked_select", "gather",
        "nonzero"};
    return n[o];
}
...
static void check(int op, int iter, Tensor* r, const int* want_shape, int want_nd, RefFn ref,
                  void* ctx, const int* in_shape, int in_nd, int p0, int p1) {
    checks++;
    if (!r) {
        report(op, iter, "returned NULL", in_shape, in_nd, p0, p1);
        return;
    }
...
static void one_case(int op, int iter) {
    ...
    case M_REPEAT_INTERLEAVE: {
        ...
        check(op, iter, uop_repeat_interleave(a, rep, dim), want, nd, ref_repeat_interleave, &x,
              shape, nd, rep, dim);
        break;
    }
    case M_CAT: {
    ...
    case M_DIAGONAL: {
        ...
        x.p      = d1;
        x.q      = d2;
        x.dim    = 0; /* ->dim carries the offset */
        int w2[MAXD + 2];
        int k = 0;
        for (int d = 0; d < nd; d++) {
            if (d == d2)
        ...
        check(op, iter, uop_diagonal(a, 0, d1, d2), w2, nd - 1, ref_diagonal, &x, shape, nd, d1,
              d2);
        break;
    }
    case M_SCATTER: {
        ...
        for (size_t i = 0; i < idx->numel; i++)
            tensor_set_float(idx, i, (float)target);
        float srcv[512];
        ...
        checks++;
        if (!r) {
            report(op, iter, "returned NULL", shape, nd, dim, target);
            break;
        }
        tensor_ensure_executed(r);
    ...
    case M_MASKED_SELECT: {
        ...
        checks++;
        if (!r) {
            report(op, iter, "returned NULL", shape, nd, 0, 0);
            break;
        }
        tensor_ensure_executed(r);
        if (r->ndim != 1 || r->numel != expect) {
            char buf[96];
            snprintf(buf, sizeof buf, "ndim %d numel %zu, expected 1-D numel %zu", r->ndim,
                     r->numel, expect);
    ...
    case M_GATHER: {
        ...
        check(op, iter, uop_gather(a, idx, dim), want, nd, ref_gather, &x, shape, nd, n_idx, dim);
        break;
    }
    case M_NONZERO: {
        ...
        checks++;
        if (!r) {
            report(op, iter, "returned NULL", shape, nd, nhit, nd);
            break;
        }
        tensor_ensure_executed(r);
        ...
        if (r->ndim != want_nd || r->numel != want_numel) {
            char buf[112];
            snprintf(buf, sizeof buf, "ndim %d numel %zu, expected ndim %d numel %zu", r->ndim,
                     r->numel, want_nd, want_numel);
...
int main(void) {
    ...
    for (int op = 0; op < NUM_MV_OPS && failures < 20; op++)
        for (int i = 0; i < ITERS && failures < 20; i++)
            one_case(op, i);
