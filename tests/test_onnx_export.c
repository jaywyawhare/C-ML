...
static void test_roundtrip_matmul_add_relu(void) {
    ...
    int rc = cml_onnx_export_graph(R->ir_context,
                                   (Tensor *[]){X}, 1,
                                   (Tensor *[]){R}, 1, path);
...
static void test_roundtrip_linear(void) {
    ...
    for (int i = 0; i < 12; i++) wd[i] = (float)((i % 7) - 3) / 7.0f;
    for (int i = 0; i < 3; i++) bd[i] = (float)(i - 1) / 3.0f;
    ...
    int rc = cml_onnx_export_graph(X->ir_context, NULL, 0,
                                   (Tensor *[]){Y}, 1, path);
    ...
    if (m) {
        ...
        CHECK("final op is Gemm", last >= 0 &&
              strcmp(m->graph.nodes[last].op_type, "Gemm") == 0);
        int transb = -1;
        if (last >= 0)
