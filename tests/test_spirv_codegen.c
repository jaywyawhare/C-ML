...
static int validate_spirv(const uint32_t* w, size_t nbytes) {
    if (!w || nbytes < 5 * sizeof(uint32_t))
        return 0;
    size_t n = nbytes / sizeof(uint32_t);
    if (w[0] != SPIRV_MAGIC) {
        printf("(bad magic 0x%08x) ", w[0]);
        return 0;
    }
    ...
    while (i < n) {
        ...
        if (wc == 0 || i + wc > n) {
            printf("(desync at word %zu wc=%u) ", i, wc);
            return 0;
        }
        if (op == OP_EXECUTION_MODE) {
            if (wc != 6 || w[i + 2] != EXEC_MODE_LOCAL_SIZE) {
                printf("(bad ExecutionMode) ");
                return 0;
            }
            found_exec_mode = 1;
        }
    ...
    if (i != n) {
        printf("(instructions don't fill module) ");
        return 0;
    }
    if (!found_exec_mode) {
        printf("(no OpExecutionMode) ");
        return 0;
    }
    return 1;
}
