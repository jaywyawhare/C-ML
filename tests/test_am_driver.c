...
static bool test_enumerate_gpus(void) {
    ...
    if (ret == 0) {
        ...
        for (int i = 0; i < count; i++) {
            printf("\n    GPU[%d]: %s [%s] gpu_id=%u CUs=%d", i, gpus[i].name, gpus[i].gfx_version,
...
static bool test_gpu_info_topology(void) {
    ...
    if (drv->gpu_id == 0)
        ok = false;
    if (drv->gfx_version[0] == '\0')
        ok = false;

    printf("(gpu_id=%u gfx=%s CUs=%d) ", drv->gpu_id, drv->gfx_version, drv->cu_count);
...
static bool test_mock_enumerate_gpus(void) {
    ...
    printf("(found %d GPU: %s [%s] gpu_id=%u) ", count, gpus[0].name, gpus[0].gfx_version,
           gpus[0].gpu_id);
...
static bool test_mock_driver_init(void) {
    ...
    printf("(gpu_id=%u gfx=%s queues=%d sdma=%d) ", drv->gpu_id, drv->gfx_version,
