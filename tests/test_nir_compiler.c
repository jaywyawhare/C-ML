...
static int test_create_fields(void) {
    ...
    if (c->target != NIR_TARGET_RADEONSI) {
        cml_nir_compiler_free(c);
        return 0;
    }

    /* SPIR-V output should be empty before compilation */
    if (c->spirv_output != NULL) {
        cml_nir_compiler_free(c);
        return 0;
    }
    if (c->spirv_size != 0) {
    ...
    if (cml_nir_binary_size(c) != 0) {
        cml_nir_compiler_free(c);
        return 0;
    }
    if (cml_nir_binary_data(c) != NULL) {
        cml_nir_compiler_free(c);
        return 0;
    }
...
static int test_emit_uop_uninit(void) {
    /* NULL compiler */
    if (cml_nir_emit_uop(NULL, UOP_ADD, 2) == 0)
        return 0;
...
static int test_binary_null(void) {
    if (cml_nir_binary_size(NULL) != 0)
        return 0;
    if (cml_nir_binary_data(NULL) != NULL)
        return 0;
    return 1;
}
