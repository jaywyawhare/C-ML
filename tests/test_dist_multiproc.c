...
static int run_rank(int rank) {
    ...
    /* ---- all-reduce SUM: rank r contributes (r+1); expect sum_{r}(r+1) ---- */
    {
        ...
        for (int i = 0; i < N; i++)
            if (!approx(buf[i], expect)) {
                fprintf(stderr, "[rank %d] allreduce SUM buf[%d]=%f want %f\n", rank, i, buf[i],
                        expect);
                fails |= 4;
                break;
            }
    ...
    /* ---- all-reduce AVG: expect mean of (r+1) = (WS+1)/2 ---- */
    {
        ...
        for (int i = 0; i < N; i++)
            buf[i] = (float)(rank + 1);
        Tensor t = make_vec(buf, N);
        if (cml_dist_allreduce(&t, DIST_REDUCE_AVG) != 0) {
            fails |= 8;
        }
        float expect = (float)(WS + 1) / 2.0f; /* 2.5 */
        for (int i = 0; i < N; i++)
            if (!approx(buf[i], expect)) {
                fprintf(stderr, "[rank %d] allreduce AVG buf[%d]=%f want %f\n", rank, i, buf[i],
                        expect);
                fails |= 8;
                break;
            }
    ...
    /* ---- broadcast from rank 2: only rank 2 has real data ---- */
    {
        float buf[N];
        for (int i = 0; i < N; i++)
            buf[i] = (rank == 2) ? (float)(100 + i) : -1.0f;
        Tensor t = make_vec(buf, N);
        if (cml_dist_broadcast(&t, 2) != 0) {
            fails |= 16;
        }
        for (int i = 0; i < N; i++)
            if (!approx(buf[i], (float)(100 + i))) {
                fprintf(stderr, "[rank %d] broadcast buf[%d]=%f want %d\n", rank, i, buf[i],
                        100 + i);
                fails |= 16;
                break;
            }
    ...
    /* ---- all-gather: rank r contributes r; output[r] must be all-r ---- */
    {
        ...
        if (cml_dist_allgather(outv, &tin) != 0)
            fails |= 64;
        for (int r = 0; r < WS && !(fails & 64); r++)
            for (int i = 0; i < N; i++)
                if (!approx(obuf[r][i], (float)r)) {
                    fprintf(stderr, "[rank %d] allgather out[%d][%d]=%f want %d\n", rank, r, i,
                            obuf[r][i], r);
                    fails |= 64;
                    break;
                }
