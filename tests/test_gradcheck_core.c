...
static Tensor* leaf(const float* v, int* shape, int ndim, bool rg) {
    ...
    if (t) t->requires_grad = rg;
    return t;
}
static float sum_all(Tensor* t) {
    tensor_ensure_executed(t);
    const float* d = (const float*)t->data;
    float s = 0; for (size_t i = 0; i < t->numel; i++) s += d[i];
    return s;
}
...
static void check_unary(const char* name, unary_fn op, float lo, float hi) {
    ...
    if (!x->grad) { printf("  FAIL: %s (null grad)\n", name); tensor_free(x); return; }
    ...
    for (int i = 0; i < 6; i++) {
        float orig = xdata[i];
        xdata[i] = orig + EPS; Tensor* lp = uop_sum(op(x), &(ReduceParams){NULL,0,false}); float fp = sum_all(lp); tensor_free(lp);
        xdata[i] = orig - EPS; Tensor* lm = uop_sum(op(x), &(ReduceParams){NULL,0,false}); float fm = sum_all(lm); tensor_free(lm);
        xdata[i] = orig;
...
static void check_matmul(void) {
    ...
    if (ok) {
        ...
        for (int i = 0; i < 6 && ok; i++) {
            float orig = xdata[i];
            xdata[i] = orig + EPS; Tensor* lp = uop_sum(uop_matmul(x, w), &(ReduceParams){NULL,0,false}); float fp = sum_all(lp); tensor_free(lp);
            xdata[i] = orig - EPS; Tensor* lm = uop_sum(uop_matmul(x, w), &(ReduceParams){NULL,0,false}); float fm = sum_all(lm); tensor_free(lm);
            xdata[i] = orig;
            float num = (fp - fm) / (2 * EPS);
