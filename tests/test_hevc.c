...
static int test_nal_extraction_vps(void) {
    ...
    uint8_t stream[] = {
        0x00, 0x00, 0x00, 0x01, 0x40, 0x01, 0xAA, 0xBB, 0xCC, 0x00, 0x00, 0x00,
        0x01, 0x42, 0x01, 0xDD, 0x00, 0x00, 0x00, 0x01, 0x44, 0x01, 0xEE,
    };
...
static int test_three_byte_start_code(void) {
    ...
    uint8_t stream[] = {
        0x00, 0x00, 0x01, 0x40, 0x01, 0x11, 0x22, 0x00, 0x00, 0x01, 0x42, 0x01, 0x33,
    };
    cml_hevc_parser_feed(p, stream, sizeof(stream));
...
static int test_emulation_prevention(void) {
    ...
    uint8_t sps_nal[] = {
        ...
        0x01,       /* sps_video_parameter_set_id(4) + sps_max_sub_layers_minus1(3) +
                       temporal_id_nesting(1) */
        0x60, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00,
        0x03, 0x00, 0x00, 0x03, 0x00, 0x7B, 0xA0, 0x02, 0x80, /* profile_tier_level padding */
        0x80, 0x28, 0x80,
    };
...
static int test_iframe_decode_unimplemented_fails_loudly(void) {
    ...
    uint8_t stream[] = {
        0x00, 0x00, 0x00, 0x01, 0x26, 0x01, 0xAA, 0xBB, 0xCC, 0xDD,
        0xEE, 0xFF, 0x00, 0x00, 0x00, 0x01, 0x02, 0x01, 0x00,
    };
    cml_hevc_parser_feed(p, stream, sizeof(stream));
...
static int test_non_idr_decode_returns_null(void) {
    ...
    CMLHEVCNalUnit nal = {
        .data = (const uint8_t*)"\x40\x01\xAA", .size = 3, .type = HEVC_NAL_VPS, .temporal_id = 0};
    CMLHEVCFrame* frame = cml_hevc_decode_iframe(p, &nal);
    cml_hevc_parser_free(p);
    return frame == NULL;
}
...
static int test_multiple_nal_sequence(void) {
    ...
    uint8_t stream[] = {
        0x00, 0x00, 0x00, 0x01, 0x40, 0x01, 0x0C, 0x01, 0x00, 0x00, 0x00, 0x01, 0x42,
        0x01, 0x01, 0x02, 0x00, 0x00, 0x00, 0x01, 0x44, 0x01, 0xC0, 0xF3, 0x00, 0x00,
        0x00, 0x01, 0x26, 0x01, 0x05, 0x06, 0x00, 0x00, 0x00, 0x01, 0x02, 0x01, 0x00,
    };
    cml_hevc_parser_feed(p, stream, sizeof(stream));
