...
static int has_glsl_inst(const uint32_t* w, int n, uint32_t glsl_op) {
    ...
    while (i < n) {
        ...
        if (wc == 0 || i + (int)wc > n)
            return 0;
        if (op == OP_EXT_INST && wc >= 5 && w[i + 4] == glsl_op)
            return 1;
        i += (int)wc;
    }
    return 0;
...
static void check_op(const char* name, UOpType uop, int n_src, uint32_t expect_core,
                     uint32_t expect_glsl) {
    int n       = 0;
    ...
    TEST(label, expect_glsl ? has_glsl_inst(w, n, expect_glsl) : has_opcode(w, n, expect_core));
    cml_free(w);
}
...
static void check_ptx(const char* name, UOpType uop, int n_src, const char* need1,
                      const char* need2) {
...
int main(void) {
    ...
    /* sigmoid/silu are composed, so they must pull in exp and a negate. */
    {
        ...
        TEST("sigmoid: composes exp(-x)",
             w && has_glsl_inst(w, n, GLSL_EXP) && has_opcode(w, n, OP_FNEGATE));
