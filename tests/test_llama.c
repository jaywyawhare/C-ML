...
static CMLLLaMAConfig tiny_test_config(void) {
    CMLLLaMAConfig config = {.vocab_size        = 64,
                             .hidden_size       = 32,
...
static int init_layer_random(CMLLLaMALayer* layer, const CMLLLaMAConfig* config) {
    ...
    if (!layer->q_proj || !layer->k_proj || !layer->v_proj || !layer->o_proj || !layer->gate_proj ||
        !layer->up_proj || !layer->down_proj || !layer->input_layernorm ||
...
static int init_model_random(CMLLLaMAModel* model) {
    ...
    if (!model->embed_tokens || !model->norm || !model->lm_head)
        return -1;

    for (int i = 0; i < model->num_layers; i++) {
        if (init_layer_random(model->layers[i], config) != 0)
            return -1;
    }
...
static int test_config_7b(void) {
    ...
    if (cfg.num_heads != 32)
        return 0;
    if (cfg.num_kv_heads != 32)
        return 0;
    if (cfg.vocab_size != 32000)
        return 0;
    if (cfg.intermediate_size != 11008)
        return 0;
    if (cfg.max_seq_len != 2048)
        return 0;
    if (fabsf(cfg.rope_theta - 10000.0f) > 1e-3f)
        return 0;
    if (fabsf(cfg.rms_norm_eps - 1e-5f) > 1e-8f)
        return 0;
    return 1;
}
...
static int test_config_13b(void) {
    ...
    if (cfg.num_heads != 40)
        return 0;
    if (cfg.num_kv_heads != 40)
        return 0;
    if (cfg.intermediate_size != 13824)
        return 0;
    return 1;
}
...
static int test_config_70b(void) {
    ...
    if (cfg.hidden_size != 8192)
        return 0;
    if (cfg.num_layers != 80)
        return 0;
    if (cfg.num_heads != 64)
        return 0;
    if (cfg.num_kv_heads != 8)
        return 0; /* GQA: 8 KV heads */
    if (cfg.intermediate_size != 28672)
        return 0;
...
static int test_model_create_free(void) {
    ...
    if (!model)
        return 0;
    if (model->num_layers != cfg.num_layers) {
...
static int test_forward_basic(void) {
    ...
    if (model->current_seq_len != seq_len) {
        tensor_free(logits);
        cml_llama_free(model);
        return 0;
    }
