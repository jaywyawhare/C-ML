...
static int test_conv_transpose2d(void) {
    ...
    if (ct->kernel_size[0] != 3 || ct->stride[0] != 2) {
        module_free((Module*)ct);
        return 0;
    }
    module_free((Module*)ct);
...
static int test_upsample_nearest(void) {
    ...
    if (!up)
        return 0;
    if (up->scale_factor != 2.0f) {
        module_free((Module*)up);
        return 0;
    }
    if (up->mode != UPSAMPLE_NEAREST) {
        module_free((Module*)up);
        return 0;
    }
    module_free((Module*)up);
...
static int test_upsample_bilinear(void) {
    ...
    if (!up)
        return 0;
    if (up->output_size[0] != 8 || up->output_size[1] != 8) {
        module_free((Module*)up);
        return 0;
    }
    if (up->mode != UPSAMPLE_BILINEAR) {
        module_free((Module*)up);
        return 0;
    }
    if (!up->align_corners) {
...
static int test_autocast(void) {
    ...
    if (!ctx || !ctx->enabled)
        return 0;
    if (ctx->target_dtype != DTYPE_FLOAT16)
        return 0;
...
static int test_grad_scaler(void) {
    ...
    if (scaler->growth_factor != 2.0f) {
        grad_scaler_free(scaler);
        return 0;
    }
    if (scaler->backoff_factor != 0.5f) {
        grad_scaler_free(scaler);
        return 0;
    }
...
static int test_flatten(void) {
    ...
    if (!fl)
        return 0;
    if (fl->start_dim != 1 || fl->end_dim != -1) {
...
static int test_nll_loss(void) {
    float lp_data[]   = {-0.5f, -1.2f, -2.0f, -1.5f, -0.3f, -1.8f};
