...
static int test_gemm_primitive(void) {
    ...
    int32_t zp  = 2;
    float y[4];
    if (cml_qmatmul_affine_int8(x, w, scale, zp, y, M, K, N) != 0)
        return 0;
    ...
    for (int m = 0; m < M; m++) {
        ...
        for (int n = 0; n < N; n++) {
            float acc = 0;
            for (int k = 0; k < K; k++)
                acc += x[m * K + k] * (float)w[k * N + n];
            float expect = scale * (acc - (float)zp * xsum);
            if (fabsf(y[m * N + n] - expect) > 1e-5f)
                return 0;
        }
    }
    return 1;
...
static int test_end_to_end(int M, int K, int N) {
    ...
    for (int i = 0; i < M * K; i++)
        xd[i] = randf(-2.0f, 2.0f);
    for (int i = 0; i < K * N; i++)
        wd[i] = randf(-1.0f, 1.0f);
    ...
    if (!y || !y->data || y->numel != (size_t)M * N) {
        free(xd);
        free(wd);
        return 0;
    }
    ...
    for (int m = 0; m < M && ok; m++) {
        ...
        for (int n = 0; n < N; n++) {
            float ref = 0;
            for (int k = 0; k < K; k++)
                ref += xd[m * K + k] * wd[k * N + n];
            if (fabsf(yd[m * N + n] - ref) > bound * 1.5f) {
                ok = 0;
                break;
            }
        }
...
static int test_int4_gemm_primitive(void) {
    ...
    float y[4];
    if (cml_qmatmul_affine_int4(x, packed, scale, 0, y, M, K, N) != 0)
        return 0;
    ...
    for (int m = 0; m < M; m++)
        for (int n = 0; n < N; n++) {
            float acc = 0;
            for (int k = 0; k < K; k++)
                acc += x[m * K + k] * (float)vals[k * N + n];
            if (fabsf(y[m * N + n] - scale * acc) > 1e-5f)
                return 0;
        }
    return 1;
}
...
static int test_nf4_end_to_end(int M, int K, int N, int block_size) {
    ...
    for (int m = 0; m < M && ok; m++) {
        ...
        for (int n = 0; n < N; n++) {
            ...
            if (d2 > worst_dequant_gap)
                worst_dequant_gap = d2;
            if (d2 > bound * 1.5f) {
                ok = 0;
                break;
            }
