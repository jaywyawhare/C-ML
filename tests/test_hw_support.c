...
static int test_blackwell_defines(void) {
    if (NV_GPU_ARCH_BLACKWELL != 0x1D0)
        return 0;
    if (QMD_VERSION_BLACKWELL != 0x05)
        return 0;
    if (BLACKWELL_CHANNEL_GPFIFO_A != 0x0000C96F)
        return 0;
    if (BLACKWELL_COMPUTE_A != 0x0000CAC0)
        return 0;
    if (BLACKWELL_DMA_COPY_A != 0x0000C9B5)
        return 0;
    return 1;
}

static int test_blackwell_arch_ordering(void) {
    if (NV_GPU_ARCH_BLACKWELL <= NV_GPU_ARCH_HOPPER)
        return 0;
    if (NV_GPU_ARCH_HOPPER <= NV_GPU_ARCH_ADA)
        return 0;
    if (NV_GPU_ARCH_ADA <= NV_GPU_ARCH_AMPERE)
        return 0;
    if (NV_GPU_ARCH_AMPERE <= NV_GPU_ARCH_TURING)
        return 0;
    return 1;
}
...
static int test_qmd_blackwell_version(void) {
    ...
    if (version_field != QMD_VERSION_BLACKWELL)
        return 0;
    return 1;
}
...
static int test_qmd_hopper_version(void) {
    ...
    uint32_t version_field = (qmd.data[0] >> 4) & 0xF;
    if (version_field != QMD_VERSION_HOPPER)
        return 0;
    return 1;
}
...
static int test_qmd_turing_version(void) {
    ...
    uint32_t version_field = (qmd.data[0] >> 4) & 0xF;
    if (version_field != QMD_VERSION_TURING)
        return 0;
    return 1;
}
...
static int test_qmd_blackwell_grid_block(void) {
    ...
    if ((qmd.data[15] & 0xFFFF) != 256)
        return 0;
    if ((qmd.data[16] & 0xFFFF) != 2)
        return 0;
    if ((qmd.data[17] & 0xFFFF) != 1)
        return 0;
    return 1;
}
...
static int test_qmd_blackwell_program_address(void) {
    ...
    if (qmd.data[8] != 0x12345678)
        return 0;
    if (qmd.data[9] != 0xDEADBEEF)
        return 0;
    return 1;
}
...
static int test_am_chiplet_config_mi300x(void) {
    ...
    if (cfg.num_xcd != 8)
        return 0;
    if (cfg.cu_per_xcd != 38)
        return 0;
    if (cfg.sdma_per_xcd != 1)
        return 0;
...
static int test_am_chiplet_config_mi350(void) {
    ...
    if (cfg.num_xcd != 8)
        return 0;
    if (cfg.cu_per_xcd != 48)
        return 0;
    if (cfg.sdma_per_xcd != 2)
        return 0;
    if (!cfg.unified_memory)
        return 0;
    return 1;
...
static int test_am_defines(void) {
    if (AM_GFX_MI300X != 0x120100)
        return 0;
    if (AM_GFX_MI350 != 0x120200)
        return 0;
    if (AM_GFX_RDNA4 != 0x120300)
        return 0;
    return 1;
}
...
static int test_tc_config_roundtrip(void) {
    ...
    CMLTCConfig custom   = {
          .min_m = 32, .min_n = 32, .min_k = 32, .allow_padding = false, .prefer_fp16 = true};
    cml_tc_set_config(&custom);
    CMLTCConfig got = cml_tc_get_config();
    if (got.min_m != 32 || got.min_n != 32 || got.min_k != 32)
        return 0;
    if (got.allow_padding != false || got.prefer_fp16 != true)
        return 0;
