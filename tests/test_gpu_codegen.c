...
static bool has_gpu(void) { return cml_cuda_available() || cml_rocm_available(); }
...
static int test_dispatch_gpu_detection(void) {
    ...
    printf("(CUDA=%s ROCm=%s) ", cuda_available ? "yes" : "no", rocm_available ? "yes" : "no");
