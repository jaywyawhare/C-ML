...
static float numerical_grad(const float* base, int n, int shape[], int idx, UnaryFn f_fn,
                            float eps) {
...
static int check_numerical_self_consistent(const float* data, int n, UnaryFn f_fn, ScalarFn fprime,
                                           const char* name) {
    int shape[] = {n};
    float eps   = 1e-3f;
    for (int i = 0; i < n; i++) {
        ...
        if (diff > GRAD_ATOL && diff / scale > 0.05f) {
            printf("\n    [%s] grad[%d]: numerical=%.6f exact=%.6f diff=%.6f", name, i, ng, exact,
                   diff);
            return 0;
        }
    }
...
static float d_tanh(float x) {
    float t = tanhf(x);
    return 1.0f - t * t;
}
static float d_abs(float x) { return (x >= 0.0f) ? 1.0f : -1.0f; }
static float d_neg(float x) {
    (void)x;
    return -1.0f;
}
...
static int test_grad_matmul(void) {
    ...
    float* d = C->data;
    if (fabsf(d[0] - 22.0f) > 1e-3f)
        return 0;
    if (fabsf(d[1] - 28.0f) > 1e-3f)
        return 0;
    tensor_free(A);
    tensor_free(B);
    tensor_free(C);
