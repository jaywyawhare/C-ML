...
static double act_ref(int op, double x, double p) {
    switch (op) {
    case A_ELU:
        return x > 0 ? x : p * (exp(x) - 1.0);
    case A_CELU:
        return (x > 0 ? x : 0.0) + fmin(0.0, p * (exp(x / p) - 1.0));
    case A_LEAKY:
        return x > 0 ? x : p * x;
    default:
        return 0;
...
static void act_case(int op, int iter) {
     ...
     * divide by it. */
    float p = (op == A_LEAKY) ? (float)rng_int(1, 40) * 0.01f : (float)rng_int(5, 30) * 0.1f;

    Tensor* r = act_build(op, a, p);
    checks++;
    if (!r) {
        printf("  %-14s NULL (rank %d)\n", act_name(op), nd);
        failures++;
        cml_reset_ir_context();
        return;
    }
    ...
    if (!shape_is(r, shape, nd)) {
        fail_shape(act_name(op), r, shape, nd);
        cml_reset_ir_context();
        return;
    }
...
static void dim_case(int op, int iter) {
    ...
    checks++;
    if (!r) {
        printf("  %-14s NULL (rank %d dim %d keepdim %d)\n", dim_name(op), nd, dim, (int)keep);
        failures++;
        cml_reset_ir_context();
        return;
    }
...
static void softmax_case(int iter) {
    ...
    for (size_t o = 0; o < outer; o++) {
        for (size_t m = 0; m < inner; m++) {
            ...
            for (size_t j = 1; j < count; j++)
                if (saved[base + j * inner] > mx)
                    mx = saved[base + j * inner];
            double sum = 0;
            for (size_t j = 0; j < count; j++)
                sum += exp(saved[base + j * inner] - mx);
            for (size_t j = 0; j < count; j++) {
...
static void masked_fill_case(int iter) {
    ...
    if (!shape_is(r, shape, nd)) {
        fail_shape("masked_fill", r, shape, nd);
        cml_reset_ir_context();
        return;
    }
...
static void unflatten_case(int iter) {
    ...
    checks++;
    if (!r) {
        printf("  %-14s NULL (rank %d dim %d -> %d,%d)\n", "unflatten", nd, dim, sizes[0],
               sizes[1]);
        failures++;
        cml_reset_ir_context();
        return;
    ...
    if (!shape_is(r, want, wn)) {
        fail_shape("unflatten", r, want, wn);
        cml_reset_ir_context();
        return;
    }
...
static void split_case(int iter, int use_chunk) {
    ...
    int n_out    = 0;
    Tensor** out = use_chunk ? uop_chunk(a, arg, dim, &n_out) : uop_split(a, arg, dim, &n_out);
    checks++;
    if (!out) {
        printf("  %-14s NULL (rank %d dim %d arg %d)\n", nm, nd, dim, arg);
        failures++;
        cml_reset_ir_context();
        return;
    }
...
static void expand_case(int iter) {
    ...
    int in_shape[MAXD];
    for (int i = 0; i < nd; i++)
        in_shape[i] = (rng_int(0, 1) == 0) ? 1 : rng_int(2, 3);
    ...
    if (!shape_is(r, out_shape, out_nd)) {
        fail_shape("expand_to", r, out_shape, out_nd);
        cml_reset_ir_context();
        return;
    }
    ...
    for (size_t i = 0; i < r->numel; i++) {
        ...
        for (int d2 = out_nd - 1; d2 >= 0; d2--) {
            ...
            if (id >= 0 && in_shape[id] != 1)
                src += (size_t)coord * in_str[id];
        }
        float got = tensor_get_float(r, i);
        if (!close_enough(got, saved[src])) {
            printf("  %-14s [%zu] = %g, expected %g   in rank %d out rank %d\n", "expand_to", i,
                   got, saved[src], nd, out_nd);
            failures++;
            break;
        }
    }
...
int main(void) {
    ...
    for (int op = 0; op < NUM_ACTS && failures < CAP; op++) {
        for (int i = 0; i < ITERS && failures < CAP; i++)
            act_case(op, i);
        act_nan_case(op);
    }
