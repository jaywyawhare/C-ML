    ...
    /* step scheduler: boundary at step_size */
    {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            if (!(isfinite(lr) && lr >= 0.0f))
                fprintf(stderr, "step bad val %g\n", (double)lr);
            if (lr > prev + 1e-6f)
                fprintf(stderr, "step rose: prev=%g now=%g\n", (double)prev, (double)lr);
...
static int test_training_metrics_matrix(void) {
    ...
    for (size_t e = 0; e < 6; e++) { /* past num_epochs too */
        ...
        training_metrics_record_epoch_full(m, e, 1.0f / (float)(e + 1), 0.1f, 0.2f, 0.3f, 0.4f,
                                           (float)e);
...
static int test_container_parameter_collection(void) {
    ...
    if (!seq || sequential_add(seq, (Module*)l1) != 0 || sequential_add(seq, (Module*)l2) != 0)
        return 0;
