...
static int test_resnet18(void) {
    ...
    if (!m)
        return 0;
    if (module_get_total_parameters(m) <= 0) {
        module_free(m);
        return 0;
    }
    module_free(m);
...
static int test_gpt2_configs(void) {
    ...
    if (sm.n_layer >= md.n_layer)
        return 0;
    if (md.n_layer >= lg.n_layer)
        return 0;
    if (lg.n_layer >= xl.n_layer)
        return 0;

    if (sm.n_embd <= 0 || md.n_embd <= 0)
        return 0;
    return 1;
}
...
static int test_bert_configs(void) {
    ...
    if (tiny.hidden_size > mini.hidden_size)
        return 0;
    if (mini.hidden_size > small.hidden_size)
        return 0;
    if (small.hidden_size > base.hidden_size)
        return 0;
...
static int test_vit_configs(void) {
    ...
    if (tiny.hidden_size > small.hidden_size)
        return 0;
    if (small.hidden_size > base.hidden_size)
        return 0;
    if (base.n_head <= 0)
...
static int test_convnext_configs_increasing(void) {
    ...
    if (tiny.dims[0] > small.dims[0])
        return 0;
    if (small.dims[0] > base.dims[0])
        return 0;
    return 1;
}
...
static int test_efficientnet_scaling(void) {
    ...
    if (b7_cfg.width_mult <= b0_cfg.width_mult)
        return 0;
    if (b7_cfg.depth_mult <= b0_cfg.depth_mult)
        return 0;
    return 1;
}
...
static int test_whisper_configs(void) {
    ...
    if (tiny.n_audio_state >= medium.n_audio_state)
        return 0;
    if (medium.n_audio_state >= large.n_audio_state)
        return 0;
    return 1;
}
...
static int test_yolov8_configs(void) {
    ...
    if (n.input_size <= 0)
        return 0;
    if (s.input_size != n.input_size)
        return 0;

    if (n.num_classes != 80 || s.num_classes != 80 || x.num_classes != 80)
        return 0;
    return 1;
}
...
static int test_clip_configs(void) {
    ...
    if (l14.vision_dim <= b32.vision_dim)
        return 0;
    (void)b16;
    return 1;
}
...
static int test_t5_configs(void) {
    ...
    if (sm.d_model >= ba.d_model)
        return 0;
    if (ba.d_model >= lg.d_model)
        return 0;
    return 1;
}
