...
static double r_selu(double x) {
    return 1.0507009873554805 * (x > 0 ? x : 1.6732632423543772 * (exp(x) - 1));
}
...
static const UnaryOp OPS[] = {
    {"neg", uop_neg, r_neg, DOM_ANY},
    {"exp", uop_exp, r_exp, DOM_SMALL},
    {"log", uop_log, r_log, DOM_POS},
    {"sqrt", uop_sqrt, r_sqrt, DOM_POS},
    {"recip", uop_recip, r_recip, DOM_POS},
    {"abs", uop_abs, r_abs, DOM_ANY},
    {"sin", uop_sin, r_sin, DOM_ANY},
    {"cos", uop_cos, r_cos, DOM_ANY},
    {"tan", uop_tan, r_tan, DOM_UNIT},
    {"relu", uop_relu, r_relu, DOM_ANY},
    {"sigmoid", uop_sigmoid, r_sigm, DOM_ANY},
    {"tanh", uop_tanh, r_tanh, DOM_ANY},
    {"gelu", uop_gelu, r_gelu, DOM_ANY},
    {"sign", uop_sign, r_sign, DOM_ANY},
    {"floor", uop_floor, r_floor, DOM_ANY},
    {"ceil", uop_ceil, r_ceil, DOM_ANY},
    {"round", uop_round, r_round, DOM_ANY},
    {"log2", uop_log2, r_log2, DOM_POS},
    {"exp2", uop_exp2, r_exp2, DOM_SMALL},
    {"asin", uop_asin, r_asin, DOM_UNIT},
    {"acos", uop_acos, r_acos, DOM_UNIT},
    {"atan", uop_atan, r_atan, DOM_ANY},
    {"square", uop_square, r_sq, DOM_ANY},
    {"rsqrt", uop_rsqrt, r_rsqrt, DOM_POS},
    {"erf", uop_erf, r_erf, DOM_ANY},
    {"bitwise_not", uop_bitwise_not, r_bnot, DOM_INT},
    {"log10", uop_log10, r_log10, DOM_POS},
    {"sinh", uop_sinh, r_sinh, DOM_SMALL},
    {"cosh", uop_cosh, r_cosh, DOM_SMALL},
    {"asinh", uop_asinh, r_asinh, DOM_ANY},
    {"acosh", uop_acosh, r_acosh, DOM_GE1},
    {"atanh", uop_atanh, r_atanh, DOM_UNIT},
    {"trunc", uop_trunc, r_trunc, DOM_ANY},
    {"isinf", uop_isinf, r_isinf, DOM_ANY},
    {"isnan", uop_isnan, r_isnan, DOM_ANY},
    {"isfinite", uop_isfinite, r_isfin, DOM_ANY},
    {"logical_not", uop_logical_not, r_lnot, DOM_ANY},
    {"erfc", uop_erfc, r_erfc, DOM_ANY},
    {"relu6", uop_relu6, r_relu6, DOM_ANY},
    ...
    {"selu", uop_selu, r_selu, DOM_ANY},
    {"mish", uop_mish, r_mish, DOM_ANY},
    {"silu", uop_silu, r_silu, DOM_ANY},
    {"hardswish", uop_hardswish, r_hswish, DOM_ANY},
};
...
static float sample(Domain d, int k) {
    switch (d) {
    case DOM_POS:
        return (float)(k % 7) * 0.5f + 0.5f; /* 0.5 .. 3.5  */
    case DOM_UNIT:
        return (float)(k % 9) * 0.2f - 0.8f; /* -0.8 .. 0.8 */
    case DOM_GE1:
        return (float)(k % 5) * 0.75f + 1.0f; /* 1 .. 4      */
    case DOM_SMALL:
        return (float)(k % 7) * 0.5f - 1.5f; /* -1.5 .. 1.5 */
    case DOM_INT:
        return (float)(k % 16);
    default:
        return (float)(k % 9) * 0.75f - 3.0f; /* -3 .. 3     */
    }
}
...
static void one_case(const UnaryOp* op, int iter) {
    ...
    int bad = (r->ndim != nd);
    for (int i = 0; !bad && i < nd; i++)
        if (r->shape[i] != shape[i])
            bad = 1;
    if (bad) {
...
int main(void) {
    ...
    for (int o = 0; o < NUM_OPS && failures < 15; o++)
        for (int i = 0; i < ITERS && failures < 15; i++)
            one_case(&OPS[o], i);
