...
static float numeric_grad(Tensor* (*op)(Tensor*), const float* xs, int n, int idx,
                          float eps, int ndim) {
...
static int eager_grad_vals(Tensor* (*op)(Tensor*), const float* xs, int n, int ndim,
                           float* out) {
    cml_reset_ir_context();
    int sh[2] = {g_rows, g_cols};
    Tensor* x = tensor_zeros(sh, ndim, &cfg);
...
static Tensor* b_mul(Tensor* x) {
    ...
    float w[64];
    for (int j = 0; j < g_cols; j++) w[j] = (float)(j + 1);
    TensorConfig c2 = cfg;
...
static Tensor* r_add_bias(Tensor* x) {
    ...
    float w[64];
    for (int j = 0; j < g_cols; j++) w[j] = 0.5f + 0.1f * j;
    TensorConfig c2 = cfg;
...
static Tensor* r_matmul(Tensor* x) {
    float w[32];
    for (int i = 0; i < g_cols * 3; i++) w[i] = 0.2f + 0.05f * (i % 7);
    TensorConfig c2 = cfg;
...
static int test_eager_accumulation(void) {
    ...
    int sh[2] = {g_rows, g_cols};
    float xs[64];
    for (int i = 0; i < N_ELT; i++) xs[i] = 0.5f;
...
static int test_eager_custom_seed(void) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        if (fabsf(read_at(x->grad, (size_t)i) - (-(float)(i + 1))) > 1e-5f) { ok = 0; break; }
    }
    tensor_free(x); tensor_free(g0);
    cml_reset_ir_context();
    return ok;
}
...
static int test_eager_detached_branch(void) {
    ...
    tensor_free(aux_in); tensor_free(x); tensor_free(aux_out);
    cml_reset_ir_context();
    return ok && aux_clean;
}
...
static int test_vjp_sweep_unary(void) {
    ...
    for (size_t i = 0; i < sizeof(SWEEP)/sizeof(SWEEP[0]); i++) {
        if (strcmp(SWEEP[i].name, "rsqrt_x") == 0) { SWEEP[i].fn = w_rsqrt; SWEEP[i].lo=0.5f; SWEEP[i].hi=2.0f; SWEEP[i].tol=4e-2f; }
        if (strcmp(SWEEP[i].name, "elu") == 0)     { SWEEP[i].fn = w_elu; SWEEP[i].lo=-1.0f; SWEEP[i].hi=1.2f; SWEEP[i].tol=4e-3f; }
        if (strcmp(SWEEP[i].name, "hardsigmoid") == 0) { SWEEP[i].fn = w_hardsigmoid; SWEEP[i].lo=0.3f; SWEEP[i].hi=1.2f; SWEEP[i].tol=4e-3f; }
    }

    int ok = 1;
    int ran = 0;
    for (size_t e = 0; e < sizeof(SWEEP)/sizeof(SWEEP[0]); e++) {
        if (!SWEEP[e].fn) continue;
        ran++;
        float xs[64];
        for (int i = 0; i < N_ELT; i++)
            xs[i] = SWEEP[e].lo +
                    (SWEEP[e].hi - SWEEP[e].lo) * ((float)i + 0.5f) / (float)N_ELT;
        ...
        for (int i = 0; i < N_ELT; i++) {
            ...
            if (isnan(want)) { printf("  %-14s numeric grad NaN\n", SWEEP[e].name); ok = 0; break; }
...
static Tensor* v_mean(Tensor* x){ ReduceParams rp={0}; return uop_mean(x,&rp); }
static Tensor* v_sum_dim0(Tensor* x){ static int d[1]={0}; ReduceParams rp={.dims=d,.num_dims=1,.keepdim=false}; return uop_sum(x,&rp); }
...
static Tensor* v_permute(Tensor* x){ static int p[2]={1,0}; PermuteParams pp={.perm=p,.num_dims=2}; return uop_permute(x,&pp); }
...
static Tensor* v_pad(Tensor* x){ static int pw[4]={1,1,0,0}; return uop_pad(x,pw,2,0.0f); }

static int test_vjp_sweep_reduce_shape(void) {
    struct { const char* name; Op1 fn; float tol; } cases[] = {
        {"mean_all",    v_mean,    3e-3f},
        {"sum_dim0",    v_sum_dim0,2e-3f},
        {"reshape_42",  v_reshape, 2e-3f},
        {"flatten",     v_flatten, 2e-3f},
        {"permute_10",  v_permute, 2e-3f},
        {"triu",        v_triu,    2e-3f},
        {"tril",        v_tril,    2e-3f},
        {"cumsum_d1",   v_cumsum,  3e-3f},
        {"roll_d1",     v_roll,    2e-3f},
        {"pad_left",    v_pad,     2e-3f},
    };
    int ok = 1;
    for (size_t c = 0; c < sizeof(cases)/sizeof(cases[0]); c++) {
        ...
        for (int i = 0; i < N_ELT; i++) {
            ...
            if (isnan(want)) continue;   /* shape-op numeric may be unstable; skip */
...
static int test_vjp_structural(void) {
    ...
    /* conv2d: input + weight grads */
    {
        ...
        for (int i = 0; i < 16; i++) d[i] = 0.1f * (i % 5) + 0.3f;
        float* wd = (float*)tensor_data_ptr(w);
        for (int i = 0; i < 4; i++) wd[i] = 0.25f * (i + 1);
        ...
        if (!y) ok = 0;
            ...
            ReduceParams rp={0};
            Tensor* s = uop_sum(y,&rp); s->requires_grad=true;
            tensor_backward(s,NULL,false,false);
            ok &= in->grad && w->grad && tensor_data_ptr(in->grad) && tensor_data_ptr(w->grad);
            tensor_free(s); tensor_free(y);
        }
    ...
    /* maxpool2d gradient routes through argmax */
    {
        ...
        Tensor* in = tensor_zeros((int[]){1,1,4,4}, 4, &cfg);
        float* d = (float*)tensor_data_ptr(in);
        for (int i = 0; i < 16; i++) d[i] = (float)((i*7)%11)*0.25f+0.1f;
        in->requires_grad = true;
        Pool2DParams pp = {.kernel_size={2,2},.stride={2,2},.padding={0,0},
        ...
        if (!y) ok = 0;
            ...
            if (in->grad) {
                ...
                int nz=0; for(int i=0;i<16;i++) if (fabsf(g[i])>1e-6f) nz++;
                ok &= nz==4;
            }
            tensor_free(s); tensor_free(y);
        }
    ...
    /* cat / stack */
    {
        ...
        Tensor* s=uop_sum(c,&rp); s->requires_grad=true;
        tensor_backward(s,NULL,false,false);
        ok &= a->grad && b->grad;
        tensor_free(c); tensor_free(s);
        if (a->grad) { tensor_free(a->grad); a->grad=NULL; }
        if (b->grad) { tensor_free(b->grad); b->grad=NULL; }
        ...
        s=uop_sum(st_,&rp); s->requires_grad=true;
        tensor_backward(s,NULL,false,false);
        ok &= a->grad && b->grad;
        tensor_free(st_); tensor_free(s);
        if (a->grad) { tensor_free(a->grad); a->grad=NULL; }
        if (b->grad) { tensor_free(b->grad); b->grad=NULL; }
        tensor_free(a); tensor_free(b);
        cml_reset_ir_context();
    ...
    /* scatter / gather round trip */
    {
        ...
        TensorConfig ic = {.dtype=DTYPE_INT32,.device=DEVICE_CPU,.has_dtype=true,.has_device=true};
        Tensor* src = tensor_zeros((int[]){2,3},2,&cfg);
        Tensor* x   = tensor_zeros((int[]){2,3},2,&cfg);
        Tensor* idx = tensor_zeros((int[]){2,3},2,&ic);
        for(int i=0;i<6;i++){((float*)tensor_data_ptr(src))[i]=0.5f;}
        for(int i=0;i<6;i++){((float*)tensor_data_ptr(x))[i]=(float)i+1;}
        x->requires_grad = true;
        ...
        for(int i=0;i<6;i++) id1[i]=i%3;
        Tensor* g = uop_gather(x, idx1d, 1);
        if (g) { tensor_ensure_executed(g); ok &= tensor_data_ptr(g)!=NULL; tensor_free(g); }
        tensor_free(idx1d);
        ...
        tensor_free(src); tensor_free(x); tensor_free(idx);
        cml_reset_ir_context();
    }
     ...
     * nodes into later losses. */
    {
        ...
        Tensor* x = tensor_zeros((int[]){2,3},2,&cfg);
        for(int i=0;i<6;i++) ((float*)tensor_data_ptr(x))[i]=(float)i*0.2f+0.3f;
        x->requires_grad = true;
        Tensor* y1 = uop_pad(x,pw,2,0.0f);
        Tensor* s1=uop_sum(y1,&rp); s1->requires_grad=true;
        tensor_backward(s1,NULL,false,false);
        ok &= x->grad && fabsf(((const float*)tensor_data_ptr(x->grad))[0]-1.0f)<1e-5f;
        tensor_free(y1); tensor_free(s1); tensor_free(x);
        cml_reset_ir_context();
        ...
        Tensor* s2=uop_sum(y2,&rp); s2->requires_grad=true;
        tensor_backward(s2,NULL,false,false);
        ok &= x->grad != NULL;
        tensor_free(y2); tensor_free(s2); tensor_free(x);
        cml_reset_ir_context();
        ...
        Tensor* s3=uop_sum(y3,&rp); s3->requires_grad=true;
        tensor_backward(s3,NULL,false,false);
        ok &= x->grad != NULL &&
              fabsf(((const float*)tensor_data_ptr(x->grad))[0]-2.0f)<1e-5f;
        tensor_free(y3); tensor_free(s3); tensor_free(x);
        cml_reset_ir_context();
    }
