...
static int test_add(void) {
    ...
    int ok    = APPROX_EQ(cd[0], 6.0f) && APPROX_EQ(cd[1], 8.0f) && APPROX_EQ(cd[2], 10.0f) &&
             APPROX_EQ(cd[3], 12.0f);
    tensor_free(a);
    tensor_free(b);
    tensor_free(c);
    return ok;
...
static int test_matmul(void) {
    ...
    float* cd = (float*)c->data;
    int ok    = APPROX_EQ(cd[0], 19.0f) && APPROX_EQ(cd[1], 22.0f) && APPROX_EQ(cd[2], 43.0f) &&
             APPROX_EQ(cd[3], 50.0f);
    tensor_free(a);
    tensor_free(b);
    tensor_free(c);
    return ok;
...
static int test_clone(void) {
    ...
    float* cd = (float*)c->data;
    int ok = (c != t) && APPROX_EQ(cd[0], 1.0f) && APPROX_EQ(cd[1], 2.0f) && APPROX_EQ(cd[2], 3.0f);
    tensor_free(t);
    tensor_free(c);
    return ok;
}
