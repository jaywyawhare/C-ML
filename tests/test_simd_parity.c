...
static void check_unary(const char* name, unary_op_fn op, unary_ref_fn ref, unary_simd_fn simd,
                        float lo, float hi, float atol, float rtol) {
    tests_run++;
    int failures = 0;
    for (int si = 0; si < N_SIZES; si++) {
        ...
        float* sim = NULL;
        if (simd) {
            sim = malloc(sizeof(float) * n);
            simd(in, sim, (size_t)n);
        }
    ...
    if (failures == 0) {
        tests_passed++;
        printf("  PASS: unary %-10s (%d sizes)\n", name, N_SIZES);
    } else {
        printf("  FAIL: unary %-10s (%d mismatches)\n", name, failures);
    }
}
...
static void check_binary(const char* name, binary_op_fn op, binary_ref_fn ref, binary_simd_fn simd,
                         float lo, float hi, float atol, float rtol) {
    tests_run++;
    int failures = 0;
    ...
    if (failures == 0) {
        tests_passed++;
        printf("  PASS: binary %-10s (3 modes x %d sizes)\n", name, N_SIZES);
    } else {
        printf("  FAIL: binary %-10s (%d mismatches)\n", name, failures);
    }
}

/* ---- reductions ------------------------------------------------------- */
static void check_reduction(const char* name, unary_op_fn op, int is_max, int is_mean) {
    tests_run++;
    int failures = 0;
    for (int si = 0; si < N_SIZES; si++) {
        int n     = SIZES[si];
        ...
        if (!y || !y->data) {
            printf("    [%s n=%d] NULL output (y=%p data=%p numel=%zu)\n", name, n, (void*)y,
                   (void*)(y ? y->data : NULL), (size_t)(y ? y->numel : 0));
            failures++;
            if (x)
                tensor_free(x);
            if (y)
    ...
    if (failures == 0) {
        tests_passed++;
        printf("  PASS: reduce %-10s (%d sizes)\n", name, N_SIZES);
    } else {
        printf("  FAIL: reduce %-10s (%d mismatches)\n", name, failures);
    }
}
...
static Tensor* red_sum(Tensor* x) {
    ReduceParams rp = {NULL, 0, false};
    return uop_sum(x, &rp);
}
static Tensor* red_mean(Tensor* x) {
    ReduceParams rp = {NULL, 0, false};
    return uop_mean(x, &rp);
}
static Tensor* red_max(Tensor* x) {
    ReduceParams rp = {NULL, 0, false};
    return uop_max_reduce(x, &rp);
}
