...
static int test_dtype_size(void) {
    if (cml_dtype_size(DTYPE_FLOAT8_E4M3_FNUZ) != 1)
        return 0;
    if (cml_dtype_size(DTYPE_FLOAT8_E5M2_FNUZ) != 1)
        return 0;
    return 1;
}
...
static int test_cast_to_fnuz(void) {
    ...
    if (e4->dtype != DTYPE_FLOAT8_E4M3_FNUZ) {
        tensor_free(src);
        tensor_free(e4);
        tensor_free(e5);
        return 0;
    }
    if (e5->dtype != DTYPE_FLOAT8_E5M2_FNUZ) {
        tensor_free(src);
        tensor_free(e4);
        tensor_free(e5);
        return 0;
    ...
    if (!APPROX_EQ(v4, 1.0f, 0.2f)) {
        tensor_free(src);
        tensor_free(e4);
        tensor_free(e5);
        return 0;
    }
    if (!APPROX_EQ(v5, 1.0f, 0.5f)) {
        tensor_free(src);
        tensor_free(e4);
        tensor_free(e5);
        return 0;
...
static int test_ones_fnuz(void) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (!APPROX_EQ(v4, 1.0f, 0.01f)) {
            tensor_free(t4);
            tensor_free(t5);
            return 0;
        }
        if (!APPROX_EQ(v5, 1.0f, 0.01f)) {
            tensor_free(t4);
            tensor_free(t5);
            return 0;
        }
...
static int test_promote_fnuz(void) {
    ...
    if (r2 != DTYPE_FLOAT8_E5M2_FNUZ)
        return 0;

    DType r3 = cml_promote_dtype(DTYPE_FLOAT8_E4M3_FNUZ, DTYPE_FLOAT8_E5M2_FNUZ);
    if (r3 != DTYPE_FLOAT8_E4M3_FNUZ && r3 != DTYPE_FLOAT8_E5M2_FNUZ)
        return 0;
