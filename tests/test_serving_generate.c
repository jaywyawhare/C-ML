...
static int mock_forward(void* model, const int* tokens, int num_tokens,
                        int position, float* logits, int vocab) {
...
static int test_max_tokens_termination(void) {
    ...
    int ok = cml_serving_get_status(ctx, id) == CML_SEQ_STATUS_FINISHED &&
             n == 8 && toks && memcmp(toks, expect, sizeof(expect)) == 0;
    cml_serving_finish_request(ctx, id);
    cml_serving_free(ctx);
    return ok;
}
...
static int test_batched_requests(void) {
    ...
    int guard = 0;
    while (guard++ < 1000 &&
           (cml_serving_get_status(ctx, a) != CML_SEQ_STATUS_FINISHED ||
