...
static void init_weights(void) {
    for (int i = 0; i < 16; i++) { W1[i] = 0.1f * (i % 5 - 2); W2[i] = 0.1f * (i % 7 - 3); }
    for (int i = 0; i < 4; i++)  { B1[i] = 0.05f * i; B2[i] = -0.1f * i; }
}
...
static void test_static_inference(void) {
    ...
    memcpy(X->data, inB, sizeof inB); cml_ir_reexecute(ir);
    memcpy(outB_s, tensor_data_ptr(out), sizeof outB_s);
    int n2 = node_count(ir);
    memcpy(X->data, inA, sizeof inA); cml_ir_reexecute(ir);
    ...
    cml_reset_ir_context();
    float xb3[16]; memcpy(xb3, inB, sizeof inB);
    float outB_d[4]; memcpy(outB_d, tensor_data_ptr(build_mlp(cml_tensor_2d(xb3, 4, 4))), sizeof outB_d);
...
static void test_static_training(void) {
    ...
    for (int i = 0; i < 16; i++) { xb[i] = 0.1f * (i % 5) - 0.2f; yb[i] = (i % 4 == 0) ? 1.0f : 0.0f; }
    for (int i = 0; i < 16; i++) w[i] = 0.01f * (i % 3 - 1);
    for (int i = 0; i < 4; i++)  bb[i] = 0.0f;

    cml_reset_ir_context();
    CMLGraph_t ir = cml_ir_get_or_create_context();
    Tensor* X = cml_tensor_2d(xb, 4, D);
    Tensor* Y = cml_tensor_2d(yb, 4, D);
    Tensor* W = cml_tensor_2d(w, D, D);  cml_set_requires_grad(W, true);
    Tensor* Bs = cml_tensor_1d(bb, D);   cml_set_requires_grad(Bs, true);
    ...
    for (int it = 0; it < 50; it++) {
        ...
        if (it == 49) { lossN = l; n_last = node_count(ir); }
        float* gw = W->grad ? (float*)W->grad->data : NULL;
        float* gb = Bs->grad ? (float*)Bs->grad->data : NULL;
        if (gw) for (int i = 0; i < 16; i++) ((float*)W->data)[i]  -= lr * gw[i];
        if (gb) for (int i = 0; i < 4; i++)  ((float*)Bs->data)[i] -= lr * gb[i];
    }
