...
static TensorConfig f32cfg = {
    .dtype = DTYPE_FLOAT32, .device = DEVICE_CPU, .has_dtype = true, .has_device = true};
static TensorConfig i32cfg = {
...
static void fixture_free(SpmmFixture* fx) {
    if (fx->sp)
        sparse_free(fx->sp);
    if (fx->indices)
        tensor_free(fx->indices);
    if (fx->values)
        tensor_free(fx->values);
    if (fx->B)
        tensor_free(fx->B);
...
static int check_grad_b(float tol) {
    ...
    SpmmFixture fx;
    if (!run_spmm_backward(&fx, xs, bs, false)) {
        fixture_free(&fx);
        return 0;
    }
    ...
    int ok = fx.B->grad != NULL && fx.B->grad->shape[0] == K && fx.B->grad->shape[1] == N &&
...
static int test_grad_shapes(void) {
    ...
    int ok = fx.B->grad != NULL && fx.B->grad->ndim == 2 && fx.B->grad->shape[0] == K &&
             fx.B->grad->shape[1] == N && fx.sp->values->grad != NULL &&
             fx.sp->values->grad->ndim == 1 && fx.sp->values->grad->shape[0] == NNZ &&
             fx.indices->grad == NULL; /* discrete coordinates take no grad */
    if (!ok)
        printf("\n    B grad %p / values grad %p shapes wrong", (void*)fx.B->grad,
               fx.sp->values ? (void*)fx.sp->values->grad : NULL);
    fixture_free(&fx);
    cml_reset_ir_context();
    return ok;
}
