...
static DType amp_env_expected_dtype(void) {
    ...
    return (env && (strcmp(env, "bf16") == 0 || strcmp(env, "bfloat16") == 0)) ? DTYPE_BFLOAT16
                                                                               : DTYPE_FLOAT16;
}
...
static int test_bf16_training(void) {
    ...
    TensorConfig bcfg = {
        .dtype = DTYPE_BFLOAT16, .device = DEVICE_CPU, .has_dtype = true, .has_device = true};
    int xs[2] = {8, 1}, ws[2] = {1, 1};

    float xd[8];
    for (int i = 0; i < 8; i++)
        xd[i] = (float)i / 4.0f - 1.0f;
    ...
    for (int epoch = 0; epoch < 300 && grads_finite; epoch++) {
        ...
        for (size_t i = 0; i < Xb->numel; i++)
            tensor_set_float(Xb, i, xd[i]);
    ...
    bool ok = grads_finite && last_loss < first_loss && last_loss < 1e-3f && isfinite(master_w);
...
static int test_amp_dtype_api(void) {
    ...
    if (autocast_default_dtype() != amp_env_expected_dtype())
        return 0;
    autocast_enter(DTYPE_FLOAT16);
    if (autocast_get_dtype() != DTYPE_FLOAT16) {
        autocast_exit();
        return 0;
    }
    autocast_exit();
    ...
    if (!ctx || ctx->target_dtype != DTYPE_BFLOAT16)
        return 0;

    autocast_enter(DTYPE_FLOAT32);
    if (autocast_get_dtype() != DTYPE_FLOAT32) {
        autocast_exit();
        return 0;
    }
    autocast_exit();
...
static int test_scaler_noop_under_bf16(void) {
    ...
    if (!scaler)
        return 0;
    if (scaler->scale_factor != 65536.0f) {
    ...
    for (int i = 0; i < 4; i++)
        if (tensor_get_float(grad, i) != 8.0f)
            noop = false;
    loss->grad = NULL;
    ...
    bool scales_fp16 =
        scaled16 && scaled16 != loss16 && fabsf(tensor_get_float(scaled16, 0) - 131072.0f) < 1.0f;
    if (scaled16)
        tensor_free(scaled16);
    tensor_free(loss16);
    grad_scaler_free(s16);
