...
static int test_f64_arith(void) {
    ...
    Tensor* d = uop_sub(ta, tb);
    tensor_ensure_executed(d);
    Tensor* m = uop_mul(ta, tb);
    tensor_ensure_executed(m);
...
static int test_f64_unary(void) {
    ...
    Tensor* ng = uop_neg(ta);
    tensor_ensure_executed(ng);
    Tensor* ab = uop_abs(uop_neg(ta));
    tensor_ensure_executed(ab);
    Tensor* rt = uop_exp(uop_log(ta));
    tensor_ensure_executed(rt); /* exp(log(x)) ~ x */
    Tensor* sq = uop_sqrt(ta);
...
static int test_reductions_global(void) {
    ...
    Tensor* s       = uop_sum(ta, &rp);
    tensor_ensure_executed(s);
    Tensor* m = uop_mean(ta, &rp);
    tensor_ensure_executed(m);
    int ok = s->dtype == DTYPE_FLOAT64 && fabs(((const double*)s->data)[0] - 15.0) < 1e-12 &&
...
static int test_reductions_axis(void) {
    ...
    int d1           = 1;
    ReduceParams rp1 = {&d1, 1, false}; /* sum cols -> [6,15] */
    Tensor* s1       = uop_sum(ta, &rp1);
    tensor_ensure_executed(s1);
    const double* s1d = (const double*)s1->data;
    int ok            = s1->numel == 2 && fabs(s1d[0] - 6.0) < 1e-12 && fabs(s1d[1] - 15.0) < 1e-12;

    int d0           = 0;
    ReduceParams rp0 = {&d0, 1, false}; /* sum rows -> [5,7,9] */
    Tensor* s0       = uop_sum(ta, &rp0);
    tensor_ensure_executed(s0);
    const double* s0d = (const double*)s0->data;
    ok = ok && s0->numel == 3 && fabs(s0d[0] - 5.0) < 1e-12 && fabs(s0d[1] - 7.0) < 1e-12 &&
         fabs(s0d[2] - 9.0) < 1e-12;
...
static int test_cast_precision(void) {
    ...
    Tensor* ta  = tensor_from_data(a, (int[]){2}, 1, &cfg_i64);
    Tensor* tb =
        tensor_cast(ta, DTYPE_INT32); /* int64 -> int32 truncates value, but low bits exact */
...
static int test_comparisons(void) {
    ...
    Tensor* lt = uop_cmplt(ta, tb);
    tensor_ensure_executed(lt); /* [1,0,0,0] */
    Tensor* gt = uop_cmpgt(ta, tb);
    tensor_ensure_executed(gt);                   /* [0,1,1,0] */
    const uint8_t* ld = (const uint8_t*)lt->data; /* bool storage is uint8 */
    const uint8_t* gd = (const uint8_t*)gt->data;
    int ok = lt->dtype == DTYPE_BOOL && gt->dtype == DTYPE_BOOL && ld[0] == 1 && ld[1] == 0 &&
             ld[2] == 0 && ld[3] == 0 && gd[0] == 0 && gd[1] == 1 && gd[2] == 1 && gd[3] == 0;
    ...
    ok = ok && masked->dtype == DTYPE_FLOAT32 && mk[0] == 10.0f && mk[1] == 0.0f && mk[2] == 0.0f &&
         mk[3] == 0.0f;
...
static int test_matmul(void) {
    ...
    int ok           = c->dtype == DTYPE_FLOAT64 && c->numel == 4 && fabs(cd[0] - 58) < 1e-9 &&
             fabs(cd[1] - 64) < 1e-9 && fabs(cd[2] - 139) < 1e-9 && fabs(cd[3] - 154) < 1e-9;
    ...
    ok = ok && ci->dtype == DTYPE_INT32 && cid[0] == 19 && cid[1] == 22 && cid[2] == 43 &&
         cid[3] == 50;
...
static int test_conv2d(void) {
    ...
    const double* cd = (const double*)c->data;
    int ok           = c && c->dtype == DTYPE_FLOAT64 && c->numel == 4 && fabs(cd[0] - 6) < 1e-9 &&
             fabs(cd[1] - 8) < 1e-9 && fabs(cd[2] - 12) < 1e-9 && fabs(cd[3] - 14) < 1e-9;
    tensor_free(ti);
    tensor_free(tw);
    tensor_free(c);
    return ok;
