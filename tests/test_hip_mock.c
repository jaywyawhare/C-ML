...
static int test_roundtrip_and_order(void) {
    ...
    void* dev = NULL;
    if (backend->hipMalloc(&dev, sizeof(host)) != 0)
        return 0;

    if (cml_hcq_rocm_memcpy_h2d(q, dev, host, sizeof(host)) != 0)
        return 0;
    ...
    if (backend->hipModuleGetFunction(&k.function, NULL, kn) != 0)
        return 0;
    ...
    if (cml_hcq_rocm_submit_kernel(q, &desc) != 0)
        return 0;

    float back[64] = {0};
    if (cml_hcq_rocm_memcpy_d2h(q, back, dev, sizeof(host)) != 0)
        return 0;
    for (int i = 0; i < 64; i++)
        if (back[i] != host[i])
            return 0; /* payload survived the roundtrip */

    /* signal lifecycle: record then queue wait */
    CMLHCQSignal* sig = cml_hcq_rocm_signal_create();
    if (!sig)
        return 0;
    if (cml_hcq_rocm_signal_record(q, sig) != 0)
        return 0;
    if (cml_hcq_rocm_queue_wait(q, sig) != 0)
        return 0;
    cml_hcq_rocm_signal_destroy(sig);

    if (cml_hcq_rocm_queue_synchronize(q) != 0)
        return 0;
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (!e)
            break;
        if (e->kind == CML_HIP_MOCK_OP_MEMCPY_H2D && i_h2d < 0)
            i_h2d = i;
        if (e->kind == CML_HIP_MOCK_OP_LAUNCH && i_launch < 0)
            i_launch = i;
        if (e->kind == CML_HIP_MOCK_OP_MEMCPY_D2H && i_d2h < 0)
            i_d2h = i;
    }
    if (!(i_h2d >= 0 && i_launch > i_h2d && i_d2h > i_launch))
        return 0;
    if (cml_hip_mock_launches() != 1)
        return 0;
    if (cml_hip_mock_outstanding_allocs() != 0)
        return 0; /* all device buffers freed */
    return 1;
}
...
static int test_event_lifecycle(void) {
    ...
    CMLROCmBackend b;
    memset(&b, 0, sizeof(b));
    if (cml_rocm_backend_init_mock(&b) != 0)
        return 0;
    if (strcmp(b.device_name, "Mock-RDNA3") != 0)
        return 0;
    void* ev = NULL;
    if (b.hipEventCreate(&ev) != 0)
        return 0;
    if (b.hipEventRecord(ev, b.stream) != 0)
        return 0;
    if (b.hipEventSynchronize(ev) != 0)
        return 0;
