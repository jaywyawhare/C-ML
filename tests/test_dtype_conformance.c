...
static double dtype_tol(DType d, double want) {
    switch (d) {
    case DTYPE_FLOAT16:
        return fabs(want) * 1e-3 + 1e-3;
    case DTYPE_BFLOAT16:
        return fabs(want) * 5e-3 + 5e-3;
    default:
        return fabs(want) * 1e-5 + 1e-5;
    }
}
...
static bool dtype_holds(DType d, double v) {
    switch (d) {
    case DTYPE_INT8:
        return v >= -128.0 && v <= 127.0;
    case DTYPE_UINT8:
        return v >= 0.0 && v <= 255.0;
    case DTYPE_INT16:
        return v >= -32768.0 && v <= 32767.0;
    default:
        return true;
    }
}
...
static void run_case(const Case* c, DType dt) {
    ...
    for (int pass = 0; pass < 2; pass++) {
        ...
        for (int i = 0; i < c->n_in; i++) {
            ...
            if (!ins[i]) {
                printf("  %-22s %s: alloc failed\n", c->name, dtype_name(dt));
                return;
            }
        }
        ...
        if (pass == 0) {
            ...
            for (size_t i = 0; i < ref_n; i++)
                ref[i] = (double)tensor_get_float(r, i);
        } else {
            if (r->numel != ref_n) {
            ...
            for (size_t i = 0; i < ref_n; i++) {
                ...
                if (fabs(got - want) > tol) {
                    printf("  %-22s %s: [%zu] got %g, expected %g (f32 gives %g)\n", c->name,
                           dtype_name(dt), i, got, want, ref[i]);
                    cml_reset_ir_context();
                    return;
                }
            }
...
#define B(name, expr)                                                                              \
    static Tensor* name(Tensor** in, int n) {                                                      \
        (void)in;                                                                                  \
        (void)n;                                                                                   \
        return (expr);                                                                             \
    }
...
static Tensor* b_topk(Tensor** in, int n) {
    (void)n;
    Tensor* idx = NULL;
    return uop_topk(in[0], 2, 1, true, &idx);
}
static Tensor* b_sum_all(Tensor** in, int n) {
    (void)n;
    ReduceParams rp = {.dims = NULL, .num_dims = 0, .keepdim = false};
...
static Tensor* b_reshape(Tensor** in, int n) {
    (void)n;
    return uop_reshape_to(in[0], (int[]){3, 2}, 2);
}
...
static Tensor* b_cat0(Tensor** in, int n) { return uop_cat(in, n, 0); }
...
static Tensor* b_conv2d(Tensor** in, int n) {
    ...
    for (int i = 0; i < 4; i++)
        tensor_set_float(w, i, (float)wv[i]);
    static int ks[2] = {2, 2}, st[2] = {1, 1}, pd[2] = {0, 0}, dl[2] = {1, 1};
    Conv2DParams p = {
        .kernel_size = ks, .stride = st, .padding = pd, .dilation = dl, .groups = 1, .bias = false};
...
static void run_int_cases(DType dt) {
    ...
    (void)shl_;
    const IntCase cases[] = {
        {"bitwise_and", i_and, and_}, {"bitwise_or", i_or, or_}, {"bitwise_xor", i_xor, xor_},
        {"mod", i_mod, mod_},         {"idiv", i_div, div_},
    };
    int shape[1] = {4};
    ...
    int shl_ok               = 0;
    if (!r) {
        printf("  lshift %s: NULL\n", dtype_name(dt));
    } else {
        tensor_ensure_executed(r);
        shl_ok = 1;
        for (int i = 0; i < 4; i++) {
            if (!dtype_holds(dt, want_shl[i]))
                continue;
            double got = (double)tensor_get_float(r, (size_t)i);
            if (fabs(got - want_shl[i]) > 1e-6) {
                printf("  %-22s %s: [%d] got %g, want %g\n", "lshift", dtype_name(dt), i, got,
                       want_shl[i]);
                shl_ok = 0;
                break;
            }
