...
static int test_where(void) {
    ...
    if (!APPROX(out[0], 10.0f))
        return 0;
    if (!APPROX(out[1], -2.0f))
        return 0;
    if (!APPROX(out[2], 30.0f))
        return 0;
    if (!APPROX(out[3], -4.0f))
        return 0;
...
static int test_one_hot(void) {
    ...
    if (!APPROX(out[0], 1.0f))
        return 0;
    if (!APPROX(out[1], 0.0f))
        return 0;
    if (!APPROX(out[4], 0.0f))
        return 0;
    if (!APPROX(out[6], 1.0f))
        return 0;
    if (!APPROX(out[9], 1.0f))
        return 0;
...
static int test_roll(void) {
    ...
    float* out = (float*)result->data;
    // roll by 2: [4,5,1,2,3]
    if (!APPROX(out[0], 4.0f))
        return 0;
    if (!APPROX(out[1], 5.0f))
        return 0;
    if (!APPROX(out[2], 1.0f))
        return 0;
    if (!APPROX(out[3], 2.0f))
        return 0;
    if (!APPROX(out[4], 3.0f))
        return 0;
...
static int test_nonzero(void) {
    ...
    if (!APPROX(out[0], 0.0f))
        return 0;
    if (!APPROX(out[1], 1.0f))
        return 0;
    if (!APPROX(out[2], 1.0f))
        return 0;
    if (!APPROX(out[3], 0.0f))
        return 0;
    if (!APPROX(out[4], 1.0f))
        return 0;
    if (!APPROX(out[5], 1.0f))
        return 0;
...
static int test_logaddexp(void) {
    ...
    if (!APPROX(out[0], expected0))
        return 0;
    if (!APPROX(out[1], expected1))
        return 0;
...
static int test_einsum_matmul(void) {
    ...
    if (result->ndim != 2 || result->shape[0] != 2 || result->shape[1] != 2)
        return 0;

    float* out = (float*)result->data;
    // [1*7+2*9+3*11, 1*8+2*10+3*12] = [58, 64]
    // [4*7+5*9+6*11, 4*8+5*10+6*12] = [139, 154]
    if (!APPROX(out[0], 58.0f))
        return 0;
    if (!APPROX(out[1], 64.0f))
        return 0;
    if (!APPROX(out[2], 139.0f))
        return 0;
    if (!APPROX(out[3], 154.0f))
        return 0;
...
static int test_einsum_diagonal(void) {
    ...
    if (result->ndim != 1 || result->shape[0] != 3)
        return 0;
...
static int test_einsum_outer(void) {
    ...
    if (result->ndim != 2 || result->shape[0] != 3 || result->shape[1] != 2)
        return 0;
    ...
    if (!APPROX(out[1], 5.0f))
        return 0;
    if (!APPROX(out[2], 8.0f))
        return 0;
    if (!APPROX(out[3], 10.0f))
        return 0;
    if (!APPROX(out[4], 12.0f))
        return 0;
    if (!APPROX(out[5], 15.0f))
        return 0;
...
static int test_einsum_transpose(void) {
    ...
    if (!APPROX(out[0], 1.0f))
        return 0;
    if (!APPROX(out[1], 4.0f))
        return 0;
    if (!APPROX(out[2], 2.0f))
        return 0;
    if (!APPROX(out[3], 5.0f))
        return 0;
    if (!APPROX(out[4], 3.0f))
        return 0;
    if (!APPROX(out[5], 6.0f))
        return 0;
