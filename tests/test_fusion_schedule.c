...
static int test_can_fuse_elementwise(void) {
    ...
    if (!cml_schedule_can_fuse(UOP_SIN, UOP_COS))
        return 0;
    if (!cml_schedule_can_fuse(UOP_ADD, UOP_MUL))
        return 0;
    if (!cml_schedule_can_fuse(UOP_EXP, UOP_NEG))
        return 0;
    return 1;
}
...
static int test_is_elementwise_predicate(void) {
    if (!cml_schedule_is_elementwise(UOP_SIN))
        return 0;
    if (!cml_schedule_is_elementwise(UOP_ADD))
        return 0;
    if (!cml_schedule_is_elementwise(UOP_MUL))
        return 0;
    if (cml_schedule_is_elementwise(UOP_SUM))
        return 0;
    if (cml_schedule_is_elementwise(UOP_MATMUL))
        return 0;
    return 1;
}

static int test_is_reduction_predicate(void) {
    if (!cml_schedule_is_reduction(UOP_SUM))
        return 0;
    if (!cml_schedule_is_reduction(UOP_MAX_REDUCE))
        return 0;
    if (!cml_schedule_is_reduction(UOP_MEAN))
        return 0;
    if (cml_schedule_is_reduction(UOP_ADD))
        return 0;
    if (cml_schedule_is_reduction(UOP_SIN))
        return 0;
    return 1;
}

static int test_is_movement_predicate(void) {
    if (!cml_schedule_is_movement(UOP_RESHAPE))
        return 0;
    if (!cml_schedule_is_movement(UOP_PERMUTE))
        return 0;
    if (cml_schedule_is_movement(UOP_SIN))
        return 0;
    if (cml_schedule_is_movement(UOP_SUM))
        return 0;
    return 1;
}
