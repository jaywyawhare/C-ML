...
static int test_quad_double_grad_finite_diff(void) {
    ...
    for (int i = 0; i < n; i++) {
        float saved = xs[i];
        xs[i]       = saved + eps;
        float fp    = quad_penalty(xs, n);
        xs[i]       = saved - eps;
        float fm    = quad_penalty(xs, n);
        xs[i]       = saved;
...
static float matmul_penalty(const float* xs, const float* ws, int m, int k, int p) {
    cml_reset_ir_context();
...
static int test_matmul_double_grad_finite_diff(void) {
    ...
    for (int i = 0; i < m * k; i++) {
        float saved = xs[i];
        xs[i]       = saved + eps;
        float fp    = matmul_penalty(xs, ws, m, k, p);
        xs[i]       = saved - eps;
        float fm    = matmul_penalty(xs, ws, m, k, p);
        xs[i]       = saved;
        if (isnan(fp) || isnan(fm))
            return 0;
        float num = (fp - fm) / (2.0f * eps);
...
static int test_composite_vjp_double_grad(void) {
    ...
    for (int i = 0; i < n; i++) {
        float saved = xs[i];
        xs[i]       = saved + eps;
        float fp    = qgelu_penalty(xs, n);
        xs[i]       = saved - eps;
        float fm    = qgelu_penalty(xs, n);
        xs[i]       = saved;
        if (isnan(fp) || isnan(fm))
            return 0;
        float num = (fp - fm) / (2.0f * eps);
...
static int test_no_create_graph_grad_inert(void) {
    ...
    if (!ok)
        printf("    grad missing or unexpectedly differentiable");
    cml_reset_ir_context();
    return ok;
}
