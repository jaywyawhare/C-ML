...
static int test_var_mean(void) {
    ...
    if (!APPROX(mv[1], 5.0f))
        return 0;
    if (!APPROX(vv[0], 2.0f / 3.0f))
        return 0;
    if (!APPROX(vv[1], 2.0f / 3.0f))
        return 0;
    tensor_free(t);
    tensor_free(out_var);
...
static int test_std_mean(void) {
    ...
    if (!APPROX(ss[0], sqrtf(2.0f / 3.0f)))
        return 0;
    if (!APPROX(ss[1], sqrtf(2.0f / 3.0f)))
        return 0;
    tensor_free(t);
    tensor_free(out_std);
    tensor_free(out_mean);
...
static int test_conv_transpose1d(void) {
    ConvTranspose1d* layer =
        nn_conv_transpose1d(1, 1, 3, 1, 0, 0, false, DTYPE_FLOAT32, DEVICE_CPU);
    if (!layer)
        return 0;
...
static int test_dtype_int16(void) {
    TensorConfig cfg = {
        .dtype = DTYPE_INT16, .device = DEVICE_CPU, .has_dtype = true, .has_device = true};
    int shape[] = {4};
    Tensor* t   = tensor_ones(shape, 1, &cfg);
    if (!t)
        return 0;
    if (cml_dtype_size(DTYPE_INT16) != 2)
        return 0;
...
static int test_dtype_uint16(void) {
    TensorConfig cfg = {
        .dtype = DTYPE_UINT16, .device = DEVICE_CPU, .has_dtype = true, .has_device = true};
    int shape[] = {4};
...
static int test_dtype_uint32(void) {
    TensorConfig cfg = {
        .dtype = DTYPE_UINT32, .device = DEVICE_CPU, .has_dtype = true, .has_device = true};
    int shape[] = {3};
    Tensor* t   = tensor_ones(shape, 1, &cfg);
    if (!t)
        return 0;
    if (cml_dtype_size(DTYPE_UINT32) != 4)
        return 0;
...
static int test_dtype_uint64(void) {
    TensorConfig cfg = {
        .dtype = DTYPE_UINT64, .device = DEVICE_CPU, .has_dtype = true, .has_device = true};
    int shape[] = {2};
    Tensor* t   = tensor_full(shape, 1, &cfg, 12345.0f);
    if (!t)
        return 0;
    if (cml_dtype_size(DTYPE_UINT64) != 8)
        return 0;
    float v = tensor_get_float(t, 0);
    if (!APPROX(v, 12345.0f))
        return 0;
    tensor_free(t);
    return 1;
}
...
static int test_maxpool3d(void) {
    ...
    for (int i = 0; i < 64; i++)
        tensor_set_float(input, i, (float)i);
...
static int test_avgpool3d(void) {
    ...
    if (ok)
        ok = fabsf(tensor_get_float(out, 0) - 4.5f) < 0.01f;
...
static int test_adaptive_maxpool2d(void) {
    ...
    if (ok)
        ok = (tensor_get_float(out, 0) == 6.0f && tensor_get_float(out, 1) == 8.0f &&
              tensor_get_float(out, 2) == 14.0f && tensor_get_float(out, 3) == 16.0f);
...
static int test_adaptive_maxpool1d(void) {
    ...
    if (ok)
        ok = (tensor_get_float(out, 0) == 3.0f && tensor_get_float(out, 1) == 4.0f);
...
static int test_qr(void) {
    ...
    int ok =
        (qr.Q->shape[0] == 3 && qr.Q->shape[1] == 2 && qr.R->shape[0] == 2 && qr.R->shape[1] == 2);
...
static int test_svd(void) {
    ...
    // U: [2, 2], S: [2], Vt: [2, 3]
    int ok = (svd.U->shape[0] == 2 && svd.U->shape[1] == 2 && svd.S->shape[0] == 2 &&
    ...
    if (ok) {
        for (int i = 0; i < 2 && ok; i++) {
            for (int j = 0; j < 3 && ok; j++) {
                float val = 0;
                for (int k = 0; k < 2; k++) {
                    val += tensor_get_float(svd.U, i * 2 + k) * tensor_get_float(svd.S, k) *
...
static int test_fp8_e4m3(void) {
    int shape[]      = {3};
    TensorConfig cfg = {
        .dtype = DTYPE_FLOAT8_E4M3, .device = DEVICE_CPU, .has_dtype = true, .has_device = true};
...
static int test_fp8_e5m2(void) {
    int shape[]      = {3};
    TensorConfig cfg = {
        .dtype = DTYPE_FLOAT8_E5M2, .device = DEVICE_CPU, .has_dtype = true, .has_device = true};
    Tensor* t = tensor_empty(shape, 1, &cfg);
    tensor_ensure_executed(t);
...
static int test_transformer_decoder_layer(void) {
    TransformerDecoderLayer* layer =
        nn_transformer_decoder_layer(8, 2, 16, 0.0f, DTYPE_FLOAT32, DEVICE_CPU);
    if (!layer)
        return 0;
