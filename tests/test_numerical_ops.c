...
#define MAKE1D(arr)                                                                                \
    tensor_from_data((arr), (int[]){(int)(sizeof(arr) / sizeof(arr[0]))}, 1, &cpu_f32)
...
static int test_logical_and_or(void) {
    ...
    float* do_ = lor->data;
    for (int i = 0; i < n; i++) {
        if (!APPROX(da[i], eand[i]))
            return 0;
        if (!APPROX(do_[i], eor[i]))
            return 0;
    }
    tensor_free(ta);
    tensor_free(tb);
...
static int test_comparison_ops(void) {
    ...
    if (eq->dtype != DTYPE_BOOL || gt->dtype != DTYPE_BOOL)
        return 0;
    float eeq[] = {1, 0, 0, 0}, ene[] = {0, 1, 1, 1};
    float ele[] = {1, 0, 1, 0}, egt[] = {0, 1, 0, 1}, ege[] = {1, 1, 0, 1};
    for (int i = 0; i < n; i++) {
        if (!APPROX(tensor_get_float(eq, i), eeq[i]))
            return 0;
        if (!APPROX(tensor_get_float(ne, i), ene[i]))
            return 0;
        if (!APPROX(tensor_get_float(le, i), ele[i]))
            return 0;
        if (!APPROX(tensor_get_float(gt, i), egt[i]))
            return 0;
        if (!APPROX(tensor_get_float(ge, i), ege[i]))
            return 0;
    }
    tensor_free(ta);
    tensor_free(tb);
...
static int test_logaddexp(void) {
    ...
    if (!APPROX(d[0], logf(2.0f)))
        return 0;

    if (!APPROX_REL(d[1], logf(expf(1.0f) + expf(2.0f))))
        return 0;
    tensor_free(ta);
    tensor_free(tb);
    tensor_free(out);
