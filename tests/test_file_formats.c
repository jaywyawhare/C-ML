...
static int test_dataset_bad_files(void) {
    ...
    /* IDX files: bad magic */
    const char bad_magic[] = "\x00\x00\x09\x09" "\x00\x00\x00\x01" "\x42";
...
static int test_dataset_lifecycle(void) {
    ...
    if (dl) {
        ...
        while ((b = dataloader_next_batch(dl)) != NULL) {
            ...
            if (in && b->batch_size > 0) tensor_ensure_executed(in);
            batch_free(b);
            batches++;
            if (batches > 100) break;   /* loop guard */
        }
        ok &= batches > 0;
        dataloader_free(dl);
    }
    ...
    if (d2) {
        ...
        if (tr) { dataset_free(tr); tr = NULL; }
        if (te) { dataset_free(te); te = NULL; }
        dataset_free(d2);
    }

    dataset_free(ds);
    if (tr) dataset_free(tr);
    if (va) dataset_free(va);
    if (te) dataset_free(te);
    return ok;
}
...
static int test_tensor_serialization_roundtrip_and_garbage(void) {
    ...
    if (r) {
        ...
        for (int i = 0; i < 6; i++) ok &= fabsf(rd[i] - d[i]) < 1e-6f;
        tensor_free(r);
    }
    ...
    /* truncated payload: valid magic/header, cut short */
    {
        ...
        if (f) {
            ...
            fclose(f);
            if (n > 16) write_bytes("trunc.bin", buf, n - 12);
...
static int test_optimizer_serialization(void) {
    ...
    par.tensor = w; par.requires_grad = true; par.name = nm;
    p = &par;
