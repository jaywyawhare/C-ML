...
int main(void) {
    ...
    if (!cp) { printf("test_tp_autograd: create failed\n[FAIL]\n"); return 1; }
    ...
    if (!out) { printf("test_tp_autograd: forward returned NULL\n[FAIL]\n"); return 1; }
    ...
    for (int j = 0; j < 4 && ok; j++) {
        ...
        if (fabsf(od[j] - ref) > EPS) { printf("forward mismatch out[%d]=%f want %f\n", j, od[j], ref); ok = 0; }
    }
    ...
    if (!g) { printf("test_tp_autograd: weight grad is NULL (TP still inference-only)\n[FAIL]\n"); return 1; }
    tensor_ensure_executed(g);
    const float* gd = (const float*)tensor_data_ptr(g);
    if (!gd) { printf("test_tp_autograd: weight grad has no data\n[FAIL]\n"); return 1; }
    ...
    if (ok) { printf("test_tp_autograd: PASSED (weight gradient flows through TP forward)\n"); return 0; }
