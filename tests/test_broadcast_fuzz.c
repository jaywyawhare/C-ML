...
static void bcast_strides(const int* shape, int ndim, const int* out_shape, int out_ndim,
    ...
    size_t acc = 1;
    size_t own[MAXD];
    for (int i = ndim - 1; i >= 0; i--) { own[i] = acc; acc *= (size_t)shape[i]; }
    for (int i = 0; i < out_ndim; i++) strides[i] = 0;
...
typedef enum {
    BOP_ADD, BOP_SUB, BOP_MUL, BOP_DIV, BOP_MAX, BOP_MIN, BOP_POW,
    BOP_CMPLT, BOP_CMPGT, BOP_CMPLE, BOP_CMPGE, BOP_CMPEQ, BOP_CMPNE,
    BOP_COPYSIGN, BOP_LOGADDEXP, BOP_LOGICAL_AND, BOP_LOGICAL_OR,
    BOP_IDIV, BOP_MOD, BOP_AND, BOP_OR, BOP_XOR, BOP_SHL, BOP_SHR,
    NUM_OPS
} OpSel;

static const char* op_name(int o) {
    static const char* n[NUM_OPS] = {
        "add","sub","mul","div","max","minimum","pow",
        "cmplt","cmpgt","cmple","cmpge","cmpeq","cmpne",
        "copysign","logaddexp","logical_and","logical_or",
        "idiv","mod","bitwise_and","bitwise_or","bitwise_xor","lshift","rshift"};
    return n[o];
}

static Tensor* build_op(int o, Tensor* a, Tensor* b) {
    switch (o) {
    case BOP_ADD: return uop_add(a, b);
    case BOP_SUB: return uop_sub(a, b);
    case BOP_MUL: return uop_mul(a, b);
    case BOP_DIV: return uop_div(a, b);
    case BOP_MAX: return uop_max(a, b);
    case BOP_MIN: return uop_minimum(a, b);
    case BOP_POW: return uop_pow(a, b);
    case BOP_CMPLT: return uop_cmplt(a, b);
    case BOP_CMPGT: return uop_cmpgt(a, b);
    case BOP_CMPLE: return uop_cmple(a, b);
    case BOP_CMPGE: return uop_cmpge(a, b);
    case BOP_CMPEQ: return uop_cmpeq(a, b);
    case BOP_CMPNE: return uop_cmpne(a, b);
    case BOP_COPYSIGN: return uop_copysign(a, b);
    case BOP_LOGADDEXP: return uop_logaddexp(a, b);
    case BOP_LOGICAL_AND: return uop_logical_and(a, b);
    case BOP_LOGICAL_OR: return uop_logical_or(a, b);
    case BOP_IDIV: return uop_idiv(a, b);
    case BOP_MOD: return uop_mod(a, b);
    case BOP_AND: return uop_bitwise_and(a, b);
    case BOP_OR:  return uop_bitwise_or(a, b);
    case BOP_XOR: return uop_bitwise_xor(a, b);
    case BOP_SHL: return uop_lshift(a, b);
    case BOP_SHR: return uop_rshift(a, b);
    default: return NULL;
    }
}
...
static float ref_op(int o, float x, float y) {
    ...
    case BOP_MAX: return x > y ? x : y;
    case BOP_MIN: return x < y ? x : y;
    case BOP_POW: return powf(x, y);
    case BOP_CMPLT: return x <  y ? 1.0f : 0.0f;
    case BOP_CMPGT: return x >  y ? 1.0f : 0.0f;
    case BOP_CMPLE: return x <= y ? 1.0f : 0.0f;
    case BOP_CMPGE: return x >= y ? 1.0f : 0.0f;
    case BOP_CMPEQ: return x == y ? 1.0f : 0.0f;
    case BOP_CMPNE: return x != y ? 1.0f : 0.0f;
    case BOP_COPYSIGN: return copysignf(x, y);
    case BOP_LOGADDEXP: {
        float m = x > y ? x : y;
        return m + logf(expf(x - m) + expf(y - m));
    }
    case BOP_LOGICAL_AND: return (x != 0.0f && y != 0.0f) ? 1.0f : 0.0f;
    case BOP_LOGICAL_OR:  return (x != 0.0f || y != 0.0f) ? 1.0f : 0.0f;
    case BOP_IDIV: return floorf(x / y);   /* documented as floor(a/b) */
    case BOP_MOD:  return fmodf(x, y);
    case BOP_AND:  return (float)((int32_t)x & (int32_t)y);
    case BOP_OR:   return (float)((int32_t)x | (int32_t)y);
    case BOP_XOR:  return (float)((int32_t)x ^ (int32_t)y);
    case BOP_SHL:  return (float)((int32_t)x << (int32_t)y);
    case BOP_SHR:  return (float)((int32_t)x >> (int32_t)y);
    default: return 0.0f;
    }
}
...
static void op_ranges(int o, int* alo, int* ahi, int* blo, int* bhi) {
    *alo = -5; *ahi = 5; *blo = -5; *bhi = 5;
    switch (o) {
    case BOP_DIV: case BOP_MOD: case BOP_IDIV: *blo = 1; *bhi = 5; break;
    case BOP_POW: *alo = 1; *ahi = 4; *blo = 0; *bhi = 3; break;
    case BOP_SHL: case BOP_SHR: *alo = 0; *ahi = 7; *blo = 0; *bhi = 3; break;
    case BOP_AND: case BOP_OR: case BOP_XOR: *alo = 0; *ahi = 15; *blo = 0; *bhi = 15; break;
    case BOP_LOGADDEXP: *alo = -3; *ahi = 3; *blo = -3; *bhi = 3; break;
    default: break;
    }
}
...
static void one_case(int iter, int opsel) {
    ...
    for (int i = 0; i < and_; i++) ash[i] = rng_int(0, 2) ? rng_int(1, 4) : 1;
    for (int i = 0; i < bnd; i++) bsh[i] = rng_int(0, 2) ? rng_int(1, 4) : 1;
    ...
    for (int k = 1; k <= ond; k++) {
        ...
        if (av != bv && av != 1 && bv != 1) return;   /* incompatible: skip */
    ...
    if (!a || !b) { failures++; cml_reset_ir_context(); return; }
    ...
    for (int i = 0; !bad_shape && i < ond; i++)
        if (r->shape[i] != osh[i]) bad_shape = 1;
    if (bad_shape) {
        printf("  %s iter %d: shape [", op_name(opsel), iter);
        for (int i = 0; i < r->ndim; i++) printf("%d%s", r->shape[i], i + 1 < r->ndim ? "," : "");
        printf("], expected [");
        for (int i = 0; i < ond; i++) printf("%d%s", osh[i], i + 1 < ond ? "," : "");
        printf("]   a=[");
        for (int k = 0; k < and_; k++) printf("%d%s", ash[k], k + 1 < and_ ? "," : "");
        printf("] b=[");
        for (int k = 0; k < bnd; k++) printf("%d%s", bsh[k], k + 1 < bnd ? "," : "");
        printf("]\n");
        failures++;
        cml_reset_ir_context();
        return;
    ...
    for (size_t i = 0; i < r->numel; i++) {
        ...
        float tol  = (opsel == BOP_POW || opsel == BOP_LOGADDEXP || opsel == BOP_DIV)
                         ? (fabsf(want) * 1e-5f + 1e-5f) : 0.0f;
        if (fabsf(got - want) > tol) {
            printf("  %s iter %d: [%zu] = %g, expected %g   a=[",
                   op_name(opsel), iter, i, got, want);
            for (int k = 0; k < and_; k++) printf("%d%s", ash[k], k + 1 < and_ ? "," : "");
            printf("] b=[");
            for (int k = 0; k < bnd; k++) printf("%d%s", bsh[k], k + 1 < bnd ? "," : "");
            printf("] out=[");
            for (int k = 0; k < ond; k++) printf("%d%s", osh[k], k + 1 < ond ? "," : "");
            printf("]\n");
            failures++;
            break;
        }
...
int main(void) {
    ...
    for (int op = 0; op < NUM_OPS && failures < 12; op++)
        for (int i = 0; i < ITERS && failures < 12; i++) one_case(i, op);
