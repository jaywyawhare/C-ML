...
static uint32_t encode_alu(int subop, int rd, int rs, int rt) {
    return (0x1 << 28) | ((uint32_t)(subop & 0xF) << 24) | ((uint32_t)(rd & 0x1F) << 19) |
           ((uint32_t)(rs & 0x1F) << 14) | ((uint32_t)(rt & 0x1F) << 9);
}

static uint32_t encode_store(int rs, int rt, int16_t offset) {
    return (0x3 << 28) | ((uint32_t)(rs & 0x1F) << 14) | ((uint32_t)(rt & 0x1F) << 9) |
           ((uint32_t)offset & 0x1FF);
}

static uint32_t encode_load(int rd, int rs, int16_t offset) {
    return (0x2 << 28) | ((uint32_t)(rd & 0x1F) << 19) | ((uint32_t)(rs & 0x1F) << 14) |
           ((uint32_t)offset & 0x1FF);
}
...
static bool test_alu_add(void) {
    CMLHexagonSim* sim = cml_hexagon_sim_create(4096);
    uint32_t prog[]    = {encode_movi(1, 10), encode_movi(2, 20),
...
static bool test_alu_sub(void) {
    CMLHexagonSim* sim = cml_hexagon_sim_create(4096);
    uint32_t prog[]    = {encode_movi(1, 50), encode_movi(2, 20),
...
static bool test_alu_mul(void) {
    CMLHexagonSim* sim = cml_hexagon_sim_create(4096);
    uint32_t prog[]    = {encode_movi(1, 7), encode_movi(2, 6),
...
static bool test_load_store(void) {
    ...
    uint32_t prog[] = {encode_movi(1, 1024),   /* r1 = 1024 (base addr) */
...
static bool test_cycle_counting(void) {
    CMLHexagonSim* sim = cml_hexagon_sim_create(4096);
    uint32_t prog[]    = {encode_nop(), encode_nop(), encode_nop(), encode_halt()};
    cml_hexagon_sim_load(sim, prog, sizeof(prog), 0);
    cml_hexagon_sim_run(sim, 100);

    if (sim->cycle_count != 4) {
        cml_hexagon_sim_free(sim);
        return false;
    }
    cml_hexagon_sim_free(sim);
...
static bool test_r0_is_zero(void) {
    CMLHexagonSim* sim = cml_hexagon_sim_create(4096);
    uint32_t prog[]    = {encode_movi(0, 999), encode_halt()};
    cml_hexagon_sim_load(sim, prog, sizeof(prog), 0);
    cml_hexagon_sim_run(sim, 100);
