...
static int on_batch(int epoch, int batch, float loss, void* ud) {
    ...
    if (batch == 0) { if (g_first < 0.0f) g_first = loss; g_last = loss; }
    return 0;
}
...
int main(void) {
    ...
    for (int s = 0; s < N; s++) {
        float acc = 0;
        for (int j = 0; j < in; j++) { X[s*in+j] = 0.1f*((s*in+j)%5) - 0.2f; acc += X[s*in+j]*(j+1); }
    ...
    TrainingConfig cfg; training_config_default(&cfg);
    cfg.epochs = 150; cfg.verbose = false; cfg.use_progress_bar = false;
    ...
    int fail = 0;
    if (!(g_last < g_first * 0.5f)) { printf("  FAIL  static training converges\n"); fail = 1; }
    else                            { printf("  PASS  static training converges (zero-rebuild)\n"); }
