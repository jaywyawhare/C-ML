...
static Tensor* make(const int* shape, int ndim, int seed, int lo, int hi) {
    ...
    for (size_t i = 0; i < t->numel; i++)
        tensor_set_float(t, i, (float)(lo + (int)((i * 3 + (size_t)seed * 7) % (size_t)(hi - lo + 1))));
    return t;
}
...
static void one_case(int iter, int opsel) {
    ...
    for (int k = 0; k < 3; k++) {
        nd[k] = rng_int(1, MAXD);
        for (int i = 0; i < nd[k]; i++) sh[k][i] = rng_int(0, 2) ? rng_int(1, 3) : 1;
    }

    /* numpy rule over all three, right-aligned. */
    int ond = nd[0] > nd[1] ? nd[0] : nd[1];
    if (nd[2] > ond) ond = nd[2];
    int osh[MAXD];
    for (int k = 1; k <= ond; k++) {
        int mx = 1;
        for (int t = 0; t < 3; t++) {
            ...
            if (v != 1 && mx != 1 && v != mx) return;   /* incompatible: skip */
            if (v > mx) mx = v;
        }
        osh[ond - k] = mx;
    }
    ...
    if (!t0 || !t1 || !t2) { failures++; cml_reset_ir_context(); return; }
    ...
    if (opsel == 0) {
        ...
        if (cml_ir_compute_broadcast_shape(node) != 0) { cml_reset_ir_context(); return; }
    ...
    int bad = (r->ndim != ond);
    for (int i = 0; !bad && i < ond; i++) if (r->shape[i] != osh[i]) bad = 1;
    if (bad) {
        ...
        printf("]   operands [");
        for (int k = 0; k < 3; k++) {
            for (int i = 0; i < nd[k]; i++) printf("%d%s", sh[k][i], i + 1 < nd[k] ? "," : "");
            printf(k < 2 ? "] [" : "]\n");
        }
        failures++;
        cml_reset_ir_context();
    ...
    size_t st[3][MAXD];
    for (int k = 0; k < 3; k++) bcast_strides(sh[k], nd[k], osh, ond, st[k]);
    Tensor* ops[3] = {t0, t1, t2};
    for (size_t i = 0; i < r->numel; i++) {
        float v[3];
        for (int k = 0; k < 3; k++)
            v[k] = tensor_get_float(ops[k], ref_index(st[k], osh, ond, i));
        float want = (opsel == 1) ? (v[0] + v[2] * (v[1] - v[0]))
                                  : (v[0] != 0.0f ? v[1] : v[2]);
        float got = tensor_get_float(r, i);
        if (fabsf(got - want) > fabsf(want) * 1e-5f + 1e-5f) {
            printf("  %s iter %d: [%zu] = %g, expected %g   operands [",
                   op_label(opsel), iter, i, got, want);
            for (int k = 0; k < 3; k++) {
                for (int j = 0; j < nd[k]; j++) printf("%d%s", sh[k][j], j + 1 < nd[k] ? "," : "");
                printf(k < 2 ? "] [" : "]");
            }
            printf(" out=[");
            for (int j = 0; j < ond; j++) printf("%d%s", osh[j], j + 1 < ond ? "," : "");
            printf("]\n");
            failures++;
            break;
        }
...
int main(void) {
    ...
    for (int op = 0; op < 3 && failures < 10; op++)
        for (int i = 0; i < ITERS && failures < 10; i++) one_case(i, op);
