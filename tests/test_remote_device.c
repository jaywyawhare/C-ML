...
static void test_connect_disconnect(void) {
    ...
    REQUIRE(dev != NULL);
    REQUIRE(cml_remote_is_connected(dev));
    REQUIRE(dev->session_id != 0);
...
static void test_null_args(void) {
    ...
    REQUIRE(cml_remote_connect(NULL, 1234) == NULL);
    REQUIRE(cml_remote_connect("localhost", 0) == NULL);
    REQUIRE(!cml_remote_is_connected(NULL));

    cml_remote_disconnect(NULL);
    REQUIRE(cml_remote_alloc(NULL, 100) == 0);
    REQUIRE(cml_remote_upload(NULL, 1, NULL, 0) == -1);
    REQUIRE(cml_remote_download(NULL, 1, NULL, 0) == -1);

    REQUIRE(cml_remote_server_create(0) == NULL);
    REQUIRE(cml_remote_server_run(NULL) == -1);
